...
class OptOutCustomDataService @Inject() (
    implicit val appConfig:          FrontendAppConfig,
    implicit val dateService:        DateServiceInterface,
    implicit val dynamicStubService: DynamicStubService)
    extends OptOutCustomDataUploadHelper {

  def uploadCalculationListData(
      nino:    Nino,
      taxYear: TaxYear,
      status:  String
    )(
    ...
    handleDefaultValues(status = status) {
      ...
      dynamicStubService.overwriteCalculationList(
        nino = nino,
        taxYearRange = taxYear.formatTaxYearRange,
  ...
  def uploadITSAStatusData(nino: Nino, taxYear: TaxYear, status: String)(implicit hc: HeaderCarrier): Future[Unit] = {
    handleDefaultValues(status = status) {
      ...
      dynamicStubService.overwriteItsaStatus(
        nino = nino,
        taxYearRange = taxYear.formatTaxYearRange,
        ITSAStatus = status
      )
  ...
  def stubITSAStatusFutureYearData(
      nino:                 String,
      taxYear:              TaxYear,
    ...
    Logger("application").info(
      s"cyMinusOneItsaStatus :: $cyMinusOneItsaStatus, cyItsaStatus:: $cyItsaStatus, cyPlusOneItsaStatus:: $cyPlusOneItsaStatus"
    )
    ...
    def taxYearFormatForITSA(taxYear: TaxYear): String =
      s"${taxYear.startYear}-${taxYear.endYear.toString.takeRight(2)}"
    ...
    val itsaStatusCombined: List[ITSAStatusResponseModel] =
      List(cyPlusOneItsaStatusResponse, cyItsaStatusResponse, cyMinusOneItsaStatusResponse)

    val payload: DataModel = DataModel(
      _id =
        s"/income-tax/$nino/person-itd/itsa-status/${taxYear.addYears(-1).formatTaxYearRange}?futureYears=true&history=false",
      schemaId = "getITSAStatusSuccess",
      method = "GET",
      status = OK,
      response = Some(Json.toJson(itsaStatusCombined))
    )
