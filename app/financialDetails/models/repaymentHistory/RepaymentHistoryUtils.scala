...
package financialDetails.models.repaymentHistory

import exceptions.MissingFieldException
import financialDetails.controllers.routes as financialDetailsRoutes
import financialDetails.models.{BalancingChargeCreditType, ChargeItem, CutOverCreditType, ITSAReturnAmendmentCredit, MfaCreditType, Payment, PaymentType, PoaOneReconciliationCredit, PoaTwoReconciliationCredit, Repayment, RepaymentInterest}
...
object RepaymentHistoryUtils {
  ...
  private def getControllerHref(transactionId: Option[String], isAgent: Boolean) = {
    if (isAgent) {
      financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocationAgent(transactionId.getOrElse(throw MissingFieldException("Transaction ID"))).url
    } else {
      financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocation(transactionId.getOrElse(throw MissingFieldException("Transaction ID"))).url
    }
  }
  ...
  private def getCreditsLinkUrl(date: LocalDate, isAgent: Boolean) = {
    ...
    if (isAgent) {
      financialDetailsRoutes.CreditsSummaryController.showAgentCreditsSummary(year).url
    } else {
      financialDetailsRoutes.CreditsSummaryController.showCreditsSummary(year).url
    }
  }
  ...
  def getChargeLinkUrl(isAgent: Boolean, taxYear: Int, chargeId: String, codedOut: Option[Boolean] = None): String = {
    val baseUrl =
      if (isAgent) {
        financialDetailsRoutes.ChargeSummaryController.showAgent(taxYear, chargeId).url
      } else {
        financialDetailsRoutes.ChargeSummaryController.show(taxYear, chargeId).url
      }
