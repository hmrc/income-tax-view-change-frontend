...
package financialDetails.models.liabilitycalculation.viewmodels
...
import financialDetails.models.SecondLatePaymentPenalty
...
case class TaxYearSummaryViewModel(
  ...
  def getChargeSummaryHref(chargeItem: TaxYearSummaryChargeItem,
    ...
    if (chargeItem.transactionType == SecondLatePaymentPenalty) {
      ...
      if (isAgent) {
        financialDetailsRoutes.ChargeSummaryController.showAgent(taxYear, chargeItem.transactionId, chargeItem.isAccruingInterest).url
      } else {
        financialDetailsRoutes.ChargeSummaryController.show(taxYear, chargeItem.transactionId, chargeItem.isAccruingInterest, origin).url
      }
    }
  }
}
