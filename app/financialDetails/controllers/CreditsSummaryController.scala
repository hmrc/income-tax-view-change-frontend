...
import financialDetails.views.html.CreditsSummaryView
...
class CreditsSummaryController @Inject()(creditsView: CreditsSummaryView,
  ...
  private def creditsSummaryUrl(calendarYear: Int, origin: Option[String]): String =
    routes.CreditsSummaryController.showCreditsSummary(calendarYear, origin).url

  lazy val moneyInYourAccountUrl: String = routes.MoneyInYourAccountController.show().url

  private def paymentHistoryUrl(origin: Option[String]): String =
    routes.PaymentHistoryController.show(origin).url

  private def agentCreditsSummaryUrl(calendarYear: Int): String =
    routes.CreditsSummaryController.showAgentCreditsSummary(calendarYear).url

  private lazy val agentMoneyInYourAccountUrl: String = routes.MoneyInYourAccountController.showAgent().url

  private lazy val agentPaymentHistoryHomeUrl: String = routes.PaymentHistoryController.showAgent().url
