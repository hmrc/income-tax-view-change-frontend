...
import financialDetails.models.repaymentHistory.{RepaymentHistoryErrorModel, RepaymentHistoryModel}
...
class RefundToTaxPayerController @Inject()(val refundToTaxPayerView: RefundToTaxPayerView,
  ...
  def show(repaymentRequestNumber: String,
    ...
    implicit user =>
      handleRequest(
        backUrl = routes.PaymentHistoryController.show(origin).url,
        origin = origin,
        itvcErrorHandler = itvcErrorHandler,
  ...
  def showAgent(repaymentRequestNumber: String): Action[AnyContent] = authActions.asMTDPrimaryAgent().async {
    implicit mtdItUser =>
      handleRequest(
        backUrl = routes.PaymentHistoryController.showAgent().url,
