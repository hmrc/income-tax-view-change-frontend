...
package financialDetails.controllers
...
import errors.routes as errorRoutes
import agent.errors.routes as agentErrorRoutes
import forms.utils.SessionKeys.gatewayPage
import models.admin.*
import financialDetails.models.chargeHistory.*
import models.core.Nino
import financialDetails.models.*
import financialDetails.models.chargeHistory.ChargeHistoryModel
import financialDetails.models.chargeSummary.{ChargeSummaryViewModel, PaymentHistoryAllocations}
import financialDetails.models.paymentAllocationCharges.PaymentAllocationViewModel
import financialDetails.services.{ChargeHistoryService, FinancialDetailsService, PaymentAllocationsService}
import financialDetails.utils.FallBackBackLinks
...
import financialDetails.views.html.{ChargeSummaryView, YourSelfAssessmentChargeSummaryView}
...
class ChargeSummaryController @Inject()(val authActions: AuthActions,
  ...
  def onError(message: String, isAgent: Boolean, showInternalServerError: Boolean)(implicit request: Request[_]): Result = {
    ...
    if (showInternalServerError) {
      ...
      if (isAgent) Redirect(agentErrorRoutes.AgentNotFoundDocumentIDLookupController.show().url)
      else Redirect(errorRoutes.NotFoundDocumentIDLookupController.show().url)
    }
  }
        ...
        if (chargeItem.isPenalty && !isEnabled(PenaltiesAndAppeals)) {
          ...
          chargeHistoryService.chargeHistoryResponse(isInterestCharge, chargeReference, isEnabled(ChargeHistory)).flatMap {
            case Right(chargeHistory) =>
              ...
              val whatYouOweUrl = {
                if (isAgent) routes.WhatYouOweController.showAgent().url
                else routes.WhatYouOweController.show(origin).url
              }
  ...
  private def checkForPoaExtraChargeLink(
    ...
    chargeItem match {
      case Some(value) =>
        ...
        extraChargeId match {
          case Some(validId) =>
            if (isAgent) Some(routes.ChargeSummaryController.showAgent(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
            else Some(routes.ChargeSummaryController.show(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
          case None => None
        }
      case None => None
    }
