...
import financialDetails.models.claimToAdjustPoa.{PoaAmendmentData, SelectYourReason}
import financialDetails.services.{NrsService, PaymentOnAccountSessionService}
import financialDetails.services.claimToAdjustPoa.{ClaimToAdjustPoaCalculationService, ClaimToAdjustService, RecalculatePoaHelper}
...
import financialDetails.views.html.claimToAdjustPoa.CheckYourAnswersView
            ...
            EitherT.rightT(
              Ok(
                checkYourAnswers(
                  ...
                  redirectUrl = routes.ConfirmationForAdjustingPoaController.show(user.isAgent).url,
                  changePoaAmountUrl = routes.EnterPoaAmountController.show(user.isAgent, CheckMode).url,
                  changePoaReasonUrl = routes.SelectYourReasonController.show(user.isAgent, CheckMode).url
                )
              )
            )
          }
