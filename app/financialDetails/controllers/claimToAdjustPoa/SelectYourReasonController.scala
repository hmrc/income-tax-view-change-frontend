...
import financialDetails.models.claimToAdjustPoa.{Increase, SelectYourReason}
    ...
    for {
      ...
      res match {
        case _ => (mode, poa.totalAmountLessThanPoa) match {
          case (NormalMode, false) if value != Increase => Redirect(routes.EnterPoaAmountController.show(user.isAgent, NormalMode))
          case (_, _) => Redirect(routes.CheckYourAnswersController.show(user.isAgent))
        }
      }
    }
  }
