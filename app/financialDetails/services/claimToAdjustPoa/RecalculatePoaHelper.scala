...
package financialDetails.services.claimToAdjustPoa
...
import financialDetails.models.claimToAdjustPoa.{ClaimToAdjustNrsPayload, PoaAmendmentData, SelectYourReason}
import financialDetails.models.claimToAdjustPoa.viewModels.PaymentOnAccountViewModel
import financialDetails.models.nrs.{IdentityData, NrsMetadata, NrsSubmission, RawPayload, SearchKeys}
      ...
      case PoaAmendmentData(Some(poaAdjustmentReason), Some(amount), _) =>
        ctaCalculationService.recalculate(nino, poa.taxYear, amount, poaAdjustmentReason) map {
          case Left(ex) =>
            ...
            Redirect(claimToAdjustPoaRoutes.ApiFailureSubmittingPoaController.show(user.isAgent))
          case Right(_) =>
            auditingService.extendedAudit(AdjustPaymentsOnAccountAuditModel(
              isSuccessful = true,
              previousPaymentOnAccountAmount = poa.totalAmountOne,
            ...
            Redirect(claimToAdjustPoaRoutes.PoaAdjustedController.show(user.isAgent))
        }
