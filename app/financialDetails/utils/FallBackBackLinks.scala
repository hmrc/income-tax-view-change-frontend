...
package financialDetails.utils
...
trait FallBackBackLinks {
  ...
  private def getIndividualPaymentAllocationBackUrl(gatewayPageOpt: Option[GatewayPage], taxYearOpt: Option[Int], origin: Option[String]): String =
    ((gatewayPageOpt, taxYearOpt) match {
      ...
      case (Some(WhatYouOwePage), _) => financialDetailsRoutes.WhatYouOweController.show(origin)
      case (Some(PaymentHistoryPage), _) => financialDetailsRoutes.PaymentHistoryController.show(origin)
  ...
  private def getAgentPaymentAllocationBackUrl(gatewayPageOpt: Option[GatewayPage], taxYearOpt: Option[Int]): String =
    ((gatewayPageOpt, taxYearOpt) match {
      ...
      case (Some(WhatYouOwePage), _) => financialDetailsRoutes.WhatYouOweController.showAgent()
      case (Some(PaymentHistoryPage), _) => financialDetailsRoutes.PaymentHistoryController.showAgent()
  ...
  private def getChargeSummaryBackUrlAgent(gatewayPageOpt: Option[GatewayPage], taxYear: Int): String =
    (gatewayPageOpt match {
      ...
      case Some(WhatYouOwePage) => financialDetailsRoutes.WhatYouOweController.showAgent()
      case Some(PaymentHistoryPage) => financialDetailsRoutes.PaymentHistoryController.showAgent()
      case _ => HomeController.showAgent()
    }).path
  ...
  private def getChargeSummaryBackUrlIndividual(gatewayPageOpt: Option[GatewayPage], taxYear: Int, origin: Option[String]): String =
    (gatewayPageOpt match {
      ...
      case Some(WhatYouOwePage) => financialDetailsRoutes.WhatYouOweController.show(origin)
      case Some(PaymentHistoryPage) => financialDetailsRoutes.PaymentHistoryController.show(origin)
      case _ => HomeController.show(origin)
    }).path
}
