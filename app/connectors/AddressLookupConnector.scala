...
class AddressLookupConnector @Inject()(val appConfig: FrontendAppConfig,
  ...
  def continueUrl(isAgent: Boolean, isChange: Boolean): String =
    if (isAgent) controllers.incomeSources.add.routes.AddBusinessAddressController.agentSubmit(None, isChange = isChange).url
    else controllers.incomeSources.add.routes.AddBusinessAddressController.submit(None, isChange = isChange).url
  ...
  def initialiseAddressLookup(isAgent: Boolean, isChange: Boolean)(implicit hc: HeaderCarrier, request: RequestHeader): Future[PostAddressLookupResponse] = {
    ...
    Logger("application").info(s"[AddressLookupConnector] - URL: $addressLookupInitializeUrl - $payload")
