    ...
    http.PUT[UpdateIncomeSourceRequestModel, HttpResponse](
      ...
      response.status match {
        case OK => response.json.validate[UpdateIncomeSourceResponseModel].fold(
          invalid => {
            Logger("application").error(s"[IncomeTaxViewChangeConnector][updateCessationDate] - " +
              s"Json validation error parsing repayment response, error $invalid")
        ...
        case status =>
          if (status >= INTERNAL_SERVER_ERROR) {
            ...
            Logger("application").warn(s"[IncomeTaxViewChangeConnector][updateCessationDate] - " +
              s"Response status: ${response.status}, body: ${response.body}")
          }
