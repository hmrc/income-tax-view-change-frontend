...
class ForeignPropertyAddedObligationsController @Inject()(val foreignPropertyObligationsView: ForeignPropertyAddedObligations,
        ...
        case Some((_, startDate)) =>
          ...
          nextUpdatesService.getObligationsViewModel(incomeSourceId, showPreviousTaxYears) map { viewModel =>
            if (isAgent) Ok(foreignPropertyObligationsView(viewModel,
              controllers.incomeSources.add.routes.ForeignPropertyAddedObligationsController.submitAgent(), agentBackUrl, isAgent = true))
            else Ok(foreignPropertyObligationsView(viewModel,
              controllers.incomeSources.add.routes.ForeignPropertyAddedObligationsController.submit(), backUrl, isAgent = false))
          }
        case _ =>
