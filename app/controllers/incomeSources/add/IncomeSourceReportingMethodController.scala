...
class IncomeSourceReportingMethodController @Inject()(val authorisedFunctions: FrontendAuthorisedFunctions,
  ...
  def handleRequest(isAgent: Boolean, incomeSourceType: IncomeSourceType)(implicit user: MtdItUser[_]): Future[Result] = {
    withSessionData(IncomeSources(Add, incomeSourceType), journeyState = AfterSubmissionPage) { sessionData =>
  ...
  Future[Result] = {
    withSessionData(IncomeSources(Add, incomeSourceType), AfterSubmissionPage) { sessionData =>
      sessionData.addIncomeSourceData.flatMap(_.incomeSourceId) match {
