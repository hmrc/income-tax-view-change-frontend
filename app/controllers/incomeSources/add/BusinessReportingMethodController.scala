...
class BusinessReportingMethodController @Inject()(val authenticate: AuthenticationPredicate,
  ...
  private def handleFormErrors(formWithErrors: Form[AddBusinessReportingMethodForm], incomeSource: Future[Either[Throwable, Option[String]]], isAgent: Boolean)
    ...
    val redirectErrorUrl: Call = if (isAgent) routes.IncomeSourceReportingMethodNotSavedController.showAgent(incomeSourceType = SelfEmployment.key) else
      routes.IncomeSourceReportingMethodNotSavedController.show(incomeSourceType = SelfEmployment.key)
  ...
  private def handleFormData(form: AddBusinessReportingMethodForm, id: String, isAgent: Boolean)
                            (implicit user: MtdItUser[_], ec: ExecutionContext): Future[Result] = {
    val redirectUrl: Call = if (isAgent) routes.BusinessAddedObligationsController.showAgent() else routes.BusinessAddedObligationsController.show()
  ...
  private def updateReportingMethod(isAgent: Boolean, id: String, newReportingMethods: Seq[TaxYearSpecific])
    ...
    val redirectUrl: Call = if (isAgent) routes.BusinessAddedObligationsController.showAgent() else
      routes.BusinessAddedObligationsController.show()
    val redirectErrorUrl: Call = if (isAgent) routes.IncomeSourceReportingMethodNotSavedController.showAgent(incomeSourceType = SelfEmployment.key) else
      routes.IncomeSourceReportingMethodNotSavedController.show(incomeSourceType = SelfEmployment.key)
