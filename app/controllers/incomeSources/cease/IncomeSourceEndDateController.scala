...
@Singleton
class IncomeSourceEndDateController @Inject() (
  ...
  def showChangeAgent(id: Option[String], incomeSourceType: IncomeSourceType): Action[AnyContent] =
    authActions.asMTDAgentWithConfirmedClient.async { implicit mtdItUser =>
      val incomeSourceIdHashMaybe: Option[IncomeSourceIdHash] = id.flatMap(x => mkFromQueryString(x).toOption)
      handleRequest(
        isAgent = true,
  ...
  def handleRequest(
    ...
    withSessionData(IncomeSourceJourneyType(Cease, incomeSourceType), journeyState = InitialPage) { sessionData =>
      val hashCompareResult: Option[Either[Throwable, IncomeSourceId]] =
        id.map(x => user.incomeSources.compareHashToQueryString(x))
          ...
          (incomeSourceType, id) match {
            ...
            case _ =>
              ...
              val newForm = dateStartedOpt match {
                case Some(date) =>
                  form(incomeSourceType, incomeSourceIdMaybe.map(_.value), isEnabled(IncomeSourcesNewJourney))
                    .fill(date)
                case None =>
                  form(incomeSourceType, incomeSourceIdMaybe.map(_.value), isEnabled(IncomeSourcesNewJourney))
              }
  ...
  def submitChange(id: Option[String], incomeSourceType: IncomeSourceType): Action[AnyContent] =
    authActions.asMTDIndividual.async { implicit user =>
      val incomeSourceIdHashMaybe: Option[IncomeSourceIdHash] = id.flatMap(x => mkFromQueryString(x).toOption)
      handleSubmitRequest(
        isAgent = false,
  ...
  def submitChangeAgent(id: Option[String], incomeSourceType: IncomeSourceType): Action[AnyContent] =
    authActions.asMTDAgentWithConfirmedClient.async { implicit mtdItUser =>
      val incomeSourceIdHashMaybe: Option[IncomeSourceIdHash] = id.flatMap(x => mkFromQueryString(x).toOption)
      handleSubmitRequest(
        isAgent = true,
  ...
  private def handleValidatedInput(
      validatedInput:      LocalDate,
      incomeSourceType:    IncomeSourceType,
    ...
    (incomeSourceType, incomeSourceIdMaybe) match {
      case (SelfEmployment, Some(incomeSourceId)) =>
        ...
        sessionService.setMultipleMongoData(mongoSetValues, IncomeSourceJourneyType(Cease, incomeSourceType)).flatMap {
          ...
          case Left(_) =>
            Future.failed(new Error(s"Failed to set data in session storage. incomeSourceType: $incomeSourceType."))
        }
      case _ =>
        ...
        sessionService
          .setMongoKey(
            key = CeaseIncomeSourceData.dateCeasedField,
