...
class FeedbackController @Inject() (
  ...
  def show: Action[AnyContent] =
    authActions.asAuthorisedUser.async { implicit request =>
    ...
    authActions.asAuthorisedUser.async { implicit request =>
      FeedbackForm.form
        ...
        .fold(
          hasErrors =>
            Future.successful(
              BadRequest(feedbackView(feedbackForm = hasErrors, postAction = routes.FeedbackController.submit))
            ),
          formData =>
    ...
    authActions.asAuthorisedUser.async { implicit request =>
      FeedbackForm.form
        ...
        .fold(
          hasErrors =>
            Future.successful(
              BadRequest(feedbackView(feedbackForm = hasErrors, postAction = routes.FeedbackController.submitAgent))
            ),
          formData =>
            feedbackConnector.submit(formData).flatMap {
              case Right(_) =>
  ...
  def thankYou: Action[AnyContent] =
    authActions.asAuthorisedUser { implicit request =>
  ...
  def thankYouAgent: Action[AnyContent] =
    authActions.asAuthorisedUser.async { implicit request =>
