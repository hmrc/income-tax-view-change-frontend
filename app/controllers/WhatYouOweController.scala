...
class WhatYouOweController @Inject() (
  ...
  def handleRequest(
    ...
    for {
      whatYouOweChargesList <-
        whatYouOweService.getWhatYouOweChargesList(isEnabled(ReviewAndReconcilePoa), isEnabled(FilterCodedOutPoas))
      ...
      val hasAccruingInterestReviewAndReconcileCharges: Boolean =
        whatYouOweChargesList.chargesList.exists(_.isNotPaidAndNotOverduePoaReconciliationDebit()(dateService))
      Ok(
        whatYouOwe(
        ...
        )(user, user, messages, dateService)
  ...
  private def claimToAdjustViewModel(
      ...
      user:        MtdItUser[_]
    ): Future[WYOClaimToAdjustViewModel] = {
