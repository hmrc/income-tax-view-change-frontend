...
import repositories.AddIncomeSourceSessionDataRepository
...
class AddBusinessNameController @Inject()(authenticate: AuthenticationPredicate,
                                          ...
                                          sessionDataRepository: AddIncomeSourceSessionDataRepository)
  ...
  def handleRequest(isAgent: Boolean, backUrl: String)(implicit user: MtdItUser[_], ec: ExecutionContext): Future[Result] = {
    if (isDisabled(IncomeSources)) {
      ...
      if (isAgent) {
        Future.successful(Ok(addBusinessView(BusinessNameForm.form, isAgent, routes.AddBusinessNameController.submitAgent(), backUrl)))
      } else {
        Future.successful(Ok(addBusinessView(BusinessNameForm.form, isAgent, routes.AddBusinessNameController.submit(), backUrl)))
      }
    }
  }
  ...
  def submit: Action[AnyContent] = (checkSessionTimeout andThen authenticate andThen retrieveNino
    ...
    implicit request =>
      BusinessNameForm.form.bindFromRequest().fold(
        ...
        formData => {
          val addIncomeSourceData = AddIncomeSourceSessionData(hc.sessionId.get.value, "add-is-se",
            businessName = Some(formData.name))
          sessionDataRepository.set(addIncomeSourceData) flatMap (_ => {
            Future.successful(Redirect(controllers.incomeSources.add.routes.AddBusinessStartDateController.show()))
          })
        }
      )
  }
