    ...
    implicit user =>
      withOptOutFS {
        for {
          ...
          val cancelUrl =
            if (isAgent) controllers.optOut.oldJourney.routes.OptOutCancelledController.showAgent().url
            else controllers.optOut.oldJourney.routes.OptOutCancelledController.show().url
