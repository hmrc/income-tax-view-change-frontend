...
@Singleton
class Authorise @Inject()(authenticate: AuthenticationPredicate,
  ...
  def action(isAgent: Boolean)(authenticatedCodeBlock: MtdItUser[_] => Future[Result]): Action[AnyContent] = {
    if (isAgent)
      Authenticated.async {
        implicit request =>
          implicit user =>
