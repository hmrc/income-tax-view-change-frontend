...
class TaxYearSummaryController @Inject() (
  ...
  private def view(
    ...
    liabilityCalc match {
      case liabilityCalc: LiabilityCalculationResponse =>
        ...
        val calculationSummary = Some(
          CalculationSummary(
            formatErrorMessages(liabilityCalc, messagesApi, isAgent)(Lang("GB"), messagesApi.preferred(lang))
          )
        )
        ...
        auditingService.extendedAudit(
          TaxYearSummaryResponseAuditModel(mtdItUser, messagesApi, taxYearSummaryViewModel, liabilityCalc.messages)
        )
  ...
  private def withTaxYearFinancials(
      taxYear: Int,
      isAgent: Boolean
    )(
      f: List[TaxYearSummaryChargeItem] => Future[Result]
    )(
        ...
        val chargeItemsNoCodingOut: List[TaxYearSummaryChargeItem] = {
          chargeItemsNoPayments
            ...
            .flatMap(dd =>
              getChargeItem(dd)
                .map(ci =>
                  TaxYearSummaryChargeItem.fromChargeItem(ci, financialDetails.findDueDateByDocumentDetails(dd))
                )
            )
        ...
        val chargeItemsLpi: List[TaxYearSummaryChargeItem] = {
          chargeItemsNoPayments
            ...
            .flatMap(dd =>
              getChargeItem(dd)
                .map(ci =>
                  TaxYearSummaryChargeItem.fromChargeItem(ci, dd.interestEndDate, isLatePaymentInterest = true)
                )
            )
        }
  ...
  private def withObligationsModel(
      taxYear: Int,
      isAgent: Boolean
    )(
      f: ObligationsModel => Future[Result]
    )(
      implicit user: MtdItUser[_]
    ): Future[Result] = {
  ...
  private def handleRequest(
    ...
    withTaxYearFinancials(taxYear, isAgent) { charges =>
      withObligationsModel(taxYear, isAgent) { obligationsModel =>
        ...
        for {
          ...
          view(
            liabilityCalcResponse,
            charges,
            taxYear,
            obligationsModel,
            viewModel,
            backUrl =
              if (isAgent) getAgentBackURL(user.headers.get(REFERER))
              else getBackURL(user.headers.get(REFERER), origin),
            origin = origin,
            isAgent = isAgent
          )
    ...
    case ex: Throwable =>
      ...
      Logger("application").error(
        s"${if (isAgent) "Agent" else "Individual"} - There was an error, status: - ${ex.getMessage} - ${ex.getCause} - "
      )
  ...
  private def claimToAdjustViewModel(
      nino:    Nino,
      taxYear: Int
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[TYSClaimToAdjustViewModel] = {
    if (isEnabled(AdjustPaymentsOnAccount)) {
      claimToAdjustService.getPoaTaxYearForEntryPoint(nino).flatMap {
        case Right(value) =>
          value match {
            case Some(value) if value.endYear == taxYear =>
              Future.successful(TYSClaimToAdjustViewModel(isEnabled(AdjustPaymentsOnAccount), Option(value)))
            case _ => Future.successful(TYSClaimToAdjustViewModel(isEnabled(AdjustPaymentsOnAccount), None))
          }
        case Left(ex: Throwable) =>
          Logger("application").error(
            s"There was an error when getting the POA Entry point" +
