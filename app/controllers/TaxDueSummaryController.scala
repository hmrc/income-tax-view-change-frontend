  ...
  def handleRequest(origin: Option[String] = None,
    ...
    for {
      ...
      case (calcErrorResponse: LiabilityCalculationError, _) if calcErrorResponse.status == NO_CONTENT =>
        Logger("application")
          .info("No calculation data returned from downstream. Not Found.")
        itvcErrorHandler.showInternalServerError()
      case (_: LiabilityCalculationError, _) =>
        Logger("application")
          .error(s"[$taxYear] No new calc deductions data error found. Downstream error")
        itvcErrorHandler.showInternalServerError()
      case (_, _: ObligationsErrorModel) =>
        Logger("application")
          .error(s"[$taxYear] Failed to retrieve obligations. Downstream error")
        itvcErrorHandler.showInternalServerError()
