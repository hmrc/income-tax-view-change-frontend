...
@Singleton
class EnterClientsUTRController @Inject() (
    enterClientsUTR:         EnterClientsUTR,
  ...
  def showWithUtr(utr: String): Action[AnyContent] =
    authActions.asAgent().async { implicit user =>
          ...
          validUTR => {
            clientDetailsService.checkClientDetails(
              ...
              case Right(clientDetails) =>
                checkAgentAuthorisedAndGetRole(clientDetails.mtdItId)
                  .flatMap { userRole =>
                    val sessionCookies: Seq[(String, String)] =
                      SessionCookieData(clientDetails, validUTR, userRole == MTDSupportingAgent).toSessionCookieSeq
                    sendAudit(true, user, validUTR, clientDetails.nino, clientDetails.mtdItId)
                    Future
                      .successful(Redirect(routes.ConfirmClientUTRController.show).addingToSession(sessionCookies: _*))
                  }
                  .recover {
                    case ex =>
                      Logger("application")
  ...
  private def sendAudit[A](
      isSuccessful: Boolean,
      user:         AuthorisedUserRequest[A],
      validUTR:     String,
      nino:         String,
      mtdItId:      String
    )(
