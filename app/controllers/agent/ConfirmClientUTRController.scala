...
@Singleton
class ConfirmClientUTRController @Inject() (
    ...
    authActions.asMTDAgentWithUnconfirmedClient.async { implicit user =>
      ...
      handleSessionCookies(
        SessionCookieData(
          user.mtditid,
          user.nino,
          user.saUtr.getOrElse(""),
          names.headOption,
          names.lastOption,
          user.isSupportingAgent
        )
      ) { _ =>
  ...
  def handleSessionCookies(
      sessionCookieData: SessionCookieData
    )(
      codeBlock: Seq[(String, String)] => Future[Result]
    )(
    ...
    if (getSessionDataStorageFS) {
      sessionDataService.postSessionData(sessionCookieData.toSessionDataModel).flatMap {
        case Left(value: SessionDataPostFailure) =>
          Logger("application").error(
            s"[Agent] Posting user session data was unsuccessful. Status: ${value.status}, error message: ${value.errorMessage}"
          )
      ...
      Logger("application").info(
        s"[Agent] GetUserSessionApi feature switch was off so session data has not been posted to the session-data service"
      )
      codeBlock(sessionCookieData.toSessionCookieSeq)
    }
  }
