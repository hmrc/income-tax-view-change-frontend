...
class BeforeYouStartController @Inject()(authActions: AuthActions,
  ...
  private def startButtonUrl(isAgent: Boolean, availableYears: Seq[TaxYear]) = {
    availableYears match {
      case Seq(_) =>
        controllers.optIn.oldJourney.routes.SingleTaxYearOptInWarningController.show(isAgent)
      case _ => controllers.optIn.oldJourney.routes.ChooseYearController.show(isAgent)
    }
  }
