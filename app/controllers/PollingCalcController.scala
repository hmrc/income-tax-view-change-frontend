...
@Singleton
class PollingCalcController @Inject()(authenticate: AuthenticationPredicate,
  ...
  def sessionCheck: Action[AnyContent] = action.async {
    implicit user =>
    implicit val hc: HeaderCarrier = HeaderCarrierConverter.fromHeadersAndSession(user.headers, Some(user.session))

    (user.session.get("calculationId"), user.session.get("nino")) match {
        case (Some(calculationId),Some(nino)) =>
          Logger.warn(s"[PollingCalcController] $calculationId and $nino found in session")
          Future.successful(Ok(""))
        case _ =>
          Logger.warn(s"[PollingCalcController] calculationId and nino not found in session")
