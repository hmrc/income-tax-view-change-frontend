    ...
    withIncomeSourcesFS {
      ...
      form.apply
        ...
        .fold(
          ...
          formData => handleValidForm(formData, isAgent)
        )
    }

  private def handleValidForm(validForm: form, isAgent: Boolean)(implicit mtdItUser: MtdItUser[_]): Future[Result] = {

    val formResponse: Option[String] = validForm.toFormMap(form.response).headOption
    val ukPropertyUrl: String = controllers.manageBusinesses.add.routes.AddIncomeSourceStartDateController
      .show(isAgent, isChange = false, UkProperty)
      .url
    val foreignPropertyUrl: String = controllers.manageBusinesses.add.routes.AddIncomeSourceStartDateController
      .show(isAgent, isChange = false, ForeignProperty)
      .url
