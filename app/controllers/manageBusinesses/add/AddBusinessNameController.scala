...
@Singleton
class AddBusinessNameController @Inject() (
  ...
  def handleRequest(
    ...
    withSessionData(IncomeSourceJourneyType(Add, SelfEmployment), journeyState = InitialPage) { sessionData =>
      ...
      val filledForm: Form[BusinessNameForm] =
        businessNameOpt.fold(BusinessNameForm.form)(name => BusinessNameForm.form.fill(BusinessNameForm(name)))
  ...
  def handleSubmitRequest(
    ...
    withSessionData(IncomeSourceJourneyType(Add, SelfEmployment), InitialPage) { sessionData =>
      ...
      BusinessNameForm
        .checkBusinessNameWithTradeName(BusinessNameForm.form.bindFromRequest(), businessTradeOpt)
        .fold(
          formWithErrors =>
            Future.successful {
              BadRequest(
