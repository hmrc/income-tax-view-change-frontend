...
class IncomeSourceAddedBackErrorController @Inject() (
  ...
  def handleRequest(isAgent: Boolean, incomeSourceType: IncomeSourceType)(implicit user: MtdItUser[_]): Future[Result] =
    withSessionData(IncomeSourceJourneyType(Add, incomeSourceType), journeyState = CannotGoBackPage) { data =>
      val cannotGoBackRedirectUrl =
        if (isAgent)
          controllers.manageBusinesses.add.routes.ReportingMethodSetBackErrorController.showAgent(incomeSourceType)
      ...
      if (data.addIncomeSourceData.exists(addData => addData.journeyIsComplete.contains(true))) {
        ...
        val postAction =
          if (isAgent)
            controllers.manageBusinesses.add.routes.IncomeSourceAddedBackErrorController.submitAgent(incomeSourceType)
  ...
  private def handleSubmit(
    ...
    withSessionData(IncomeSourceJourneyType(Add, incomeSourceType), CannotGoBackPage) {
      _.addIncomeSourceData.map(_.incomeSourceId) match {
        ...
        case None =>
          Logger("application").error("Error: Unable to find id in session")
          Future.successful {
            errorHandler.showInternalServerError()
          }
      }
