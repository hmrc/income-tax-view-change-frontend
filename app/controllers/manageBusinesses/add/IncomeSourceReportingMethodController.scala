...
class IncomeSourceReportingMethodController @Inject() (
  ...
  lazy val backUrl: (Boolean, IncomeSourceType) => String = (isAgent: Boolean, incomeSourceType: IncomeSourceType) =>
    (isAgent, incomeSourceType.equals(SelfEmployment)) match {
      ...
      case (_, _) =>
        routes.AddIncomeSourceStartDateCheckController.show(isAgent, isChange = false, incomeSourceType).url
    }

  lazy val errorRedirectUrl: (Boolean, IncomeSourceType) => String =
  ...
  lazy val redirectUrl: (Boolean, IncomeSourceType) => String =
    (isAgent: Boolean, incomeSourceType: IncomeSourceType) =>
  ...
  private def handleIncomeSourceIdRetrievalSuccess(
      incomeSourceType: IncomeSourceType,
      id:               String,
  ...
  private def updateIncomeSourceAsAdded(
      sessionData: UIJourneySessionData
    )(
  ...
  private def updateReportingMethod(
      sessionData:             UIJourneySessionData,
      reportingMethodTaxYear1: Option[String],
      reportingMethodTaxYear2: Option[String]
    )(
      implicit hc: HeaderCarrier
    ): Future[Boolean] = {
    val updatedAddIncomeSourceSessionData =
      sessionData.addIncomeSourceData
        .getOrElse(AddIncomeSourceData())
  ...
  private def getLatencyDetails(
      incomeSourceType: IncomeSourceType,
      incomeSourceId:   String
    )(
      implicit user: MtdItUser[_]
    ): Option[LatencyDetails] = {
  ...
  private def getViewModel(
      ...
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Option[IncomeSourceReportingMethodViewModel]] = {
      ...
      case Some(LatencyDetails(_, taxYear1, taxYear1LatencyIndicator, taxYear2, taxYear2LatencyIndicator)) =>
        calculationListService.determineTaxYearCrystallised(taxYear1.toInt).flatMap {
          case true =>
            Future.successful {
              Some(
                IncomeSourceReportingMethodViewModel(
                  latencyYear1 = None,
          ...
          case false =>
            Future.successful {
              Some(
                IncomeSourceReportingMethodViewModel(
                  latencyYear1 = Some(LatencyYear(taxYear1, taxYear1LatencyIndicator)),
                  latencyYear2 = Some(LatencyYear(taxYear2, taxYear2LatencyIndicator))
                )
              )
            }
  ...
  private def handleSubmit(
    ...
    withSessionData(IncomeSourceJourneyType(Add, incomeSourceType), AfterSubmissionPage) { sessionData =>
      sessionData.addIncomeSourceData.flatMap(_.incomeSourceId) match {
        case Some(id) =>
          IncomeSourceReportingMethodForm.form
            .bindFromRequest()
            .fold(
  ...
  private def handleInvalidForm(
      form:             Form[IncomeSourceReportingMethodForm],
      incomeSourceType: IncomeSourceType,
      id:               IncomeSourceId,
      isAgent:          Boolean
    )(
  ...
  private def handleValidForm(
    ...
    updateReportingMethod(sessionData, form.newTaxYear1ReportingMethod, form.newTaxYear2ReportingMethod).flatMap {
      ...
      case false =>
        Logger("application").error(
          s"${if (isAgent) "[Agent]"}" + s"Error saving reporting method data < newReportingMethodTaxYear1: ${form.newTaxYear1ReportingMethod}, newReportingMethodTaxYear2: ${form.newTaxYear2ReportingMethod} >"
        )
        Future.successful {
          errorHandler(isAgent).showInternalServerError()
        }
  ...
  private def updateReportingMethod(
      ...
      id:                  IncomeSourceId,
      incomeSourceType:    IncomeSourceType,
      newReportingMethods: Seq[TaxYearSpecific]
    )(
      implicit user: MtdItUser[_]
    ): Future[Result] = {
    val results = newReportingMethods.foldLeft(Future.successful(Seq.empty[UpdateIncomeSourceResponse])) {
      (prevFutRes, taxYearSpec) =>
  ...
  private def sendAuditEvent(
      ...
      newReportingMethod: TaxYearSpecific,
      incomeSourceType:   IncomeSourceType,
      id:                 String
    )(
      implicit user: MtdItUser[_]
    ...
    val businessName: String =
      user.incomeSources.getIncomeSourceBusinessName(incomeSourceType, Some(id)).getOrElse("Unknown")
  ...
  private def handleUpdateResults(
      isAgent:          Boolean,
      incomeSourceType: IncomeSourceType,
      id:               IncomeSourceId,
      updateResults:    Future[Seq[UpdateIncomeSourceResponse]]
    ): Future[Result] = {
    ...
    updateResults.map { results =>
      ...
      if (successCount == results.length) {
        Logger("application").info(
          prefix + s"Successfully updated all new selected reporting methods for $incomeSourceType"
        )
        Redirect(redirectUrl(isAgent, incomeSourceType))
      } else if (errorCount == results.length) {
        Logger("application").info(
          prefix + s"Unable to update all new selected reporting methods for $incomeSourceType"
        )
        Redirect(errorRedirectUrl(isAgent, incomeSourceType))
      } else {
        Logger("application").info(
          prefix + s"Successfully updated one new selected reporting method for $incomeSourceType, the other one failed"
        )
        Redirect(errorRedirectUrl(isAgent, incomeSourceType))
      }
    }
