...
class ManageObligationsController @Inject()(val authActions: AuthActions,
  ...
  def submit(isAgent: Boolean): Action[AnyContent] =
    authActions.asMTDIndividualOrAgentWithClient(isAgent).async { _ =>
      if (isAgent) Future(Redirect(controllers.manageBusinesses.routes.ManageYourBusinessesController.showAgent()))
      else Future(Redirect(controllers.manageBusinesses.routes.ManageYourBusinessesController.show()))
    }
}
