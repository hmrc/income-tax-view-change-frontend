...
class CeaseCheckIncomeSourceDetailsController @Inject() (
  ...
  private lazy val backUrl: Boolean => String = (isAgent: Boolean) =>
    if (isAgent) routes.CeaseIncomeSourceController.showAgent().url
    ...
    withSessionData(IncomeSourceJourneyType(Cease, incomeSourceType), BeforeSubmissionPage) { sessionData =>
      ...
      getViewModel(incomeSourceType, endDateOpt, incomeSourceIdOpt) match {
        ...
        case None =>
          Future.successful {
            Redirect(
              controllers.manageBusinesses.cease.routes.IncomeSourceNotCeasedController.show(isAgent, incomeSourceType)
            )
          }
      }
    }.recover {
  ...
  def getViewModel(
      ...
      incomeSourceIdOpt: Option[String]
    )(
      implicit user: MtdItUser[_]
    ): Option[CheckCeaseIncomeSourceDetailsViewModel] = {
    (incomeSourceIdOpt, endDateOpt, incomeSourceType) match {
      case (Some(id), Some(endDate), SelfEmployment) =>
        incomeSourceDetailsService.getCheckCeaseSelfEmploymentDetailsViewModel(
          user.incomeSources,
          IncomeSourceId(id),
          endDate
        ) match {
      ...
      case (None, Some(endDate), _) =>
        incomeSourceDetailsService.getCheckCeasePropertyIncomeSourceDetailsViewModel(
          user.incomeSources,
          endDate,
          incomeSourceType
        ) match {
  ...
  private def ceaseSelfEmployment(
      incomeSourceIdOpt: Option[String],
      endDateOpt:        Option[LocalDate],
      isAgent:           Boolean
    )(
    ...
    (incomeSourceIdOpt, endDateOpt) match {
      case (Some(incomeSourceId), Some(endDate)) =>
        updateCessationDate(endDate, SelfEmployment, IncomeSourceId(incomeSourceId), isAgent)
      case _ =>
        Future.successful {
  ...
  def submit(incomeSourceType: IncomeSourceType): Action[AnyContent] =
    authActions.asMTDIndividual.async { implicit request =>
      handleSubmitRequest(isAgent = false, incomeSourceType = incomeSourceType)(implicitly, itvcErrorHandler)
    }

  def submitAgent(incomeSourceType: IncomeSourceType): Action[AnyContent] =
    authActions.asMTDAgentWithConfirmedClient.async { implicit mtdItUser =>
      handleSubmitRequest(isAgent = true, incomeSourceType = incomeSourceType)(implicitly, itvcErrorHandlerAgent)
    }
}
