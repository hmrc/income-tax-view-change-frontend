...
class HomeController @Inject()(val homeView: views.html.HomeView,
  ...
  private def handleYourTasks(origin: Option[String] = None, isAgent: Boolean)
                             (implicit user: MtdItUser[_]): Future[Result] = {
    if(isAgent){
      Future.successful(Redirect(routes.NewHomeYourTasksController.showAgent()))
    }else {
     Future.successful(Redirect(routes.NewHomeYourTasksController.show()))
    }
  }
    ...
    implicit user => {
      ...
      yield {
        Ok(newHomeOverviewView(origin, isAgent, user.isSupportingAgent, dateService.getCurrentTaxYear,
          ...
          appConfig.itvcRebrand, mandatedOrVoluntary))
      }
    }
  }
