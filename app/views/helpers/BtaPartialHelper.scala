          ...
          estimate.calculation match {
            case calc: LastTaxCalculation =>
              ...
              s"""<p id="current-estimate-$taxYear">${estimatesMessage(taxYear, calc.calcAmount)}</p>
                 |<a data-journey-click="itvcPartial:clickedLink:View Estimated Tax Liability $taxYear" id="estimates-link-$taxYear" href="${config.itvcFrontendEnvironment + controllers.routes.CalculationController.showCalculationForYear(taxYear).url}">
