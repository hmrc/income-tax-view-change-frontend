...
class CompleteStepsController @Inject()(view: CompleteStepsView,
                                       ...
                                       (mcc: MessagesControllerComponents,
                                        ...
                                        implicit val ec: ExecutionContext) extends FrontendController(mcc) with I18nSupport with TriggeredMigrationUtils with Logging {
        ...
        for {
          yearOfMigration <- yearOfMigrationService.getYearOfMigration(user.nino).flatMap {
            _.yearOfMigrationEndYear match {
              ...
              case None =>
                logger.error("[CompleteStepsController][Show] Year of migration not found for triggered migrated user")
                Future.failed(new RuntimeException("[CompleteStepsController][Show] Year of migration not found"))
            }
          }
        } yield {
          Ok(
