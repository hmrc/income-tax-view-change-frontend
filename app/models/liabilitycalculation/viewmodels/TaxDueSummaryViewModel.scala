      ...
      case Some(calc) =>
        TaxDueSummaryViewModel(
          ...
          class2VoluntaryContributions =
            calcResponse.inputs.personalInformation.class2VoluntaryContributions.getOrElse(false),
          ...
          totalResidentialFinanceCostsRelief =
            calc.reliefs.flatMap(r => r.residentialFinanceCosts.map(rfc => rfc.totalResidentialFinanceCostsRelief)),
          totalForeignTaxCreditRelief =
            calc.reliefs.flatMap(r => r.foreignTaxCreditRelief.map(rfc => rfc.totalForeignTaxCreditRelief)),
          ...
          giftAidTaxReductionWhereBasicRateDiffers =
            calc.reliefs.flatMap(r => r.giftAidTaxReductionWhereBasicRateDiffers.flatMap(gatr => gatr.amount)),
          totalTaxableIncome = calc.taxCalculation.map(tc => BigDecimal(tc.incomeTax.totalTaxableIncome)),
          payPensionsProfitBands =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.payPensionsProfit.map(ppp => ppp.taxBands.getOrElse(Seq()))),
          savingsAndGainsBands =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.savingsAndGains.map(ppp => ppp.taxBands.getOrElse(Seq()))),
          lumpSumsBands =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.lumpSums.map(ppp => ppp.taxBands.getOrElse(Seq()))),
          dividendsBands =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.dividends.map(ppp => ppp.taxBands.getOrElse(Seq()))),
          gainsOnLifePoliciesBands = calc.taxCalculation.flatMap(tc =>
            tc.incomeTax.gainsOnLifePolicies.map(ppp => ppp.taxBands.getOrElse(Seq()))
          ),
          totalNotionalTax = calc.taxCalculation.flatMap(tc => tc.incomeTax.totalNotionalTax),
          incomeTaxDueAfterTaxReductions =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.incomeTaxDueAfterTaxReductions),
          ...
          nic4Bands = calc.taxCalculation.flatMap(tc =>
            tc.nics.flatMap(nics => nics.class4Nics.map(class4nics => class4nics.nic4Bands))
          ),
          class2NicsAmount = calc.taxCalculation.flatMap(tc =>
            tc.nics.flatMap(nics => nics.class2Nics.flatMap(class2 => class2.amount))
          ),
          ...
          giftAidTaxChargeWhereBasicRateDiffers =
            calc.taxCalculation.flatMap(tc => tc.incomeTax.giftAidTaxChargeWhereBasicRateDiffers),
...
object TransitionProfitRow {
  def apply(
      ...
      totalTaxableProfit: Option[BigDecimal]
    ): Option[TransitionProfitRow] = {
