        ...
        Some(
          IncomeBreakdownViewModel(
            totalPayeEmploymentAndLumpSumIncome =
              c.employmentAndPensionsIncome.flatMap(eapi => eapi.totalPayeEmploymentAndLumpSumIncome),
            ...
            totalSelfEmploymentProfit = c.incomeSummaryTotals.flatMap(ist => ist.totalSelfEmploymentProfit) map { i =>
              i: BigDecimal
            },
            totalPropertyProfit = c.incomeSummaryTotals.flatMap(ist => ist.totalPropertyProfit) map { i =>
              i: BigDecimal
            },
            totalFHLPropertyProfit = c.incomeSummaryTotals.flatMap(ist => ist.totalFHLPropertyProfit) map { i =>
              i: BigDecimal
            },
            totalForeignPropertyProfit = c.incomeSummaryTotals.flatMap(ist => ist.totalForeignPropertyProfit) map { i =>
              i: BigDecimal
            },
            totalEeaFhlProfit = c.incomeSummaryTotals.flatMap(ist => ist.totalEeaFhlProfit) map { i => i: BigDecimal },
            chargeableForeignDividends = c.dividendsIncome.flatMap(di => di.chargeableForeignDividends) map { i =>
              i: BigDecimal
            },
            chargeableForeignSavingsAndGains =
              c.savingsAndGainsIncome.flatMap(sagi => sagi.chargeableForeignSavingsAndGains) map { i => i: BigDecimal },
            chargeableOverseasPensionsStateBenefitsRoyalties =
              c.foreignIncome.flatMap(fi => fi.chargeableOverseasPensionsStateBenefitsRoyalties),
            chargeableAllOtherIncomeReceivedWhilstAbroad =
              c.foreignIncome.flatMap(fi => fi.chargeableAllOtherIncomeReceivedWhilstAbroad),
            totalOverseasIncomeAndGains =
              c.foreignIncome.flatMap(fi => fi.overseasIncomeAndGains.flatMap(oiag => Some(oiag.gainAmount))),
            ...
            dividendsTaxableIncome = c.taxCalculation.flatMap(tc =>
              Some(tc.incomeTax).flatMap(it => it.dividends.flatMap(d => Some(d.taxableIncome)))
            ),
            otherIncome = c.otherIncome.map(oi => oi.totalOtherIncome),
            totalOccupationalPensionIncome =
              c.employmentAndPensionsIncome.flatMap(eapi => eapi.totalOccupationalPensionIncome),
            totalStateBenefitsIncome =
              c.stateBenefitsIncome.flatMap(sbi => sbi.totalStateBenefitsIncomeExcStatePensionLumpSum),
            totalShareSchemesIncome = c.shareSchemesIncome.flatMap(ssi => Some(ssi.totalIncome)),
            totalIncomeReceived = c.taxCalculation.flatMap(tc =>
              Some(tc.incomeTax).flatMap(it => Some(it.totalIncomeReceivedFromAllSources))
            )
          )
        )
      case _ => None
