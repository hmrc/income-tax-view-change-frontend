...
case class DocumentDetail(
  ...
  def paymentOrChargeCredit: Option[BigDecimal] =
  ...
  def originalAmountIsNotZeroOrNegative: Boolean =
    originalAmount match {
      case amount if amount <= 0 => false
      case _                     => true
    }

  def isLatePaymentInterest: Boolean =
    latePaymentInterestAmount match {
  ...
  def getChargeTypeKey: String =
    documentDescription match {
      ...
      case Some(TRMNewCharge.key) | Some(TRMAmendCharge.key) =>
        (isClass2Nic, isPayeSelfAssessment, isCancelledPayeSelfAssessment) match {
...
case class DocumentDetailWithDueDate(
  ...
  def isOnlyInterest: Boolean = {
    (isOverdue && isLatePaymentInterest) || (documentDetail.interestRemainingToPay > 0 && documentDetail.isPaid)
  }
}
