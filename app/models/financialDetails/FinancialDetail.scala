...
case class FinancialDetail(
  ...
  def payments(implicit dateService: DateServiceInterface): Seq[Payment] =
    items match {
      case Some(subItems) =>
        subItems
          .map { subItem =>
            Payment(
              ...
              dueDate = subItem.clearingDate,
              documentDate = dateService.getCurrentDate,
...
object FinancialDetail {
  ...
  object Types {
    ...
    val supportedBCDCTypeParts = Set(
      CTypePartITSA,
      CTypePartNIC4,
      CTypePartNIC2,
