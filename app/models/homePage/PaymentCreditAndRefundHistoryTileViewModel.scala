...
case class PaymentCreditAndRefundHistoryTileViewModel(
    unpaidCharges:                List[FinancialDetailsResponseModel],
    creditsRefundsRepayEnabled:   Boolean,
    paymentHistoryRefundsEnabled: Boolean,
  ...
  def title()(implicit messages: Messages): String =
    if (paymentHistoryRefundsEnabled) {
      messages("home.paymentHistoryRefund.heading")
    } else {
      messages("home.paymentHistory.heading")
    }

  def paymentCreditRefundHistoryMessageKey(): String =
    (creditsRefundsRepayEnabled, paymentHistoryRefundsEnabled) match {
