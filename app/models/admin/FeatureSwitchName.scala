...
object FeatureSwitchName {
  ...
  implicit val reads: Reads[FeatureSwitchName] = {
    ...
    case name if name == JsString(UserSessionApi.name) =>
      JsSuccess(UserSessionApi)
  ...
  val allFeatureSwitches: immutable.Set[FeatureSwitchName] =
    Set(ITSASubmissionIntegration, ChargeHistory, PaymentAllocation, CodingOut, NavBarFs,
      ...
      PaymentHistoryRefunds, CalendarQuarterTypes, IncomeSourcesNewJourney, IncomeSources, OptOut, AdjustPaymentsOnAccount, UserSessionApi)
...
case object UserSessionApi extends FeatureSwitchName {
  override val name: String = s"user-session-API"
  override val toString: String = "User Session API"
}
