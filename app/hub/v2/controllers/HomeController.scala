...
import hub.v2.views.html.newHomePage.*
...
@Singleton
class HomeController @Inject()(val newHomeRecentActivityView: NewHomeRecentActivityView,
                               val newHomeOverviewView: NewHomeOverviewView,
                               val newHomeHelpView: NewHomeHelpView,
    ...
    implicit user => {
      ...
      yield {
        Ok(newHomeOverviewView(origin, user.isSupportingAgent, dateService.getCurrentTaxYear,
          ...
          isEnabled(BusinessDetailsFrontend), isEnabled(ObligationsFrontend), isEnabled(FinancialsFrontend), isEnabled(ReturnsFrontend)))
      }
    }
  }
