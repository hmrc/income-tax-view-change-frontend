...
@Singleton
class DateUKPropertyStartedForm @Inject()(val dateService: DateService, val dateFormatter: ImplicitDateFormatterImpl) extends Constraints {
...
object DateUKPropertyStartedForm {
  def apply(dateService: DateService, dateFormatter: ImplicitDateFormatterImpl)(implicit user: MtdItUser[_], messages: Messages): Form[DateFormElement] = {
    new DateUKPropertyStartedForm(dateService, dateFormatter).apply
  }
}
