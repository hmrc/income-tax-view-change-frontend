...
object IncomeSourceReportingMethodForm extends CustomConstraints {
    ...
    mapping(
      newTaxYear1ReportingMethod -> optional(text)
        .verifying(
          ...
          taxYearReporting1 => taxYearReporting1.isEmpty || validRadioOptions.contains(taxYearReporting1.get)
        ),
      newTaxYear2ReportingMethod -> optional(text)
        .verifying(
          newTaxYear2ReportingMethod,
          taxYearReporting2 => taxYearReporting2.isDefined && validRadioOptions.contains(taxYearReporting2.get)
        ),
  ...
  def updateErrorMessagesWithValues(
      form: Form[IncomeSourceReportingMethodForm]
    ): Form[IncomeSourceReportingMethodForm] = {
    form.errors.foldLeft[Form[IncomeSourceReportingMethodForm]](form.discardingErrors)(
      (reportingMethodFormValues, formError) => {
        reportingMethodFormValues.data.get(formError.message + "_tax_year") match {
          case Some(year) =>
            val taxYearTo   = year.toInt
            val taxYearFrom = taxYearTo - 1
            reportingMethodFormValues.withError(
              formError.message,
              radioMustBeSelectedMessageKey,
              taxYearFrom.toString,
              taxYearTo.toString
            )
...
case class IncomeSourceReportingMethodForm(
  ...
  def toFormMap: Map[String, Option[String]] =
    Map(
