...
case class IncomeSourceReportingFrequencyForm(reportingFrequencyQuarterly: Option[Boolean], csrfToken: String)
...
object IncomeSourceReportingFrequencyForm {
  ...
  def apply()(implicit messages: Messages): Form[IncomeSourceReportingFrequencyForm] = {
    ...
    Form(
      mapping(
        reportQuarterlyField -> optional(boolean).verifying(
