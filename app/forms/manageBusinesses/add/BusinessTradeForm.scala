  ...
  val form: Form[BusinessTradeForm] = Form(
    mapping(
      businessTrade.trim() -> text
        .verifying(firstError(isNotEmpty andThen isNotTooLong andThen isNotTooShort andThen isValidChars))
    )(BusinessTradeForm.apply)(BusinessTradeForm.unapply)
  )

  def checkBusinessTradeWithBusinessName(
      form:         Form[BusinessTradeForm],
      businessName: Option[String]
    ): Form[BusinessTradeForm] = {
    businessName match {
      case Some(businessName)
          if !form.hasErrors && form.get.trade.toLowerCase.trim.equals(businessName.toLowerCase.trim) =>
