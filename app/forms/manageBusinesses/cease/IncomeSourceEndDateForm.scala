...
class IncomeSourceEndDateForm @Inject() (val dateService: DateService)(implicit val appConfig: FrontendAppConfig)
  ...
  def dateMustNotBeInFuture(incomeSourceType: IncomeSourceType) =
    s"incomeSources.cease.endDate.${incomeSourceType.messagesCamel}.future"

  def dateMustBeAfterBusinessStartDate(incomeSourceType: IncomeSourceType) =
    s"incomeSources.cease.endDate.${incomeSourceType.messagesCamel}.beforeStartDate"

  def dateMustNotBeBefore6April2015(incomeSourceType: IncomeSourceType) =
  ...
  private def dateMustBeCompleteKey(incomeSourceType: IncomeSourceType): String =
  ...
  private def checkRequiredFields(incomeSourceType: IncomeSourceType): Constraint[(String, String, String)] =
    Constraint("constraints.requiredFields") {
