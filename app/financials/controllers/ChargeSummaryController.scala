...
class ChargeSummaryController @Inject()(val authActions: AuthActions,
  ...
  def onError(message: String, isAgent: Boolean, showInternalServerError: Boolean)(implicit request: Request[_]): Result = {
    ...
    if (showInternalServerError) {
      ...
      if (isAgent) Redirect(agentErrorRoutes.AgentNotFoundDocumentIDLookupController.show())
      else Redirect(errorRoutes.NotFoundDocumentIDLookupController.show())
    }
  }
          ...
          chargeHistoryService.chargeHistoryResponse(isInterestCharge, chargeReference, isEnabled(ChargeHistory)).flatMap {
            case Right(chargeHistory) =>
              ...
              val whatYouOweUrl = {
                ...
                else financialsRoutes.WhatYouOweController.show(origin).url
              }
  ...
  private def checkForPoaExtraChargeLink(
    ...
    chargeItem match {
      case Some(value) =>
        ...
        extraChargeId match {
          case Some(validId) =>
            if (isAgent) Some(financialsRoutes.ChargeSummaryController.showAgent(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
            else Some(financialsRoutes.ChargeSummaryController.show(documentDetailWithDueDate.documentDetail.taxYear, validId).url)
          case None => None
        }
      case None => None
    }
