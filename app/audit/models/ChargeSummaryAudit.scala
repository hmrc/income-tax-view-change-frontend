...
case class ChargeSummaryAudit(mtdItUser: MtdItUser[_],
  ...
  private def getChargeTypeFromKey(key: Option[String]): String = key match {
    ...
    case Some("chargeSummary.chargeHistory.amend.paymentOnAccount1.text") => s"Amount adjusted to be collected through your PAYE tax code ${taxYear.startYear} to ${taxYear.endYear} tax code."
    case Some("chargeSummary.chargeHistory.amend.paymentOnAccount2.text") => s"Amount adjusted to be collected through your PAYE tax code ${taxYear.startYear} to ${taxYear.endYear} tax code."
