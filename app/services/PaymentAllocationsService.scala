...
class PaymentAllocationsService @Inject() (
  ...
  def getPaymentAllocation(
    ...
    financialDetailsConnector.getFinancialDetailsByDocumentId(nino, documentNumber) flatMap {
      ...
      case documentDetailsWithFinancialDetailsModel: FinancialDetailsWithDocumentDetailsModel =>
        financialDetailsConnector.getPaymentAllocations(
          nino,
          documentDetailsWithFinancialDetailsModel.documentDetails.head.paymentLot.get,
          documentDetailsWithFinancialDetailsModel.documentDetails.head.paymentLotItem.get
        ) flatMap {
      ...
      case _ =>
        Logger("application").error(
          s"$functionName Could not retrieve document with financial details for payment charge model"
        )
        Future.successful(Left(PaymentAllocationError()))
    }
  }

  private def handlePaymentAllocations(
    ...
    if (paymentAllocations.allocations.exists(_.mainType.get.contains("Late Payment Interest"))) {
      createPaymentAllocationForLpi(paymentAllocations, documentDetailsWithFinancialDetailsModel) map {
        lpiPaymentAllocationDetails =>
          lpiPaymentAllocationDetails
            .map(lpiPaymentAllocationDetails =>
              Right(
                PaymentAllocationViewModel(
                  paymentAllocationChargeModel = documentDetailsWithFinancialDetailsModel,
                  latePaymentInterestPaymentAllocationDetails = Some(lpiPaymentAllocationDetails),
                  isLpiPayment = true
                )
              )
            )
            .getOrElse(Left(PaymentAllocationError()))
      }
    } else {
  ...
  private def createPaymentAllocationForLpi(
      paymentCharge:                       PaymentAllocations,
      documentDetailsWithFinancialDetails: FinancialDetailsWithDocumentDetailsModel
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[Option[LatePaymentInterestPaymentAllocationDetails]] = {
        ...
        case (_, financialDetails: FinancialDetailsModel) =>
          financialDetails.documentDetailsWithLpiId(paymentCharge.allocations.head.chargeReference).map {
            documentDetailsWithLpiId =>
              LatePaymentInterestPaymentAllocationDetails(
                documentDetailsWithLpiId,
