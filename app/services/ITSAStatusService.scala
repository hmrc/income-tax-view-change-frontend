...
class ITSAStatusService @Inject() (
  ...
  private def getITSAStatusDetail(
      taxYear:     TaxYear,
      futureYears: Boolean,
      history:     Boolean
    )(
  ...
  def hasMandatedOrVoluntaryStatusCurrentYear(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Boolean] = {
    ...
    getITSAStatusDetail(taxYear, futureYears = false, history = false)
      .map(statusDetail => statusDetail.exists(_.itsaStatusDetails.exists(_.exists(_.isMandatedOrVoluntary))))
  }

  def hasMandatedOrVoluntaryStatusForLatencyYears(
  ...
  def getStatusTillAvailableFutureYears(
      taxYear: TaxYear
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Map[TaxYear, StatusDetail]] = {
