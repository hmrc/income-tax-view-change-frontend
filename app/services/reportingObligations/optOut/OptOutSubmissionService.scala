...
import models.reportingObligations.optOut.{OptOutSessionData, OptOutYearToUpdate}
import connectors.obligations.itsastatus.ITSAStatusUpdateConnectorModel.{ITSAStatusUpdateResponse, ITSAStatusUpdateResponseFailure, optOutUpdateReason}
  ...
  private[services] def blockOptOutForSubsequentYearsValidation(yearsToUpdate: List[OptOutYearToUpdate]): List[OptOutYearToUpdate] = {
    yearsToUpdate match {
      case taxYears@List(OptOutYearToUpdate(_, Voluntary), OptOutYearToUpdate(_, cyStatus), OptOutYearToUpdate(_, Voluntary)) if cyStatus != Voluntary =>
        List(taxYears.head)
      case taxYears@List(OptOutYearToUpdate(_, Voluntary), OptOutYearToUpdate(_, Mandated), OptOutYearToUpdate(_, _)) =>
        List(taxYears.head)
      case allTaxYears =>
        allTaxYears
    }
