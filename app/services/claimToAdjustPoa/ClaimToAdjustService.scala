...
@Singleton
class ClaimToAdjustService @Inject() (
  ...
  def getPoaTaxYearForEntryPoint(
      nino: Nino
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[Either[Throwable, Option[TaxYear]]] = {
    {
      for {
  ...
  def getPoaForNonCrystallisedTaxYear(
      nino: Nino
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[Either[Throwable, Option[PaymentOnAccountViewModel]]] = {
    {
      for {
        financialDetailsMaybe <- EitherT(getNonCrystallisedFinancialDetails(nino))
        paymentOnAccountViewModelMaybe <- EitherT(
          Future.successful(
            financialDetailsMaybe
  ...
  def getPoaViewModelWithAdjustmentReason(
      nino: Nino
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_],
      ec:          ExecutionContext
    ): Future[Either[Throwable, PaymentOnAccountViewModel]] = {
    for {
  ...
  private def getPoaAdjustmentReason(
    ...
    ): Future[Either[Throwable, Option[String]]] = {
    {
      for {
        ...
        chargeHistoryModelMaybe <- EitherT(
          getChargeHistory(chargeHistoryConnector, financialDetails.flatMap(_.chargeReference))
        )
  ...
  private def getNonCrystallisedFinancialDetails(
      nino: Nino
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[Either[Throwable, Option[FinancialDetailsModel]]] = {
  ...
  private def getPoaModelAndFinancialDetailsForNonCrystallised(
      nino: Nino
    )(
      implicit hc: HeaderCarrier,
      user:        MtdItUser[_]
    ): Future[Either[Throwable, FinancialDetailsAndPoaModel]] = {
    checkCrystallisation(nino, getPoaAdjustableTaxYears)(hc, dateService, calculationListConnector, ec).flatMap {
