...
class SessionDataService @Inject() (sessionDataConnector: SessionDataConnector)(implicit ec: ExecutionContext) {
  ...
  private def getSessionResponseFromCookie(implicit request: Request[_]): Future[SessionGetResponse] = {
    ...
    (optMtdid, optUtr, optNino) match {
      case (Some(mtdItId), Some(utr), Some(nino)) =>
        Future(
          Right(
