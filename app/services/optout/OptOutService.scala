...
import services.reporting_frequency.ReportingFrequency.{QuarterlyUpdatesCountForTaxYearModel, noQuarterlyUpdates}
...
class OptOutService @Inject()(
  ...
  def getQuarterlyUpdatesCountForOfferedYears(proposition: OptOutProposition)
    ...
    annualQuarterlyUpdateCounts.
      map(cumulativeQuarterlyUpdateCounts.apply).
