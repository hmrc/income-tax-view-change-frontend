...
class OptOutService @Inject() (
  ...
  def recallOptOutProposition()(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[OptOutProposition] = {
  ...
  private def getITSAStatusesFrom(
      previousYear: TaxYear
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
  ...
  def makeOptOutUpdateRequest(
    ...
    recallOptOutProposition().flatMap { proposition =>
      proposition.optOutPropositionType
        .map {
  ...
  def makeOptOutUpdateRequest(
    ...
    Future
      .sequence(responsesSeqOfFutures)
  ...
  private def makeUpdateCalls(
      optOutYearsToUpdate: Seq[TaxYear]
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier
    ): Seq[Future[ITSAStatusUpdateResponse]] = {
  ...
  def nextUpdatesPageOptOutViewModels(
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[(NextUpdatesQuarterlyReportingContentChecks, Option[OptOutViewModel])] = {
    for {
      proposition <- fetchOptOutProposition()
      _           <- repository.initialiseOptOutJourney(proposition)
  ...
  private def nextUpdatesOptOutViewModel(proposition: OptOutProposition): Option[OptOutViewModel] = {
    proposition.optOutPropositionType.map {
      case p: OneYearOptOutProposition =>
        OptOutOneYearViewModel(oneYearOptOutTaxYear = p.intent.taxYear, state = p.state())
      case _: MultiYearOptOutProposition => OptOutMultiYearViewModel()
    }
  }

  def recallNextUpdatesPageOptOutViewModel(
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Option[OptOutViewModel]] = {
  ...
  def optOutCheckPointPageViewModel(
      ...
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Option[OptOutCheckpointViewModel]] = {

    def processPropositionType(
      ...
      propositionType match {
        case p: OneYearOptOutProposition =>
          Some(
            OneYearOptOutCheckpointViewModel(intent = p.intent.taxYear, state = p.state(), Some(quarterlyUpdatesCount))
          )
  ...
  def optOutConfirmedPageViewModel(
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[Option[ConfirmedOptOutViewModel]] = {
    recallOptOutProposition().flatMap { proposition =>
      proposition.optOutPropositionType match {
        ...
        case Some(p: MultiYearOptOutProposition) =>
          repository.fetchSavedIntent().map(_.map(taxYear => ConfirmedOptOutViewModel(taxYear, p.state())))
        case _ => Future.successful(None)
      }
    }
  }

  def getQuarterlyUpdatesCountForOfferedYears(
      proposition: OptOutProposition
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
    ): Future[QuarterlyUpdatesCountForTaxYearModel] = {

    def cumulativeQuarterlyUpdateCounts(
        taxYearToCount: Seq[QuarterlyUpdatesCountForTaxYear]
      ): Seq[QuarterlyUpdatesCountForTaxYear] =
    ...
    val annualQuarterlyUpdateCounts = Future.sequence(proposition.availableOptOutYears.map {
    ...
    annualQuarterlyUpdateCounts.map(cumulativeQuarterlyUpdateCounts).map(QuarterlyUpdatesCountForTaxYearModel)
  }

  def getTaxYearForOptOutCancelled(
    )(
      implicit user: MtdItUser[_],
      hc:            HeaderCarrier,
      ec:            ExecutionContext
