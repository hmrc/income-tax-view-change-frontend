...
class InYearTaxCalculationControllerISpec extends ComponentSpecBase {
  ...
  object ExpectedValues {
    val title = messages(s"titlePattern.serviceName.govUk") + " - " + messages("base.govUk")
  ...
  object ExpectedValuesWelsh {
    val title = toMessages("CY")(s"titlePattern.serviceName.govUk") + " - " + toMessages("CY")("base.govUk")
  ...
  s"calling GET ${controllers.routes.InYearTaxCalculationController.show().url}" should {
    "display the new calc page in english" which {
      ...
      "have the correct title" in {
        document.title() shouldBe document.getElementsByTag("h1").text() + " - " + ExpectedValues.title
      }
