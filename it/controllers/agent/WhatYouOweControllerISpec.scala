  ...
  "YearOfMigration exists" when {
    "with a multiple charge from financial details and BCD and ACI charges from CESA" in {
      ...
      IncomeTaxViewChangeStub.stubGetFinancialDetailsByDateRange(testNino, s"$previousTaxYearEnd-04-06", s"$currentTaxYearEnd-04-05")(OK,
        testValidFinancialDetailsModelJson(
          2000, 2000, (currentTaxYearEnd - 1).toString, LocalDate.now().toString))
    ...
    "with multiple charges and one charge equals zero" in {
      ...
      val mixedJson = Json.obj(
        ...
        "documentDetails" -> Json.arr(
          documentDetailJson(3400.00, 1000.00, (currentTaxYearEnd - 1).toString, "ITSA- POA 1", transactionId = "transId1"),
          documentDetailJson(1000.00, 100.00, (currentTaxYearEnd - 1).toString, "ITSA- POA 1", transactionId = "transId2"),
          documentDetailJson(1000.00, 0, (currentTaxYearEnd - 1).toString, "ITSA - POA 2", transactionId = "transId3")
        ),
        "financialDetails" -> Json.arr(
          financialDetailJson((currentTaxYearEnd - 1).toString, transactionId = "transId1"),
          financialDetailJson((currentTaxYearEnd - 1).toString, "SA Payment on Account 1", LocalDate.now().plusDays(1).toString, transactionId = "transId2"),
          financialDetailJson((currentTaxYearEnd - 1).toString, "SA Payment on Account 2", LocalDate.now().minusDays(1).toString, transactionId = "transId3")
        ))
    ...
    "YearOfMigration does not exists" when {
      "with a no charge" in {
        ...
        result should have(
          ...
          isElementVisibleById("payment-details-content-2")(expectedValue = false),
          isElementVisibleById("payment-details-content-3")(expectedValue = false),
          isElementVisibleById("payment-details-content-4")(expectedValue = false),
          isElementVisibleById("future-payments-heading")(expectedValue = false),
