    ...
    " [IT-AGENT-TEST-2.1] return the tax year summary page" when {
      " [IT-AGENT-TEST-2.1.1] all calls were successful and returned data" in {
        ...
        result should have(
          ...
          elementTextBySelectorList("#payments", "tbody", "tr:nth-of-type(1)", "a")(poa1Lpi),
          ...
          elementTextBySelectorList("#payments", "tbody", "tr:nth-of-type(2)", "a")(poa1),
          ...
          elementTextBySelectorList("#updates", "div", "table", "tbody", "tr:nth-of-type(2)", "th:nth-of-type(1)")(annualUpdate),
        ...
        verifyAuditEvent(TaxYearSummaryResponseAuditModel(testUser, financialDetailsSuccess.getAllDocumentDetailsWithDueDates(),
          allObligations, Some(TaxYearSummaryViewModel(liabilityCalculationModelSuccessful,testTaxYear)), false))
      ...
      " [IT-AGENT-TEST-2.1.6] previous obligations data was not found" in {
        ...
        verifyAuditEvent(TaxYearSummaryResponseAuditModel(testUser, financialDetailsSuccess.getAllDocumentDetailsWithDueDates(),
          currentObligationsSuccess, Some(TaxYearSummaryViewModel(liabilityCalculationModelSuccessful,testTaxYear)), false))
        verifyAuditContainsDetail(NextUpdatesResponseAuditModel(testUser, "testId", currentObligationsSuccess.obligations.flatMap(_.obligations)).detail)
      }
    }
    ...
    "MFA Debits" when {
      ...
      def verifyMFADebitsResults(result: WSResponse): Any = {
        ...
        verifyAuditEvent(TaxYearSummaryResponseAuditModel(testUser, auditDD, allObligations,
          Some(TaxYearSummaryViewModel(liabilityCalculationModelSuccessful,testTaxYear)), isEnabled(R7bTxmEvents)))
        verifyAuditContainsDetail(NextUpdatesResponseAuditModel(testUser, "testId", currentObligationsSuccess.obligations.flatMap(_.obligations)).detail)
        verifyAuditContainsDetail(NextUpdatesResponseAuditModel(testUser, "testId2", previousObligationsSuccess.obligations.flatMap(_.obligations)).detail)
