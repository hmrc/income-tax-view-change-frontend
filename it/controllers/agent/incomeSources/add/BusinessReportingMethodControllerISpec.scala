...
import testConstants.IncomeSourceIntegrationTestConstants.{multipleBusinessesAndPropertyResponse, singleBusinessResponse, singleBusinessResponseInLatencyPeriod}
...
class BusinessReportingMethodControllerISpec extends ComponentSpecBase {
  ...
  val businessAddedShowUrl: String = controllers.incomeSources.add.routes.IncomeSourceAddedController.showAgent(testSelfEmploymentId, SelfEmployment).url
  ...
  s"calling GET $businessReportingMethodShowUrl" should {
    "render the Business Reporting Method page" when {
      "URL contains a valid income source ID and authorised user is within latency period (Tax Year 1 NOT crystallised)" in {
        ...
        val result = IncomeTaxViewChangeFrontend.get(s"/income-sources/add/business-reporting-method?id=$testSelfEmploymentId", clientDetailsWithConfirmation)
  ...
  s"calling POST $businessReportingMethodSubmitUrl" should {
    s"redirect to $businessAddedShowUrl" when {
      "user completes the form and API 1776 updateIncomeSource returns a success response" in {
        ...
        AuditStub.verifyAuditContainsDetail(IncomeSourceReportingMethodAuditModel(true, SelfEmployment.journeyType, "ADD", "Annually", "2023-2024", "business")(testUser).detail)
