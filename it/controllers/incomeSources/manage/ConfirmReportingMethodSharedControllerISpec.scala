...
import testConstants.BaseIntegrationTestConstants.{credId, testFourDigitTaxYearRange, testMtditid, testNino, testPropertyIncomeId, testSaUtr, testSelfEmploymentId, testTaxYear}
        ...
        AuditStub
          .verifyAuditEvent(
            ChangeReportingMethodAuditModel(
              ...
              reportingMethodChangeTo = annual,
              taxYear = taxYear,
              errorMessage = Seq(FormError("key", s"${ConfirmReportingMethodForm.noSelectionError}.annual")),
              messagesApi = messagesAPI
            )(
              MtdItUser(
                mtditid = testMtditid,
