...
class UIJourneySessionDataISpec extends ComponentSpecBase {
  ...
  "UI Journey Session Data" should {
    "set some data" in {
      val acknowledged = await(
        repository.set(
          UIJourneySessionData(
            "session-12345",
            IncomeSourceJourneyType(Add, UkProperty).toString,
            Some(AddIncomeSourceData(Some("business1")))
          )
        )
      )
    ...
    "updateDate should set a data field correctly" in {
      ...
      val updateResult = await(
        repository.updateData(
          UIJourneySessionData("session-12345", IncomeSourceJourneyType(Add, UkProperty).toString),
          "addIncomeSourceData.businessName",
          "business2"
        )
      )
    ...
    "deleteOne should remove a sessionData item" in {
      ...
      val result = await(
        repository.deleteOne(UIJourneySessionData("session-12345", IncomeSourceJourneyType(Add, UkProperty).toString))
      )
    ...
    "clearSession should remove all session data with a session ID" in {
      ...
      await(
        repository.set(
          UIJourneySessionData(
            "session-12345",
            IncomeSourceJourneyType(Add, ForeignProperty).toString,
            Some(AddIncomeSourceData(Some("business1")))
          )
        )
      )
