        ...
        "redirect to the cannot go back page" in {
          ...
          val redirectUrl = {
            if(mtdUserRole != MTDIndividual)
              obligations.controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = true, isSignUpJourney = Some(true)).url
            else
              obligations.controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = false, isSignUpJourney = Some(true)).url
          }
