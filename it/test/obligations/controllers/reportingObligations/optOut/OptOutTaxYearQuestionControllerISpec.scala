...
import obligations.helpers.{ITSAStatusUpdateConnectorStub, ObligationsStub, OptOutSessionRepositoryHelper}
...
class OptOutTaxYearQuestionControllerISpec extends ControllerISpecHelper {
  ...
  object optOutTaxYearQuestionMessages {
    ...
    val previousSingleYearDesc1 = "You can only opt out for the 2021 to 2022 tax year."
    ...
    val singleYearFollowedByMandatedDesc1 = "You can only opt out for the 2022 to 2023 tax year."
    ...
    s"a user is a $mtdUserRole" that {
      "is authenticated with valid enrolment" should {
        "render the opt out tax year question page - CY-1 Onwards (V, V, V)" in {
          ...
          ObligationsStub.stubGetAllObligations(testNino, taxYear.toFinancialYearStart, taxYear.toFinancialYearEnd, allObligations)
        ...
        "render the opt out tax year question page - Single Year Followed By Mandated (CY-1)" in {
          ...
          GetInsourceDetailsStub.stubGetIncomeSourceDetailsResponse(testMtditid)(OK, propertyOnlyResponse)
          ObligationsStub.stubGetAllObligations(testNino, taxYear.toFinancialYearStart, taxYear.toFinancialYearEnd, obligationWithSubmittedQuarterlyUpdates)
          ITSAStatusDetailsStub.stubGetITSAStatusFutureYearsDetails(
            taxYear = taxYear,
            `itsaStatusCY-1` = Voluntary,
      ...
      "submit the answer to the opt out tax year question - single year" in {
        ...
        val expectedURI = if (mtdUserRole == MTDIndividual) {
          obligations.controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent = false).url
        } else {
          obligations.controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent = true).url
        }
      ...
      "submit the answer to the opt out tax year question - opt out followed by mandated redirects to check your answers" in {
        ...
        val expectedURI =
          obligations.controllers.reportingObligations.optOut.routes.ConfirmOptOutUpdateController
      ...
      "has already completed the journey (according to session data)" should {
        "redirect to the cannot go back page" in {
          ...
          val redirectUrl: String =
            if (mtdUserRole != MTDIndividual) obligations.controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = true, isSignUpJourney = Some(false)).url
            else obligations.controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent = false, isSignUpJourney = Some(false)).url
          val result = buildGETMTDClient(s"$path?taxYear=$currentYear", additionalCookies).futureValue
