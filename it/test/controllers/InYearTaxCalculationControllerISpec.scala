...
class InYearTaxCalculationControllerISpec extends ControllerISpecHelper {
  ...
  val (taxYear, month, dayOfMonth) = (
    if (currentDate.isAfter(toTaxYearEndDate(currentDate.getYear))) {
  ...
  object Selectors {
    ...
    val incomeRowAmount =
      "#income-deductions-contributions-table > tbody > tr:nth-child(1) > td.govuk-table__cell.govuk-table__cell--numeric"
    val allowanceRowText =
      "#income-deductions-contributions-table > tbody > tr:nth-child(2) > th:nth-child(1) > div > a"
    val allowanceRowAmount =
      "#income-deductions-contributions-table > tbody > tr:nth-child(2) > td.govuk-table__cell.govuk-table__cell--numeric"
    val taxIsDueRowText = "#income-deductions-contributions-table > tbody > tr:nth-child(3) > th:nth-child(1)"
    val taxIsDueRowAmount =
      "#income-deductions-contributions-table > tbody > tr:nth-child(3) > td.govuk-table__cell.govuk-table__cell--numeric"
  ...
  object ExpectedValuesWelsh {
    ...
    val title: Boolean => String = isAgent =>
      if (isAgent) {
        s"Trosolwg o dreth eich cleient 6 Ebrill $previousTaxYearString i 5 Ebrill $taxYearString - Rheoli diweddariadau Treth Incwm eich cleient - GOV.UK"
      } else {
        s"Trosolwg o’ch treth 6 Ebrill $previousTaxYearString i 5 Ebrill $taxYearString - Rheoli’ch diweddariadau Treth Incwm - GOV.UK"
      }
              ...
              "render the new calc page" that {
                "is in english" in {
                  ...
                  res should have(
                    ...
                    elementAttributeBySelector(Selectors.allowanceRowText, "href")(
                      ExpectedValues.allowanceLink(isAgent)
                    ),
                ...
                "is in welsh" in {
                  ...
                  res should have(
                    ...
                    elementAttributeBySelector(Selectors.incomeRowText, "href")(
                      ExpectedValuesWelsh.incomeLink(isAgent)
                    ),
                    ...
                    elementAttributeBySelector(Selectors.allowanceRowText, "href")(
                      ExpectedValuesWelsh.allowanceLink(isAgent)
                    ),
