  ...
  s"calling POST $addIncomeSourcesAccountingMethodShowUrlSoleTrader" should {
    s"redirect to $checkBusinessDetailsShowUrl" when {
      "user selects 'cash basis accounting', 'cash' should be added to session storage" in {
        ...
        runPostTest(checkBusinessDetailsShowUrl, "/income-sources/add/business-accounting-method", formData, SelfEmployment, Some("cash"))
      }
      s"redirect to $checkBusinessDetailsShowUrl" when {
        "user selects 'traditional accounting', 'accruals' should be added to session storage" in {
          ...
          runPostTest(checkBusinessDetailsShowUrl, "/income-sources/add/business-accounting-method", formData, SelfEmployment, Some("accruals"))
        }
      }
  ...
  s"calling POST $addIncomeSourcesAccountingMethodShowUrlUK" should {
    s"redirect to $checkUKPropertyDetailsShowUrl" when {
      "user selects 'cash basis accounting', 'cash' should be added to session storage" in {
        ...
        runPostTest(checkUKPropertyDetailsShowUrl, "/income-sources/add/uk-property-accounting-method", formData, UkProperty, Some("cash"))
      }
      s"redirect to $checkUKPropertyDetailsShowUrl" when {
        "user selects 'traditional accounting', 'accruals' should be added to session storage" in {
          val formData: Map[String, Seq[String]] = Map(UKPropertyAccountingMethod -> Seq("traditional"))
          runPostTest(checkUKPropertyDetailsShowUrl, "/income-sources/add/uk-property-accounting-method", formData, UkProperty, Some("accruals"))
        }
      }
  ...
  s"calling POST $addIncomeSourcesAccountingMethodShowUrlForeign" should {
    s"redirect to $foreignPropertyCheckDetailsShowUrl" when {
      "user selects 'cash basis accounting', 'cash' should be added to session storage" in {
        ...
        runPostTest(foreignPropertyCheckDetailsShowUrl, "/income-sources/add/foreign-property-business-accounting-method", formData, ForeignProperty, Some("cash"))
      }
      s"redirect to $foreignPropertyCheckDetailsShowUrl" when {
        "user selects 'traditional accounting', 'accruals' should be added to session storage" in {
          val formData: Map[String, Seq[String]] = Map(foreignPropertyAccountingMethod -> Seq("traditional"))
          runPostTest(foreignPropertyCheckDetailsShowUrl, "/income-sources/add/foreign-property-business-accounting-method", formData, ForeignProperty, Some("accruals"))
        }
      }
