...
class SingleTaxYearOptInWarningControllerISpec extends ControllerISpecHelper {
  ...
  private val confirmTaxYearPage = controllers.optIn.routes.ConfirmTaxYearController.show(isAgent).url

  private val optInCancelledPageUrl = controllers.optIn.routes.OptInCancelledController.show().url
        ...
        optInSessionData =
          Some(OptInSessionData(
            optInContextData = Some(OptInContextData(
              ...
              currentYearITSAStatus = statusToString(status = currentYearStatus, isNextYear = false),
              nextYearITSAStatus = statusToString(status = nextYearStatus, isNextYear = true)
            )),
