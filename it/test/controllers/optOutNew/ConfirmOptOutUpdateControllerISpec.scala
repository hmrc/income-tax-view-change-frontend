...
import models.admin.{NavBarFs, OptOutFs, ReportingFrequencyPage}
          ...
          "redirect [TEMP LOOP]" when {
            "user confirms opt-out for one-year scenario" in {
              ...
              result should have(
                ...
                redirectURI(controllers.optOutNew.routes.ConfirmOptOutUpdateController.show(isAgent, currentTaxYear(dateService).startYear.toString).url)
              )
