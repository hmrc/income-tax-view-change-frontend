    ...
    s"a user is a $mtdUserRole" that {
      ...
      "submit the answer to the opt out tax year question - single year" in {
        ...
        val expectedURI = if (mtdUserRole == MTDIndividual) {
          controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent = false).url
        } else {
          controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent = true).url
        }
      ...
      "submit the answer to the opt out tax year question - opt out followed by mandated redirects to check your answers" in {
        ...
        val expectedURI =
          controllers.reportingObligations.optOut.routes.ConfirmOptOutUpdateController
      ...
      "has already completed the journey (according to session data)" should {
        "redirect to the cannot go back page" in {
          ...
          val redirectUrl: String =
            if (mtdUserRole != MTDIndividual) controllers.reportingObligations.routes.SignUpOptOutCannotGoBackController.show(isAgent = true, isSignUpJourney = Some(false)).url
            else controllers.reportingObligations.routes.SignUpOptOutCannotGoBackController.show(isAgent = false, isSignUpJourney = Some(false)).url
          val result = buildGETMTDClient(s"$path?taxYear=$currentYear", additionalCookies).futureValue
