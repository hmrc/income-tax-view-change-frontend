      ...
      s"a user is a $mtdUserRole" that {
        ...
        "has already completed the journey (according to session data)" should {
          s"redirect to the cannot go back page" in {
            ...
            val redirectUrl: String = controllers.reportingObligations.routes.SignUpOptOutCannotGoBackController.show(isAgent, isSignUpJourney = Some(false)).url
            val result = buildGETMTDClient(path, additionalCookies).futureValue
            IncomeTaxViewChangeStub.verifyGetIncomeSourceDetails(testMtditid)
        ...
        "is authenticated, with a valid enrolment" should {
          ...
          "redirect to the completion page" when {
            ...
            "user confirms opt-out for one-year scenario" in {
              ...
              result should have(
                httpStatus(SEE_OTHER),
                redirectURI(controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent).url)
              )
            }
          }
