            ...
            if (mtdUserRole == MTDSupportingAgent) {
              ...
              "redirects to Tax summary" when {
                "the session key contains calculationId" in {
                  ...
                  val result =
                    buildGETMTDClient(path, additionalCookies ++ Map(SessionKeys.calculationId -> "idOne")).futureValue
                ...
                "calculation service returns retryable response back initially and then returns success response before interval time completed" in {
                  ...
                  IncomeTaxCalculationStub.stubGetCalculationErrorResponseByCalcId(testNino, "idFour", testTaxYear)(
                    NO_CONTENT,
                  ...
                  IncomeTaxCalculationStub.verifyGetCalculationResponseByCalcId(
                    testNino,
                    "idFour",
                    testTaxYear,
                    noOfCalls = 6
              ...
              "render the error page" when {
                "calculation service returns non-retryable response back" in {
                  ...
                  IncomeTaxCalculationStub.stubGetCalculationErrorResponseByCalcId(testNino, "idTwo", testTaxYear)(
                    INTERNAL_SERVER_ERROR,
                    LiabilityCalculationError(INTERNAL_SERVER_ERROR, "error")
                  )

                  val result =
                    buildGETMTDClient(path, additionalCookies ++ Map(SessionKeys.calculationId -> "idTwo")).futureValue
                ...
                "calculation service returns retryable response back" in {
                  ...
                  IncomeTaxCalculationStub.stubGetCalculationErrorResponseByCalcId(testNino, "idThree", testTaxYear)(
                    NO_CONTENT,
                    LiabilityCalculationError(NO_CONTENT, "no content")
                  )

                  val result = buildGETMTDClient(
                    path,
                    additionalCookies ++ Map(SessionKeys.calculationId -> "idThree")
                  ).futureValue
                  IncomeTaxCalculationStub.verifyGetCalculationResponseByCalcId(
                    testNino,
                    "idThree",
                ...
                "calculation service returns retryable response back initially and then returns non-retryable error before interval time completed" in {
                  ...
                  IncomeTaxCalculationStub.stubGetCalculationErrorResponseByCalcId(testNino, "idFive", testTaxYear)(
                    NO_CONTENT,
                    LiabilityCalculationError(NO_CONTENT, "no content")
                  )
            ...
            if (mtdUserRole == MTDSupportingAgent) {
              ...
              "redirects to final tax calculation page" when {
                "the session key contains calculationId" in {
                  ...
                  val result = buildGETMTDClient(
                    finalCalcPath,
                    additionalCookies ++ Map(SessionKeys.calculationId -> "idOne")
                  ).futureValue
                  val expectedRedirectUrl = if (isAgent) {
                ...
                "calculation service returns retryable response back initially and then returns success response before interval time completed" in {
                  ...
                  val res =
                    buildGETMTDClient(finalCalcPath, additionalCookies ++ Map(SessionKeys.calculationId -> "idFour"))
              ...
              "render the error page" when {
                "calculation service returns non-retryable response back" in {
                  ...
                  val result = buildGETMTDClient(
                    finalCalcPath,
                    additionalCookies ++ Map(SessionKeys.calculationId -> "idTwo")
                  ).futureValue
                ...
                "calculation service returns retryable response back initially and then returns non-retryable error before interval time completed" in {
                  ...
                  val res =
                    buildGETMTDClient(finalCalcPath, additionalCookies ++ Map(SessionKeys.calculationId -> "idFive"))
