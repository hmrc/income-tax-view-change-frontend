...
class CreditAndRefundControllerISpec extends ControllerISpecHelper {
              ...
              "render the credit and refund page" that {
                "has all credits/refund types" when {
                  "a valid response is received and feature switches are enabled" in new CustomFinancialDetailsResponse(
                    Seq(
                      FinancialDetailsResponse(taxYear = testTaxYear, code = OK, json = Json.toJson(validResponseModel))
                    ),
                    mtdUserRole
                  ) {
                    ...
                    IncomeTaxViewChangeStub.verifyGetFinancialDetailsCreditsByDateRange(
                      testNino,
                      s"$testPreviousTaxYear-04-06",
                      s"$testTaxYear-04-05"
                    )
                    ...
                    res should have(
                      httpStatus(OK),
                      elementTextBySelectorList("#main-content", "li:nth-child(1)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.row.repaymentInterest-2"
                          ) + s" $testPreviousTaxYear to $testTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(2)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.credit-from-balancing-charge-prt-1"
                          ) + s" $testPreviousTaxYear to $testTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(3)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.credit-from-adjustment-prt-1"
                          ) + s" $testPreviousTaxYear to $testTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(4)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.credit-from-earlier-tax-year"
                          ) + s" $testPreviousTaxYear to $testTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(5)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.credit-from-earlier-tax-year"
                          ) + s" ${testPreviousTaxYear - 1} to $testPreviousTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(6)", "p")(expectedValue =
                        "£2,000.00 " +
                          messagesAPI(
                            "credit-and-refund.credit-from-earlier-tax-year"
                          ) + s" ${testPreviousTaxYear - 1} to $testPreviousTaxYear tax year"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(7)", "p")(expectedValue =
                        "£500.00 " +
                          messagesAPI("credit-and-refund.payment") + " 29 March 2022"
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(8)", "p")(expectedValue =
                        "£3.00 "
                          + messagesAPI("credit-and-refund.refundProgress-prt-2")
                      ),
                      elementTextBySelectorList("#main-content", "li:nth-child(9)", "p")(expectedValue =
                        "£2.00 "
                          + messagesAPI("credit-and-refund.refundProgress-prt-2")
                      ),
                ...
                "displays 'no money in your account' message" when {
                  ...
                  "a not found response from the API is received" in new CustomFinancialDetailsResponse(
                    ...
                    res should have(
                      httpStatus(OK),
                      elementTextBySelectorList("#main-content", "p")(expectedValue =
  ...
  class CustomFinancialDetailsResponse(
      responses:              Seq[FinancialDetailsResponse],
