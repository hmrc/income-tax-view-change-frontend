...
class ManageIncomeSourceDetailsISpecHelper extends ControllerISpecHelper {
  ...
  val manageSelfEmploymentShowUrl: String =
    controllers.manageBusinesses.manage.routes.ManageIncomeSourceDetailsController
      .show(isAgent = false, SelfEmployment, Some(testSelfEmploymentId))
      .url
  val manageUKPropertyShowUrl: String = controllers.manageBusinesses.manage.routes.ManageIncomeSourceDetailsController
    .show(isAgent = false, UkProperty, None)
    .url
  val manageForeignPropertyShowUrl: String =
    controllers.manageBusinesses.manage.routes.ManageIncomeSourceDetailsController
  ...
  val messagesAnnuallyGracePeriod: String = messagesAPI(
    "incomeSources.manage.business-manage-details.annually.graceperiod"
  )
  val messagesQuarterlyGracePeriod: String = messagesAPI(
    "incomeSources.manage.business-manage-details.quarterly.graceperiod"
  )
