...
class RemoveClientDetailsSessionsControllerISpec extends ControllerISpecHelper {
  ...
  s"GET ${hub.v1.controllers.agent.routes.EnterClientsUTRController.show().url}" when {
      ...
      "Removing the client details session keys" should {
        "redirect to client UTR page" in {
          ...
          result should have(
            ...
            redirectURI(hub.v1.controllers.agent.routes.EnterClientsUTRController.show().url)
          )
