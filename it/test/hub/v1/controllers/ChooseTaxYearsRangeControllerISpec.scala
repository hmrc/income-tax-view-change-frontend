  ...
  s"POST $path" should {
    "redirect to your-tasks when MTD option is selected" in {
      ...
      result should have(
        ...
        redirectURI("/report-quarterly/income-and-expenses/view/your-tasks")
      )
    ...
    testAuthFailures(
      path,
      MTDIndividual,
      optBody = Some(Map(ChooseTaxYearsRangeForm.response -> Seq(ChooseTaxYearsRangeForm.mtdOption)))
    )
  }
}
