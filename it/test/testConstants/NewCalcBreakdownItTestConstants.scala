...
object NewCalcBreakdownItTestConstants {
          ...
          Reliefs(
            reliefsClaimed = Some(
              Seq(
                ...
                ReliefsClaimed(`type` = "vctSubscriptions2", amountUsed = Some(5000.99))
              )
            ),
  ...
  val liabilityCalculationModelSuccessfulNotCrystallised: LiabilityCalculationResponse =
  ...
  val liabilityCalculationModelErrorMessages = liabilityCalculationModelDeductionsMinimal.copy(messages =
    Some(
      Messages(
        errors = Some(
          List(
  ...
  val liabilityCalculationModelErrorMessagesFormatted: MTDUserRole => LiabilityCalculationResponse = mtdUserRole =>
    liabilityCalculationModelDeductionsMinimal.copy(messages =
      Some(
        Messages(
          errors = Some(
            List(
              ...
              Message(
                "C15507",
                s"${if (mtdUserRole == MTDIndividual) "you’ve" else "your client"} claimed £2000 in Property Income Allowance but this is more than turnover for ${if (mtdUserRole == MTDIndividual) "your" else "their"} UK property."
              ),
              Message(
                "C15510",
                "the Rent a Room relief claimed for a jointly let property cannot be more than 10% of the Rent a Room limit."
