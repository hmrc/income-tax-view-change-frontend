...
trait ComponentSpecBase extends TestSuite with CustomMatchers
  ...
  object IncomeTaxViewChangeFrontend {
    ...
    def getUTRError(clientUTR: Map[String, String] = Map.empty): WSResponse = get("/not-authorised-to-view-client", clientUTR)

    def getAgentError: WSResponse = get("/agent-error")

    def postUTRError: WSResponse = post("/not-authorised-to-view-client")(Map.empty)
