...
import financials.mocks.services.{MockClaimToAdjustPoaCalculationService, MockClaimToAdjustService, MockPaymentOnAccountSessionService}
        ...
        if (mtdRole == MTDSupportingAgent) {
          ...
          "redirect to the You Cannot Go Back page" when {
            "the journeyCompleted flag is set to true in session" in {
              ...
              redirectLocation(result) shouldBe Some(claimToAdjustPoaRoutes.YouCannotGoBackController.show(isAgent).url)
            }
          }
        ...
        if (mtdRole == MTDSupportingAgent) {
          ...
          "redirect to PoaAdjustedController page" when {
            "data to API 1773 successfully sent" in {
              ...
              val result = action(fakeRequest)
              redirectLocation(result) shouldBe Some(claimToAdjustPoaRoutes.PoaAdjustedController.show(isAgent = isAgent).url)
            }
          }
          ...
          "redirect to API error page" when {
            "data to API 1773 failed to be sent" in {
              ...
              setupMockSuccess(mtdRole, withNrs = true)
              val result = action(fakeRequest)
              redirectLocation(result) shouldBe Some(claimToAdjustPoaRoutes.ApiFailureSubmittingPoaController.show(isAgent).url)
            }
          }
