...
package financials.services.claimToAdjust
...
import financials.mocks.connectors.{MockChargeHistoryConnector, MockFinancialDetailsConnector}
...
class ClaimToAdjustServiceSpec extends TestSupport with MockFinancialDetailsConnector with MockChargeHistoryConnector
  ...
  def newFixture(date: LocalDate): (ClaimToAdjustService, DateService) = {
    ...
    val service = new ClaimToAdjustService(
  ...
  val financialDetailsModelBothPoas: FinancialDetailsModel = FinancialDetailsModel(
    balanceDetails = BalanceDetails(0.0, 0.0, 0.0, 0.0, None, None, None, None, None, None, None, None),
  ...
  "getAmendablePoaViewModel" should {
    "return a future right with AmendablePoaViewModel when POAs are unpaid" in {
      ...
      val (service, _)  = newFixture(LocalDate.of(taxYear, 8, 27))
      val result = service.getAmendablePoaViewModel(testUserNino)
    ...
    "return a future left with an exception when an exception is thrown" in {
      ...
      val (service, _)  = newFixture(LocalDate.of(taxYear, 8, 27))
      val futureResult = service.getAmendablePoaViewModel(testUserNino)
  ...
  "getPoaViewModelWithAdjustmentReason" should {
    ...
    "return a future right with PoaAmountViewModel when POAs are unpaid" in {
      ...
      val result = service.getPoaViewModelWithAdjustmentReason(testUserNino)
    ...
    "return a future left with an exception when an exception is thrown" in {
      ...
      val (service, _)  = newFixture(LocalDate.of(taxYear, 8, 27))
      val futureResult = service.getPoaViewModelWithAdjustmentReason(testUserNino)
