      ...
      s"the user is authenticated as a $mtdRole" should {
        ...
        s"redirect to cannot-go-back page" when {
          "there is no tax year eligible for opt out" in {
            ...
            redirectLocation(result) shouldBe Some(obligations.controllers.errors.routes.SignUpOptOutCannotGoBackController.show(isAgent, isSignUpJourney = Some(false)).url)
          }
