...
import obligations.services.reportingObligations.optOut.{CurrentOptOutTaxYear, MultiYearOptOutDefault, OneYearOptOutFollowedByMandated, OptOutProposition, OptOutService, OptOutSubmissionService}
...
//          mockMakeOptOutUpdateRequest(Future.successful(ITSAStatusUpdateResponseSuccess()))
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe Some(obligations.controllers.reportingObligations.optOut.routes.ConfirmedOptOutController.show(isAgent).url)
        }
...
//          mockMakeOptOutUpdateRequest(Future.successful(ITSAStatusUpdateResponseSuccess()))
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe Some(obligations.controllers.reportingObligations.optOut.routes.ConfirmOptOutUpdateController.show(isAgent, currentYear.getOrElse("")).url)
        }
