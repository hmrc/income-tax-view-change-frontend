...
class BusinessEndDateFormSpec extends AnyWordSpec with Matchers with TestSupport {
  ...
  "BusinessEndDate form" should {
    "bind with a valid date" in {
      val form: Form[DateFormElement] = new BusinessEndDateForm(mockDateService).apply(SelfEmployment, Some(testSelfEmploymentId))(testUser)
    ...
    "bind with an invalid date field - Self employment" in {
      ...
      completedForm.errors shouldBe List(FormError("business-end-date", List("incomeSources.cease.endDate.selfEmployment.error.invalid"), List()))
    }
    "bind with an invalid date field - Foreign property" in {
      val form: Form[DateFormElement] = new BusinessEndDateForm(mockDateService).apply(ForeignProperty, Some(testSelfEmploymentId))(testUser)
      val formData = Map("business-end-date.day" -> "yo", "business-end-date.month" -> "yo", "business-end-date.year" -> "supp")
      val completedForm = form.bind(formData)
      ...
      completedForm.data.get("business-end-date.month") shouldBe Some("yo")
      completedForm.data.get("business-end-date.year") shouldBe Some("supp")
      completedForm.errors shouldBe List(FormError("business-end-date", List("incomeSources.cease.endDate.foreignProperty.error.invalid"), List()))
    }
    "bind with an invalid date field - UK Property" in {
      val form: Form[DateFormElement] = new BusinessEndDateForm(mockDateService).apply(UkProperty, Some(testSelfEmploymentId))(testUser)
      val formData = Map("business-end-date.day" -> "yo", "business-end-date.month" -> "yo", "business-end-date.year" -> "supp")
      val completedForm = form.bind(formData)
      ...
      completedForm.data.get("business-end-date.month") shouldBe Some("yo")
      completedForm.data.get("business-end-date.year") shouldBe Some("supp")
      completedForm.errors shouldBe List(FormError("business-end-date", List("incomeSources.cease.endDate.ukProperty.error.invalid"), List()))
    }
    "bind with a future date" in {
      ...
      completedForm.errors shouldBe List(FormError("business-end-date", List("dateForm.error.future"), List()))
    }
    "bind with a date earlier than the business start date" in {
      ...
      completedForm.errors shouldBe List(FormError("business-end-date", List("dateFrom.error.beforeStartDate"), List()))

    }
    "give the correct error when binding with a date both before business start date and the 6th of April 2015" in {
      val form: Form[DateFormElement] = new BusinessEndDateForm(mockDateService).apply(SelfEmployment, Some(testSelfEmploymentId))(testUser2)
      ...
      completedForm.errors shouldBe List(
        FormError("business-end-date", List("incomeSources.cease.endDate.selfEmployment.error.beforeEarliestDate"), List())
      )
    }
