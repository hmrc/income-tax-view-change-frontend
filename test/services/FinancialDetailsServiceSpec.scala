...
class FinancialDetailsServiceSpec extends TestSupport with MockIncomeTaxViewChangeConnector with FeatureSwitching {
  ...
  private def getFinancialDetailSuccess(taxYear: Int,
    ...
    FinancialDetailsModel(balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None), codingDetails = codingDetails, documentDetails = documentDetails, financialDetails = financialDetails)
  }
      ...
      "return a single non-overdue date" when {
        "there are no overdue dates, but there are dates upcoming" in {
          val financialDetailsCurrentYear: FinancialDetailsModel = FinancialDetailsModel(
            ...
            financialDetails = List(
              FinancialDetail("testYear1", None, Some("testTransactionId1"), None, None, None, None, None, None, None, None, Some(Seq(SubItem(Some(LocalDate.now.plusDays(7)))))),
              FinancialDetail("testYear1", None, Some("testTransactionId2"), None, None, None, None, None, None, None, None, Some(Seq(SubItem(Some(LocalDate.now.plusDays(1))))))
            )
          )
