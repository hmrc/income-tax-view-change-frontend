...
class OptInPropositionExcelSpec extends UnitSpec {
  ...
  private def toTaxYear(option: String): TaxYear = {
    option match {
  ...
  private def toITSAStatus(status: String): ITSAStatus =
    status match {
      case "M" => Mandated
      case "V" => Voluntary
      case "A" => Annual
  ...
  "check all required scenarios" in {
    ...
    forAll(scenarios) { (
                          cyStatus: String,
                          nyStatus: String,
                          intent: String, // user chosen tax year
                          valid: Boolean,
                          presented: List[String],
                          sent: String,
                          expectedCY: String,
      ...
      "OptInProposition" when {

        ".afterwardsStatus()" when {

          s"$cyStatus and $nyStatus with user chosen tax year: $intent" should {

            s"return the expected expectedCY: $expectedCY and expectedNY: $expectedNY statuses" in {

              optInProposition.afterwardsStatus(toTaxYear(intent)) shouldBe Seq(toITSAStatus(expectedCY), toITSAStatus(expectedNY))
            }
          }
        }
      }
