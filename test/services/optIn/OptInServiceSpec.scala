...
import models.optin.{OptInCompletedViewModel, OptInContextData, OptInSessionData}
...
class OptInServiceSpec extends UnitSpec
  ...
  "OptInService.optinCompletedPageModel" when {

    "CY is chosen as the selected tax year intent scenario" when {
      ...
      "configuring the showAnnualReportingAdvice flag" when {

        "both CY & CY+1 statuses are both Annual" should {

          "return correctly configured OptInCompletedViewModel, NOT show the showAnnualReportingAdvice" in {
        ...
        "CY & CY+1 statuses are both NOT Annual" should {

          "return correctly configured OptInCompletedViewModel, NOT showing the showAnnualReportingAdvice" in {

            val isAgent = false

            val taxYear2023 = TaxYear(2023, 2024)

            val optInContext = Some(OptInContextData(taxYear2023.toString, statusToString(Voluntary), statusToString(Voluntary)))
    ...
    "CY+1 is chosen as the selected tax year intent scenario" when {

      "selected tax year intent is NOT equal to the current year" should {

        "return correctly configured OptInCompletedViewModel, isCurrentYear == false" in {
          ...
          mockRepository(optInContextData = optInContext, selectedOptInYear = Some(taxYear2023.nextYear.toString))
          ...
          result.value.futureValue shouldBe
            Some(
              OptInCompletedViewModel(
                isAgent = isAgent,
                optInTaxYear = taxYear2023.nextYear,
      ...
      "configuring the showAnnualReportingAdvice flag" when {
        ...
        "both CY & CY+1 statuses are both Annual" should {
          ...
          "return correctly configured OptInCompletedViewModel, showing the showAnnualReportingAdvice" in {
