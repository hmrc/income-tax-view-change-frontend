...
class DateServiceSpec extends TestSupport with FeatureSwitching {
  ...
  "The getCurrentDate method when TimeMachineAddYear FS is off" should {
    "return the current date" in {
      ...
      TestDateService.getCurrentDate(false) should equal(LocalDate.now)
    }

    "return mocked current date: 2018-03-29" in {
      val f = fixture()
      disable(TimeMachineAddYear)
      f.mockedTestDateService.getCurrentDate(false) should equal(LocalDate.parse("2018-03-29"))
    ...
    "return mocked current date: 2019-04-05" in {
      ...
      disable(TimeMachineAddYear)
      f.mockedTestDateService.getCurrentDate(false) should equal(LocalDate.parse("2019-04-05"))
      f.mockedTestDateService.isDayBeforeTaxYearLastDay(false) shouldBe true
    }
  ...
  "The getCurrentTaxYearEnd" should {
    "return the current tax year" in {
      ...
      TestDateService.getCurrentTaxYearEnd(false) shouldBe expectedYear
    }
