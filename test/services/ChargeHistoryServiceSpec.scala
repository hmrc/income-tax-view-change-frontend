...
class ChargeHistoryServiceSpec extends TestSupport with MockChargeHistoryConnector {
  ...
  object TestChargeHistoryService
      extends ChargeHistoryService(
  ...
  val testChargeHistory: List[ChargeHistoryModel] = List(
    ChargeHistoryModel(
      taxYear = taxYear.toString,
      documentId = docNumber,
      documentDate = LocalDate.of(2021, 1, 1),
      documentDescription = "desc",
      totalAmount = 1000,
      reversalDate = LocalDate.of(2021, 1, 1),
      reversalReason = "",
      poaAdjustmentReason = Some(MainIncomeLower.code)
    )
  )

  val chargesHistoryWithHistory: ChargesHistoryModel =
    ChargesHistoryModel("NINO", "AB123456C", "ITSA", Some(testChargeHistory))
  ...
  "ChargeHistoryService.chargeHistoryResponse" should {
    "return a Right(Nil)" when {
      "the conditions are not met" in {
        val res = TestChargeHistoryService.chargeHistoryResponse(
          isLatePaymentCharge = true,
          isPayeSelfAssessment = true,
          None,
          isChargeHistoryEnabled = false
        )
      ...
      "the chargeHistory has no details" in {
        ...
        val res = TestChargeHistoryService.chargeHistoryResponse(
          ...
          isPayeSelfAssessment = false,
          None,
          isChargeHistoryEnabled = true
        )
