...
class AddressLookupServiceSpec extends TestSupport with FeatureSwitching {

  case class AddressError(status: String) extends RuntimeException

  val testBusinessAddressModel: BusinessAddressModel =
    BusinessAddressModel("auditRef", Address(Seq("Line 1", "Line 2"), Some("AA1 1AA")))

  val mockAddressLookupConnector: AddressLookupConnector = mock(classOf[AddressLookupConnector])

  object TestAddressLookupService
  ...
  "AddressLookupService" should {
    "initialiseAddressJourney" should {
      "return an error when connector lookup fails" in {
        ...
        val result: Future[Either[Throwable, Option[String]]] =
          TestAddressLookupService.initialiseAddressJourney(isAgent = false, isChange = false)
        result map {
    ...
    "initialiseAddressJourney on change page" should {
      "return an error when connector lookup fails and isChange = true" in {
        ...
        val result: Future[Either[Throwable, Option[String]]] =
          TestAddressLookupService.initialiseAddressJourney(isAgent = false, isChange = true)
        result map {
          case Left(AddressError(value)) => value shouldBe "status: 418"
    ...
    "fetchAddress" should {
      "return an error when getting address details fails" in {
        ...
        val result: Future[Either[Throwable, BusinessAddressModel]] =
          TestAddressLookupService.fetchAddress(Some(mkIncomeSourceId("")))
        result map {
          case Left(AddressError(value)) => value shouldBe "status: 418"
          case Right(_)                  => Fail("Error not returned")
          case Left(_)                   => Fail("Wrong error type returned")
