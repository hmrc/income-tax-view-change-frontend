...
class HandleYourTasksServiceSpec extends TestSupport {
  ...
  val overdueCharge = ChargeItem("100", fixedTaxYear, BalancingCharge, None, LocalDate.of(2018, 3, 29), Some(fixedDate.minusYears(3)),10000,1000,Some(10),None,None,Some(LocalDate.of(2020, 8, 1)),Some(LocalDate.of(2021, 7, 21)),Some(3),None,None,false,None,Some(LocalDate.of(2020, 8, 13)),None,None,None,Some("AA000000000001"), None)
  ...
  val upcomingChargeCard: UpcomingTaskCard = UpcomingTaskCard("newHome.yourTasks.selfAssessment.upcomingCharge", "newHome.yourTasks.selfAssessment", "/report-quarterly/income-and-expenses/view/what-you-owe", "newHome.yourTasks.selfAssessment.upcomingCharge.label", Some(fixedDate.plusYears(100)), None, Upcoming, FINANCIALS)
  ...
  "getYourTasksCards" should {
    "return overdue task cards" when {
      "there's an overdue charge" in new TestSetup(chargeItemList = List(overdueCharge)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueChargeCard), Seq.empty, Seq.empty)
      }
      "there's a late payment penalty" in new TestSetup(chargeItemList = List(overdueLatePaymentPenalty)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueLppCard), Seq.empty, Seq.empty)
      }
      "there's a late submission penalty" in new TestSetup(chargeItemList = List(overdueLateSubmissionPenalty)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueLspCard), Seq.empty, Seq.empty)
      }
      "there's multiple late payment penalties" in new TestSetup(chargeItemList = List(overdueLatePaymentPenalty, overdueLatePaymentPenalty)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueMultipleLppCard), Seq.empty, Seq.empty)
      }
      "there's multiple late submission penalties" in new TestSetup(chargeItemList = List(overdueLateSubmissionPenalty, overdueLateSubmissionPenalty)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueMultipleLspCard), Seq.empty, Seq.empty)
      }
      "there's multiple charges" in new TestSetup(chargeItemList = List(overdueCharge, overdueCharge)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueMultipleChargeCard), Seq.empty, Seq.empty)
      }
      ...
      "there's an overdue annual submission" in new TestSetup(obligations = Seq(singleObligation(fixedDate.minusYears(2), isQuarterly = false))) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueAnnualSubmissionCard), Seq.empty, Seq.empty)
      }
      ...
      "there's an overdue quarterly submission" in new TestSetup(obligations = Seq(singleObligation(fixedDate.minusYears(2), isQuarterly = true)), currentItsaStatus = ITSAStatus.Mandated) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueQuarterlySubmissionCard), Seq.empty, Seq.empty)
      }
      ...
      "there's multiple overdue annual submissions" in new TestSetup(obligations = Seq(singleObligation(fixedDate.minusYears(2), isQuarterly = false), singleObligation(fixedDate.minusYears(3), isQuarterly = false))) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueMultipleAnnualSubmissionsCard), Seq.empty, Seq.empty)
      }
      ...
      "there's multiple overdue quarterly submissions" in new TestSetup(obligations = Seq(singleObligation(fixedDate.minusYears(2), isQuarterly = true), singleObligation(fixedDate.minusYears(3), isQuarterly = true)), currentItsaStatus = ITSAStatus.Mandated) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueMultipleQuarterlySubmissionsCard), Seq.empty, Seq.empty)
      }
    }
    ...
    "return a dateless task card" when {
      "there is money in a user's account with creditsRefundsRepay enabled" in new TestSetup(creditsRefundsRepayEnabled = true, credits = creditsModel(1000)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq.empty, Seq(moneyInYourAccountTask), Seq.empty)
      }
    }
    ...
    "return the correct upcoming task cards" when {
      "there is a future annual submission" in new TestSetup(hasUpcomingAnnualDate = true, obligations = Seq(singleObligation(fixedDate.plusYears(2), isQuarterly = false))) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq.empty, Seq.empty, Seq(upcomingAnnualSubmissionsTaskCard))
      }
      ...
      "there is a future quarterly submission and the user's ITSA status is mandated" in new TestSetup(hasUpcomingQuarterlyDate = true, currentItsaStatus = ITSAStatus.Mandated, obligations = Seq(singleObligation(fixedDate.plusYears(2), isQuarterly = true))) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq.empty, Seq.empty, Seq(upcomingQuarterlySubmissionsTaskCard))
      }
      ...
      "there is upcoming payments" in new TestSetup(chargeItemList = List(upcomingCharge)) {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq.empty, Seq.empty, Seq(upcomingChargeCard))
      }
    }
    "return the no task card" when {
      "there are no overdue, dateless or upcoming tasks" in new TestSetup() {
        resultViewModel shouldBe HandleYourTasksViewModel(Seq.empty, Seq.empty, Seq.empty)
      }
    }
    ...
    "put the cards in the correct order" when {
      "an overdue submissions card and an overdue penalty card have the same due date (Finacials first then Penalties and then Submissions)" in new TestSetup(chargeItemList = List(overdueCharge, overdueLateSubmissionPenalty.copy(dueDate = Some(fixedDate.minusYears(3)))), obligations = Seq(singleObligation(fixedDate.minusYears(3), isQuarterly = false))){
        resultViewModel shouldBe HandleYourTasksViewModel(Seq(overdueChargeCard, overdueLspCard.copy(dueDate = Some(fixedDate.minusYears(3))), overdueAnnualSubmissionCard.copy(dueDate = Some(fixedDate.minusYears(3)))), Seq.empty, Seq.empty)
      }
    }
  }
}
