...
package hub.v1.views.agent

import common.testUtils.ViewSpec
import hub.v1.views.html.agent.ConfirmClientUTRView
import play.api.mvc.Call


class ConfirmClientViewSpec extends ViewSpec {

  lazy val postAction: Call = hub.v1.controllers.agent.routes.ConfirmClientUTRController.submit()
  lazy val backUrl: String = hub.v1.controllers.agent.routes.EnterClientsUTRController.show().url
  ...
  "the Confirm Client page" should {
    ...
    s"have a ${confirmClientMessages.changeClient} link" in new Setup(confirmClientView()) {
      layoutContent.hasCorrectHref(hub.v1.controllers.agent.routes.EnterClientsUTRController.show().url)
    }
