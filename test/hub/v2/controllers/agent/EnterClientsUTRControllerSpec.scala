...
import hub.v2.mocks.agent.MockEnterClientsUTR
import hub.v2.views.html.agent.EnterClientsUTRView
    ...
    "the user is not authenticated" should {
      ...
      "redirect to the UTR Error page" when {
        "a client details not found error is returned from the client lookup" in {
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe Some(hub.v2.controllers.agent.routes.UTRErrorController.show().url)
        }
        ...
        "client details exist but there is no agent/client relationship" in {
          ...
          whenReady(testEnterClientsUTRController.submit(fakePostRequestWithActiveSession.withFormUrlEncodedBody(ClientsUTRForm.utr -> validUTR))) { result =>
            ...
            result.header.headers.get(LOCATION) shouldBe Some(hub.v2.controllers.agent.routes.UTRErrorController.show().url)
          }
        }
      }
