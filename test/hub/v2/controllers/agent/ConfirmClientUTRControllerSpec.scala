...
import errors.routes as agentErrorRoutes
...
import hub.v2.mocks.agent.MockConfirmClient
    ...
    s"there is a user" that {
      ...
      s"does not have client details in session" should {
        "redirect to the Enter Client UTR page" in {
          ...
          status(result) shouldBe SEE_OTHER
          redirectLocation(result) shouldBe Some(hub.v2.controllers.agent.routes.EnterClientsUTRController.show().url)
        }
      }
      ...
      s"there is a $agentType user" that {
        ...
        "is fully authenticated" should {
          "redirect to Home page and relevant data added to session or sent to session data service successfully" in {
            ...
            status(result) shouldBe SEE_OTHER
            redirectLocation(result) shouldBe Some(hub.v2.controllers.routes.HomeController.showAgent().url)
