...
package hub.v2.views.agent

import common.testUtils.ViewSpec
import hub.v2.views.html.agent.ConfirmClientUTRView
import play.api.mvc.Call


class ConfirmClientViewSpec extends ViewSpec {

  lazy val postAction: Call = hub.v2.controllers.agent.routes.ConfirmClientUTRController.submit()
  lazy val backUrl: String = hub.v2.controllers.agent.routes.EnterClientsUTRController.show().url
  ...
  "the Confirm Client page" should {
    ...
    s"have a ${confirmClientMessages.changeClient} link" in new Setup(confirmClientView()) {
      layoutContent.hasCorrectHref(hub.v2.controllers.agent.routes.EnterClientsUTRController.show().url)
    }
