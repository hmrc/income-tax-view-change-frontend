...
import _root_.financialDetails.models.PoaTwoReconciliationCredit
import _root_.financialDetails.models.chargeHistory.{AdjustmentHistoryModel, AdjustmentModel}
import _root_.financialDetails.models.repaymentHistory.RepaymentHistoryUtils
import _root_.financialDetails.services.{ChargeHistoryService, FinancialDetailsService, PaymentAllocationsService}
import financialDetails.controllers.errors.routes as errorRoutes
import financialDetails.controllers.agent.errors.routes as agentErrorRoutes
          ...
          "Redirect the user to NotFoundDocumentIDLookup" when {
            "the charge id provided does not match any charges in the response" in new Setup(financialDetailsModel(), docId = "fakeId") {
              ...
              val notFoundDocumentIDUrl = if (isAgent) {
                agentErrorRoutes.AgentNotFoundDocumentIDLookupController.show().url
              } else {
                errorRoutes.NotFoundDocumentIDLookupController.show().url
              }
