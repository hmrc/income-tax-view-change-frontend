...
import financialDetails.CheckYourAnswersController
        ...
        if (mtdRole == MTDSupportingAgent) {
          ...
          s"redirect to POA adjusted page" when {
            "data to API 1773 successfully sent" in {
              ...
              val result = action(fakeRequest)
              redirectLocation(result) shouldBe Some(routes.PoaAdjustedController.show(isAgent).url)
            }
          }
          "redirect to API error page" when {
            "data to API 1773 failed to be sent" in {
              ...
              val result = action(fakeRequest)
              redirectLocation(result) shouldBe Some(routes.ApiFailureSubmittingPoaController.show(isAgent).url)
            }
          }
