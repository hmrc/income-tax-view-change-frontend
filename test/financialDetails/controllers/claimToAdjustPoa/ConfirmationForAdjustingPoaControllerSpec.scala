...
import financialDetails.controllers.claimToAdjustPoa.ConfirmationForAdjustingPoaController
import financialDetails.models.claimToAdjustPoa.{MainIncomeLower, PoaAmendmentData}
import financialDetails.services.PaymentOnAccountSessionService
import financialDetails.services.claimToAdjustPoa.{ClaimToAdjustPoaCalculationService, ClaimToAdjustService}
        ...
        if (mtdRole == MTDSupportingAgent) {
          ...
          "redirect to the You Cannot Go Back page" when {
            "the journeyCompleted flag is set to true in session" in {
              ...
              redirectLocation(result) shouldBe Some(routes.YouCannotGoBackController.show(isAgent).url)
            }
          }
        ...
        if (mtdRole == MTDSupportingAgent) {
          ...
          "redirect to PoaAdjustedController page" when {
            "data to API 1773 successfully sent" in {
              ...
              val result = action(fakeRequest)
              redirectLocation(result) shouldBe Some(routes.PoaAdjustedController.show(isAgent = isAgent).url)
            }
          }
