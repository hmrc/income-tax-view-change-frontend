...
package financialDetails.models.creditsandrefunds

import financialDetails.models.creditsandrefunds.*
...
class MoneyInYourAccountViewModelSpec extends UnitSpec {
  ...
  "CreditViewRow" should {
    s"have the correct description link url" in {
      ...
      chargeSummaryCredits.foreach { creditRow =>
        creditRow.descriptionLink(false) shouldBe financialDetailsRoutes.ChargeSummaryController.show(creditTaxYear, creditId).url
        creditRow.descriptionLink(true) shouldBe financialDetailsRoutes.ChargeSummaryController.showAgent(creditTaxYear, creditId).url
      }

      creditSummaryCredits.foreach { creditRow =>
        creditRow.descriptionLink(false) shouldBe financialDetailsRoutes.CreditsSummaryController.showCreditsSummary(creditTaxYear).url
        creditRow.descriptionLink(true) shouldBe financialDetailsRoutes.CreditsSummaryController.showAgentCreditsSummary(creditTaxYear).url
      }
    }
  }
  ...
  "PaymentCreditRow" should {
    ...
    "have the correct description link url" in {
      val paymentRow = PaymentCreditRow("payment", 20.0, dateInYear(2024), dateInYear(2024))
      paymentRow.descriptionLink(false) shouldBe financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocation("payment").url
      paymentRow.descriptionLink(true) shouldBe financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocationAgent("payment").url
    }
  }
  "RefundRow" should {
    "have the correct description link url" in {
      val refundRow = RefundRow(30.0, dateInYear(2024))
      refundRow.descriptionLink shouldBe financialDetailsRoutes.PaymentHistoryController.refundStatus().url
    }
  }
