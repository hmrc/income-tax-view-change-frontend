...
class TaxYearSummaryViewSpec extends ViewSpec with FeatureSwitching with ChargeConstants {
  ...
  def multipleDunningLockView(isAgent: Boolean = false): Html = taxYearSummaryView(
    ...
    viewModel = TaxYearSummaryViewModel(Some(modelComplete(crystallised = false)), None, testDunningLockChargesList,
      testObligationsModel, ctaViewModel = emptyCTAModel, LPP2Url = "", pfaEnabled = false, financialsFrontendEnabled = false),
    backUrl = "testBackURL",
    isAgent = isAgent,
    ctaLink = ctaLink,
    taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def forecastWithNoCalcData(isAgent: Boolean = false): Html =
    taxYearSummaryView(
      ...
      viewModel = TaxYearSummaryViewModel(None, None, testChargesList, testObligationsModel, showForecastData = true,
        ctaViewModel = emptyCTAModel, LPP2Url = "", pfaEnabled = false, financialsFrontendEnabled = false),
      backUrl = "testBackURL",
      isAgent = isAgent,
      ctaLink = ctaLink,
      taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def poaView(isAgent: Boolean = false): Html = {
    ...
    taxYearSummaryView(
      taxYear = testYear,
      viewModel = TaxYearSummaryViewModel(Some(modelWithErrorMessages), None, testChargesList, testObligationsModel,
        ctaViewModel = testCTAModel, LPP2Url = "", pfaEnabled = false, financialsFrontendEnabled = false),
      backUrl = "testBackURL",
      isAgent = isAgent,
      ctaLink = ctaLink,
      taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def calculationWithLatestAmendmentsView(isAgent: Boolean): HtmlFormat.Appendable = taxYearSummaryView(
    ...
    viewModel = TaxYearSummaryViewModel(Some(modelComplete(crystallised = false, isAmended = true)), None,
      List.empty, testObligationsModel, ctaViewModel = emptyCTAModel, LPP2Url = "", pfaEnabled = true, financialsFrontendEnabled = false),
    backUrl = "testBackURL",
    isAgent = isAgent,
    ctaLink = ctaLink,
    taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def calculationWithLatestAndPreviousAmendmentsView(isAgent: Boolean): HtmlFormat.Appendable =
    taxYearSummaryView(
      ...
      viewModel = TaxYearSummaryViewModel(Some(modelComplete(crystallised = false, isAmended = true)),
        ...
        ctaViewModel = emptyCTAModel, LPP2Url = "", pfaEnabled = true, financialsFrontendEnabled = false),
      backUrl = "testBackURL",
      isAgent = isAgent,
      ctaLink = ctaLink,
      taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def calculationWithNoAmendmentPfaEnabled(isAgent: Boolean): HtmlFormat.Appendable =
    taxYearSummaryView(
      taxYear = testYear,
      viewModel = TaxYearSummaryViewModel(Some(modelComplete(crystallised = false)), None, List.empty,
        testObligationsModel, ctaViewModel = emptyCTAModel, LPP2Url = "", pfaEnabled = true, financialsFrontendEnabled = false),
      backUrl = "testBackURL",
      isAgent = isAgent,
      ctaLink = ctaLink,
      taxYearViewScenarios = MtdSoftwareShowCalc,
  ...
  def crystallisedNoAmendmentPfaEnabled(isAgent: Boolean): HtmlFormat.Appendable =
    taxYearSummaryView(
      ...
      viewModel = TaxYearSummaryViewModel(
        ...
        LPP2Url = "",
        pfaEnabled = true, financialsFrontendEnabled = false
      ),
      backUrl = "testBackUrl",
      isAgent = isAgent,
      ctaLink = ctaLink,
  ...
  "taxYearSummary" when {
    "the user is an individual" should {
      ...
      "display the payment type as a link to Charge Summary in the Payments tab" in new Setup(estimateView(chargeItems = testChargesWithoutLpiList)) {
        ...
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryIndividualUrl(testYear, fullDocumentDetailModel.transactionId, false, None, financialsFrontendEnabled = false)
      }
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest POA1" in new Setup(estimateView()) {
        ...
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryIndividualUrl(testYear,
          fullDocumentDetailModel.transactionId, true, financialsFrontendEnabled = false)
      }
      ...
      "display the Class 2 National Insurance payment link on the payments table when coding out is enabled" in new Setup(class2NicsView()) {
        ...
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryIndividualUrl(testYear,
          fullDocumentDetailModel.transactionId, false, financialsFrontendEnabled = false)
      }
    ...
    "the user is an agent" should {
      ...
      "display the Income Tax and National Insurance Contributions Due row in the Tax Calculation tab" in new Setup(estimateView(isAgent = true)) {
        ...
        totalTaxDueLink.attr("href") shouldBe returns.controllers.routes.TaxDueSummaryController.showTaxDueSummaryAgent(testYear).url
        layoutContent.selectHead("#calculation-income-deductions-contributions-table tr:nth-child(4) td:nth-child(2)").text shouldBe modelComplete(crystallised = false).taxDue.toCurrencyString
      ...
      "display the payment type as a link to Charge Summary in the Payments tab" in new Setup(estimateView(chargeItems = testChargesWithoutLpiList, isAgent = true)) {
        ...
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryAgentUrl(testYear, fullDocumentDetailModel.transactionId,
          false, financialsFrontendEnabled = false)
      }
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest POA1" in new Setup(estimateView(isAgent = true)) {
        ...
        paymentTypeLink.text shouldBe lpiPaymentOnAccount1
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryAgentUrl(testYear, fullDocumentDetailModel.transactionId,
          true, financialsFrontendEnabled = false)
      }
      ...
      "display MFA Debits - Individual" in new Setup(mfaDebitsView(isAgent = false)) {
        ...
        paymentTypeLink.attr("href") shouldBe appConfig.financialsChargeSummaryIndividualUrl(testYear,
          MFADebitsDocumentsTransactionIds.head, false, financialsFrontendEnabled = false)
