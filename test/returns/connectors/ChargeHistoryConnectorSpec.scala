...
import returns.models.chargeHistory.ChargesHistoryResponse.ChargesHistoryResponse
import returns.models.chargeHistory.{ChargeHistoryDetailsModel, ChargeHistoryResponseModel, ChargesHistoryErrorModel, ChargesHistoryModel}
import returns.testConstants.ChargeHistoryTestConstants.{testChargeHistoryErrorModelParsing, testValidChargeHistoryModel}
...
class ChargeHistoryConnectorSpec extends TestSupport with MockHttpV2 with MockAuditingService {
  ...
  "getChargeHistory" should {
    ...
    val testChargeHistoryModelSuccess = ChargeHistoryDetailsModel(taxYear = "2017", documentId = "123456789", documentDate = LocalDate.of(2020, 1, 29),
      documentDescription = "Balancing Charge", totalAmount = 123456789012345.67,
      reversalDate = LocalDateTime.of(LocalDate.of(2020, 2, 24), LocalTime.of(9, 30, 45)), reversalReason = "amended return", poaAdjustmentReason = Some("005"), None)
