...
class RoutesSpec extends TestSupport {
  ...
  "The URL for the NextUpdatesController.getNextObligation action" should {
    s"be equal to $contextRoute/submission-deadlines" in {
      obligations.controllers.routes.NextUpdatesController.show().url shouldBe s"$contextRoute/submission-deadlines"
    }
  }
  ...
  "The URL for the CalculationController.renderCalculationPage(year) action" should {
    s"be equal to $contextRoute/calculation/2018" in {
      controllers.routes.TaxYearSummaryController.renderTaxYearSummaryPage(2018).url shouldBe s"$contextRoute/tax-year-summary/2018"
    }
  }
