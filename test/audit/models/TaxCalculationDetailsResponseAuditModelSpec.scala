...
class TaxCalculationDetailsResponseAuditModelSpec extends AnyWordSpecLike with Matchers {
  ...
  def taxCalculationDetailsResponseAuditModelMinimal(
      userType: Option[AffinityGroup] = Some(Individual)
    ): TaxDueResponseAuditModel =
    TaxDueResponseAuditModel(
  ...
  val taxCalcDetailsResponseAuditModelDetailJsonFull: JsObject =
    commonAuditDetails(Individual) ++ Json.obj(
      ...
      "capitalGainsTax" -> Json.obj(
        ...
        "rates" -> Json.arr(
          Json.obj(
            "rateBand" -> "Business Asset Disposal Relief and or Investors' Relief gains (£5,000.99 at 20%)",
            "amount"   -> 5000.99
          ),
          Json.obj(
            "rateBand" -> "Residential property and carried interest basic rate (£5,000.99 at 20%)",
            "amount"   -> 5000.99
          ),
          Json.obj(
            "rateBand" -> "Residential property and carried interest lowerRate2 (£5,000.99 at 21%)",
            "amount"   -> 5000.99
          ),
          Json.obj("rateBand" -> "Other gains basic rate (£5,000.99 at 20%)", "amount" -> 5000.99),
          Json.obj("rateBand" -> "Other gains lowerRate2 (£5,000.99 at 21%)", "amount" -> 5000.99)
        )
      ),
