...
class TaxYearJsonSummaryResponseAuditModelSpec extends AnyWordSpecLike with TestSupport with ImplicitDateParser {
  ...
  def taxYearOverviewResponseAuditFull(userType: Option[AffinityGroup] = Some(Agent),
    ...
    TaxYearSummaryResponseAuditModel(
      mtdItUser = MtdItUser(
        ...
        incomeSources = IncomeSourceDetailsModel("mtditid", None, business, Nil),
        btaNavPartial = None,
        saUtr = Some("saUtr"),
        credId = Some("credId"),
        userType = userType,
