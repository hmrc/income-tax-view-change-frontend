...
class ForecastIncomeAuditModelSpec extends TestSupport {
  ...
  val testUserIndividual =
    defaultMTDITUser(Some(Individual), IncomeSourceDetailsModel(testNino, testMtditid, None, Nil, Nil))

  val testUserAgent = defaultMTDITUser(Some(Agent), IncomeSourceDetailsModel(testNino, testMtditid, None, Nil, Nil))

  def testForecastIncomeAuditModelIndividual(
      endOfYearEstimate: EndOfYearEstimate = endOfYearEstimate
    ): ForecastIncomeAuditModel =
    ForecastIncomeAuditModel(
      user = testUserIndividual,
      endOfYearEstimate = endOfYearEstimate
    )

  def testForecastIncomeAuditModelAgent(
      endOfYearEstimate: EndOfYearEstimate = endOfYearEstimate
    ): ForecastIncomeAuditModel =
    ForecastIncomeAuditModel(
      user = testUserAgent,
