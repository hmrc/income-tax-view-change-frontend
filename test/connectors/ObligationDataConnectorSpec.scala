...
class ObligationDataConnectorSpec extends TestSupport with MockHttp {
  ...
  "EstimatedTaxLiabilityConnector.getEstimatedTaxLiability" should {
    ...
    "return a SuccessResponse with JSON in case of sucess" in {
      setupMockHttpGet(TestObligationDataConnector.getObligationDataUrl(testNino))(successResponse)
      val result = TestObligationDataConnector.getObligationData(testNino)
      await(result) shouldBe SuccessResponse(Json.parse("{}"))
    }

    "return ErrorResponse model in case of failure" in {
      setupMockHttpGet(TestObligationDataConnector.getObligationDataUrl(testNino))(badResponse)
      val result = TestObligationDataConnector.getObligationData(testNino)
