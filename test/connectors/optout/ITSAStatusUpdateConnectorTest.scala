  ...
  "For OptOutConnector.requestOptOutForTaxYear " when {
    ...
    "happy case" should {
      ...
      "return successful response" in {
        ...
        val apiRequest = OptOutUpdateRequest(connector.toFormat(taxYear), optOutUpdateReason)
  ...
  "For OptOutConnector.taxYearToFormat " when {
    "happy case" should {
      "match required format" in {
        connector.toFormat(TaxYear.forYearEnd(2024)) shouldBe "2023-24"
      }
    }

    "unhappy case" should {
      "default format not match required format" in {
        connector.toFormat(TaxYear.forYearEnd(2024)) should not be TaxYear.forYearEnd(2024).toString
      }
    }
