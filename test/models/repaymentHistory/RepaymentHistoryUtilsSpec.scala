...
class RepaymentHistoryUtilsSpec extends TestSupport with Matchers {
  ...
  val payments: List[Payment] = List(
    Payment(
      ...
      Some(-150.00),
      method = Some("method"),
      documentDescription = Some("mfa1"),
      lot = None,
      lotItem = None,
      dueDate = None,
      documentDate = LocalDate.parse("2020-04-13"),
      Some("AY777777202201"),
    ...
    Payment(
      reference = Some("mfa2"),
      amount = Some(-11000.00),
      Some(-150.00),
      method = Some("method"),
      documentDescription = Some("mfa2"),
      lot = None,
      lotItem = None,
      dueDate = None,
      documentDate = LocalDate.parse("2020-04-12"),
      Some("AY777777202210"),
      mainType = Some("ITSA Overpayment Relief"),
      mainTransaction = Some("4004")
    ),
    Payment(
      ...
      Some(-150.00),
      method = Some("method"),
      documentDescription = Some("desc1"),
      lot = None,
      lotItem = None,
      dueDate = Some(LocalDate.parse("2019-12-25")),
      documentDate = LocalDate.parse("2019-04-25"),
      Some("AY777777202202"),
    ...
    Payment(
      ...
      Some("lot"),
      Some("lotitem"),
      Some(LocalDate.parse("2019-12-26")),
    ...
    Payment(
      ...
      Some("lot"),
      Some("lotitem"),
      Some(LocalDate.parse("2019-12-25")),
      LocalDate.parse("2019-12-25"),
      Some("DOCID02"),
      mainType = Some("Payment"),
      mainTransaction = Some("0060")
    ...
    Payment(
      reference = Some("bcc1"),
      amount = Some(-12000.00),
      Some(-150.00),
      method = Some("method"),
      documentDescription = Some("bcc1"),
      lot = None,
      lotItem = None,
      dueDate = Some(LocalDate.parse("2019-12-24")),
      documentDate = LocalDate.parse("2019-04-24"),
      Some("AY777777202203"),
      mainType = Some("SA Balancing Charge Credit"),
      mainTransaction = Some("4905")
    ),
    Payment(
      reference = Some("rar-poa1"),
      amount = Some(-11000.00),
      Some(-150.00),
      method = Some("method"),
      documentDescription = Some("desc1"),
      lot = None,
      lotItem = None,
      dueDate = Some(LocalDate.parse("2019-12-23")),
      documentDate = LocalDate.parse("2019-04-20"),
      Some("AY777777202298"),
      mainType = Some("SA POA 1 Reconciliation Credit"),
      mainTransaction = Some("4912")
    ),
    Payment(
      reference = Some("rar-poa1"),
      amount = Some(-11000.00),
      ...
      documentDate = LocalDate.parse("2019-04-20"),
      Some("AY777777202299"),
      mainType = Some("SA POA 2 Reconciliation Credit"),
      mainTransaction = Some("4914")
    )
  )
  ...
  private def groupedRepayments(isAgent: Boolean = false) =
    List(
      (
        2021,
        List(
          PaymentHistoryEntry(
            ...
            Some(301.0),
            None,
            s"${if (isAgent) "agents/" else ""}refund-to-taxpayer/$repaymentRequestNumber",
            repaymentRequestNumber
          ),
          PaymentHistoryEntry(
            LocalDate.parse("2021-08-21"),
            Repayment,
            Some(300.0),
            None,
            s"${if (isAgent) "agents/" else ""}refund-to-taxpayer/$repaymentRequestNumber",
  ...
  private def groupedPayments(
      cutoverEnabled:            Boolean = true,
      mfaEnabled:                Boolean = true,
      reviewAndReconcileEnabled: Boolean = true,
      isAgent:                   Boolean = false
    ) = {
    val bcc = List(
      PaymentHistoryEntry(
        LocalDate.parse("2019-12-24"),
        BalancingChargeCreditType,
        Some(-12000.0),
        None,
        s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}credits-from-hmrc/2019",
        "AY777777202203"
      )
    )
    val cutover =
    ...
    val reviewAndReconcileCredits =
      if (reviewAndReconcileEnabled)
        List(
          PaymentHistoryEntry(
            LocalDate.parse("2019-12-23"),
            PoaOneReconciliationCredit,
            Some(-11000.0),
            None,
            s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}tax-years/2019/charge?id=AY777777202298",
            "AY777777202298"
          ),
          PaymentHistoryEntry(
            LocalDate.parse("2019-12-23"),
            ...
            Some(-11000.0),
            None,
            s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}tax-years/2019/charge?id=AY777777202299",
            "AY777777202299"
          )
        )
      else Nil
    val standardPayments = List(
      PaymentHistoryEntry(
        ...
        Some(10000),
        Some("DOCID02"),
        s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}payment-made-to-hmrc?documentNumber=DOCID02",
        "2019-12-25 &pound;10,000.00"
      ),
      PaymentHistoryEntry(
        ...
        Some(10000),
        Some("DOCID01"),
        s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}payment-made-to-hmrc?documentNumber=DOCID01",
        "2019-12-26 &pound;10,000.00"
      )
    )
    val mfa =
      if (mfaEnabled)
          ...
          (
            ...
            List(
              PaymentHistoryEntry(
                ...
                Some(-11000.0),
                None,
                s"/report-quarterly/income-and-expenses/view/${if (isAgent) "agents/" else ""}credits-from-hmrc/2020",
                "AY777777202210"
              ),
              PaymentHistoryEntry(
