...
object RepaymentHistoryAggSpec extends Properties("RepaymentHistory_aggregate") {
  ...
  def repaymentSupplementItemGen: Gen[RepaymentSupplementItem] =
    for {
      from    <- dateGen
      addDays <- Gen.choose(30, 90)
      amount  <- Gen.choose(100, 99999)
      rate    <- Gen.choose(1, 200)
    } yield RepaymentSupplementItem(
      parentCreditReference = None,
  ...
  def repaymentHistoryGen: Gen[RepaymentHistory] =
    for {
  ...
  property("aggregate") = forAll(repaymentHistoryGen) { repaymentHistoryRecord =>
    ...
    val minDateOpt = repaymentHistoryRecord.repaymentItems.get.headOption
      .map(
        _.repaymentSupplementItem
          .map(_.fromDate)
          .collect {
    ...
    val maxDateOpt = repaymentHistoryRecord.repaymentItems.get.headOption
      .map(
        _.repaymentSupplementItem
          .map(_.toDate)
          .collect {
    ...
    val minRateOpt = repaymentHistoryRecord.repaymentItems.get.headOption
      .map(
        _.repaymentSupplementItem
          .map(_.rate)
          .collect {
    ...
    val maxRateOpt = repaymentHistoryRecord.repaymentItems.get.headOption
      .map(
        _.repaymentSupplementItem
          .map(_.rate)
          .collect {
    ...
    val totalOpt = repaymentHistoryRecord.repaymentItems.get.headOption
      .map(
        _.repaymentSupplementItem
          .map(_.amount)
          .collect {
