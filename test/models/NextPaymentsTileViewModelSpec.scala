...
class NextPaymentsTileViewModelSpec extends UnitSpec with Matchers{
  ...
  val nextPaymentsTileViewModel: NextPaymentsTileViewModel = NextPaymentsTileViewModel(mockDate, 1, 0, false, false)
  ...
  "The verify method" when {
    "overdue payment exists" should {
      "create the NextPaymentsTileViewModel" in {
        NextPaymentsTileViewModel(mockDate, 1, 0, false, false).verify shouldBe Right(nextPaymentsTileViewModel)
      }
    }

    "overdue payment doesn't exist" should {
      "return an error" in {
        NextPaymentsTileViewModel(None, 1, 0, false, false).verify.isLeft shouldBe true
      }
    }
  }
