...
class SessionDataGetResponseSpec extends UnitSpec {
  ...
  val jsonValidationError: Seq[(String, Seq[JsonValidationError])] = Seq(
    "/nino" -> Seq(JsonValidationError("error.path.missing"))
  )
  ...
  "SessionDataGetResponseRead.read" should {
    ...
    "response an OK status and return SessionDataUnexpectedResponse when unable to parse successfully" in {
      ...
      result shouldBe Left(
        SessionDataUnexpectedResponse(s"Json validation error for SessionDataModel. Invalid: $jsonValidationError")
      )
    }
