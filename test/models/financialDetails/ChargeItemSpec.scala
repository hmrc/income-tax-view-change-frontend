...
class ChargeItemSpec extends UnitSpec with ChargeConstants  {
  ...
  "fromDocumentPair" when {
      ...
      "from Payment on Account 1" in {
        ...
        chargeItem.codingOutStatus shouldBe None
      }
      ...
      "from Balancing Payment Nics2" in {
        ...
        chargeItem.codingOutStatus shouldBe Some(Nics2)
      }
      ...
      "from Balancing Payment Accepted" in {
        ...
        chargeItem.transactionType shouldBe BalancingCharge
        chargeItem.codingOutStatus shouldBe Some(Accepted)
      }
      ...
      "from Balancing Payment Cancelled" in {
        ...
        chargeItem.transactionType shouldBe BalancingCharge
        chargeItem.codingOutStatus shouldBe Some(Cancelled)
      }
    ...
    "getChargeKey" when {
        ...
        "charge is a POA 1" in {
          val poa1 = chargeItemModel(transactionType = PoaOneDebit, codingOutStatus = None)
          val key = poa1.getChargeTypeKey
          key shouldBe "paymentOnAccount1.text"
        }

        "charge is a POA 2" in {
          val poa1 = chargeItemModel(transactionType = PoaTwoDebit, codingOutStatus = None)
          val key = poa1.getChargeTypeKey
        ...
        "charge is a HMRC adjustment" in {
          val poa1 = chargeItemModel(transactionType = MfaDebitCharge, codingOutStatus = None)
          val key = poa1.getChargeTypeKey
          key shouldBe "hmrcAdjustment.text"
        }

        "charge is a Class 2 National Insurance Balancing Charge" in {
          val poa1 = chargeItemModel(transactionType = BalancingCharge, codingOutStatus = Some(Nics2))
          val key = poa1.getChargeTypeKey
          key shouldBe "class2Nic.text"
        }

        "charge is a PAYE payment" in {
          val poa1 = chargeItemModel(transactionType = BalancingCharge, codingOutStatus = Some(Accepted))
          val key = poa1.getChargeTypeKey
          key shouldBe "codingOut.text"
        }

        "charge is a cancelled PAYE SA payment" in {
          val poa1 = chargeItemModel(transactionType = BalancingCharge, codingOutStatus = Some(Cancelled))
          val key = poa1.getChargeTypeKey
          key shouldBe "cancelledPayeSelfAssessment.text"
        }

        "charge is a balancing charge" in {
          val poa1 = chargeItemModel(transactionType = BalancingCharge, codingOutStatus = None)
          val key = poa1.getChargeTypeKey
