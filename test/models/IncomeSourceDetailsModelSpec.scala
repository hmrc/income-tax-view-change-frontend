    ...
    "the sanitise method" should {
      "remove all unnecessary fields" in {
        val expected = IncomeSourceDetailsModel(
          ...
          List(
            BusinessDetailsModel(None, None, None, None, None),
            BusinessDetailsModel(None, None, None, None, Some(LocalDate.of(getCurrentTaxEndYear(LocalDate.now) - 1, Month.APRIL, 5)))
          ),
          Some(PropertyDetailsModel(None, None, None, None))
        )
