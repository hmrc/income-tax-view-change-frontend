...
import _root_.financialDetails.models.chargeHistory.{AdjustmentHistoryModel, AdjustmentModel, ChargeHistoryModel}
import _root_.financialDetails.models.chargeSummary.{ChargeSummaryViewModel, PaymentHistoryAllocation, PaymentHistoryAllocations}
import _root_.financialDetails.models.*
import _root_.financialDetails.views.html.ChargeSummaryView
...
class ChargeSummaryViewSpec extends ViewSpec with FeatureSwitching with ChargeConstants {
  ...
  val whatYouOweAgentUrl: String = financialDetailsRoutes.WhatYouOweController.showAgent().url
  ...
  "user is an individual" when {
    ...
    "render the row for the charge" should {
      "charge is a Review and Reconcile credit for Payment on Account 1" in new TestSetup(
        ...
        document.selectById("rar-charge-link").attr("href") shouldBe financialDetailsRoutes.ChargeSummaryController.show(2020, "some-id").url
      ...
      "show payment allocations in history table" when {
        ...
        "allocations are present in the list" when {
          ...
          "chargeHistory enabled with a matching link to the payment allocations page" in new TestSetup(chargeItem = chargeItemModel(),
            ...
            document.select(Selectors.table).select("a").forall(_.attr("href") == financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocation("PAYID01").url) shouldBe true
          }
  ...
  "agent" when {
    ...
    "render the row for the charge" should {
      "charge is a Review and Reconcile credit for Payment on Account 1" in new TestSetup(
        ...
        document.selectById("rar-charge-link").text() shouldBe "First payment on account: credit from your tax return"
        document.selectById("rar-charge-link").attr("href") shouldBe financialDetailsRoutes.ChargeSummaryController.showAgent(2020, "some-id").url
    ...
    "The charge summary view" should {
      ...
      "list payment allocations with right number of rows and agent payment allocations link" in new TestSetup(chargeItem = chargeItemModel(),
        ...
        document.select(Selectors.table).select("a").forall(_.attr("href") == financialDetailsRoutes.PaymentAllocationsController.viewPaymentAllocationAgent("PAYID01").url) shouldBe true
      }
    }
