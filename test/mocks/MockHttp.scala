...
trait MockHttp extends UnitSpec with BeforeAndAfterEach {
  ...
  def setupMockHttpPost(url: String, body: JsValue)(response: HttpResponse): OngoingStubbing[Future[HttpResponse]] =
    when(
      httpClientMock.POST[JsValue, HttpResponse](matches(url), ArgumentMatchers.eq(body), ArgumentMatchers.any())(
  ...
  def setupMockHttpGet(url: String)(response: HttpResponse): OngoingStubbing[Future[HttpResponse]] =
    when(
      httpClientMock.GET[HttpResponse](matches(url), ArgumentMatchers.any(), ArgumentMatchers.any())(
        ArgumentMatchers.any(),
        ArgumentMatchers.any(),
        ArgumentMatchers.any()
      )
  ...
  def setupMockHttpPutWithHeaderCarrier[R](
      url:     String,
    ...
    when(
      httpClientMock.PUT[R, HttpResponse](
  ...
  def setupAgentMockHttpGet(url: Option[String] = None)(status: Int, response: JsValue): Unit = {
    ...
    when(
      httpClientMock.GET[HttpResponse](urlMatcher)(
        ArgumentMatchers.any(),
        ArgumentMatchers.any(),
        ArgumentMatchers.any[ExecutionContext]
      )
    ).thenReturn(Future.successful(HttpResponse(status = status, json = response, headers = Map.empty)))
  }

  def setupMockHttpGetWithParams(
      url:    String,
    ...
    when(
      httpClientMock.GET[HttpResponse](matches(url), ArgumentMatchers.eq(params), ArgumentMatchers.any())(
        ArgumentMatchers.any(),
        ArgumentMatchers.any(),
        ArgumentMatchers.any()
      )
  ...
  def setupMockFailedHttpGetWithParams(
      url:    String,
      params: Seq[(String, String)]
    )(
      response: HttpResponse
  ...
  def setupMockHttpGetPartial(url: String)(response: HtmlPartial): OngoingStubbing[Future[HtmlPartial]] =
    when(
      httpClientMock
        .GET[HtmlPartial](matches(url))(ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())
    ).thenReturn(Future.successful(response))
}
