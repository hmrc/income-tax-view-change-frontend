      ...
      s"the user is authenticated as a $mtdRole" when {
          ...
          "render the SingleTaxYearWarningView" in {
            ...
            contentAsString(result) shouldBe
              singleTaxYearWarningView(
                form = form,
                submitAction = controllers.optIn.oldJourney.routes.SingleTaxYearOptInWarningController.submit(isAgent),
                isAgent = isAgent,
        ...
        "no tax years are returned" should {
          ...
          "redirect to ChooseYear page - SEE_OTHER" in {
            ...
            redirectLocation(result) shouldBe Some(controllers.optIn.oldJourney.routes.ChooseYearController.show(isAgent).url)
          }
        }
        ...
        "a there is a single available tax year" when {
          ...
          "user answers Yes - true" should {
            ...
            "handle the submit request and redirect to the ConfirmTaxYear page" in {
              ...
              status(result) shouldBe SEE_OTHER
              redirectLocation(result) shouldBe Some(controllers.optIn.oldJourney.routes.ConfirmTaxYearController.show(isAgent).url)
            }
          }
          ...
          "user answers No - false" should {
            ...
            "handle the submit request and redirect to the Opt In Cancelled page"  in {
              ...
              val expectedUrl =
                if (isAgent) controllers.optIn.oldJourney.routes.OptInCancelledController.showAgent().url
                else controllers.optIn.oldJourney.routes.OptInCancelledController.show().url
