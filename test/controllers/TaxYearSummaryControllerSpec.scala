...
class TaxYearSummaryControllerSpec
  ...
  val testChargesList: List[TaxYearSummaryChargeItem] = List(
    ...
    TaxYearSummaryChargeItem.fromChargeItem(
      testCharge,
      dueDate = testCharge.interestEndDate,
  ...
  val payeChargesList: List[TaxYearSummaryChargeItem] = List(
    chargeItemModel(
      transactionType = BalancingCharge,
  ...
  val populatedCTAViewModel: TYSClaimToAdjustViewModel =
    TYSClaimToAdjustViewModel(adjustPaymentsOnAccountFSEnabled = true, Some(TaxYear(2023, 2024)))
  lazy val ctaLink: Boolean => String = isAgent => {
    "/report-quarterly/income-and-expenses/view" + { if (isAgent) "/agents" else "" } + "/adjust-poa/start"
  }

  val testObligtionsModel: ObligationsModel = ObligationsModel(
    Seq(
  ...
  mtdAllRoles.foreach {
    case mtdUserRole =>
      ...
      val action =
        if (isAgent) testController.renderAgentTaxYearSummaryPage(testTaxYear)
        else testController.renderTaxYearSummaryPage(testTaxYear)
            ...
            "render the tax year summary page" that {
              ...
              "Review and Reconcile debit charges in the charges table" when {
                "the user has Review and Reconcile debit charges" in {
                  ...
                  mockFinancialDetailsSuccess(financialDetailsModelResponse =
                    financialDetailsWithReviewAndReconcileDebits
                  )
                  mockgetNextUpdates(
                    fromDate = LocalDate.of(testTaxYear - 1, 4, 6),
                    toDate = LocalDate.of(testTaxYear, 4, 5)
                  ...
                  val result = action(fakeRequest)
                  def chargeSummaryUrl(id: String) =
                    if (isAgent) {
                  ...
                  Jsoup
                    ...
                    .getElementById("accrues-interest-tag")
                    .text() shouldBe "ACCRUES INTEREST"
                  Jsoup
                    .parse(contentAsString(result))
                    .getElementById("paymentTypeText-0")
                    .text() shouldBe "First payment on account: extra amount from your tax return"
                  Jsoup
                    .parse(contentAsString(result))
                    .getElementById("paymentTypeLink-0")
                    .attr("href") shouldBe chargeSummaryUrl("RARDEBIT01")
                  Jsoup
                    .parse(contentAsString(result))
                    .getElementById("paymentTypeText-1")
                    .text() shouldBe "Second payment on account: extra amount from your tax return"
                  Jsoup
                    .parse(contentAsString(result))
                    .getElementById("paymentTypeLink-1")
                    .attr("href") shouldBe chargeSummaryUrl("RARDEBIT02")
                }
              }
              ...
              "does not Review and Reconcile debit charges in the charges table" when {
                "tReviewAndReconcilePoa FS is disabled" in {
                  ...
                  mockCalculationSuccessfulNew(testMtditid)
                  mockFinancialDetailsSuccess(financialDetailsModelResponse =
                    financialDetailsWithReviewAndReconcileDebitsOverdue
                  )
                  mockgetNextUpdates(
                    fromDate = LocalDate.of(testTaxYear - 1, 4, 6),
                    toDate = LocalDate.of(testTaxYear, 4, 5)
                  ...
                  status(result) shouldBe OK
                  Option(
                    Jsoup.parse(contentAsString(result)).getElementById("paymentTypeText-0")
                  ).isDefined shouldBe false
                  Option(
                    Jsoup.parse(contentAsString(result)).getElementById("paymentTypeLink-0")
                  ).isDefined shouldBe false
                  Option(
                    Jsoup.parse(contentAsString(result)).getElementById("paymentTypeText-1")
                  ).isDefined shouldBe false
                  Option(
                    Jsoup.parse(contentAsString(result)).getElementById("paymentTypeLink-1")
                  ).isDefined shouldBe false
                }
              }
              ...
              "does not have calculation summary" when {
                "calculation service returned not found" in {
                  ...
                  val expectedContent: String = Jsoup
                    .parse(
                      taxYearSummaryView(
                        testTaxYear,
                        TaxYearSummaryViewModel(
              ...
              "show the Tax Year Summary Page with error messages" when {
                "liability Calculation has error messages" in {
                  ...
                  val errorMessageVariableValues = testController.formatErrorMessages(
                    liabilityCalculationModelErrorMessagesForIndividual,
                    messagesApi,
                    isAgent = false
                  )(Lang("GB"), messages)
          ...
          s"the $mtdUserRole is authenticated" that {
            "has liability Calculation error messages" should {
              "filter out the variable value from messages" in {
                val actual = testController.formatErrorMessages(
                  liabilityCalculationModelErrorMessagesForIndividual,
                  messagesApi,
                  isAgent = isAgent
                )(Lang("GB"), messages)

                actual shouldBe liabilityCalculationModelErrorMessagesForIndividual.copy(messages =
                  Some(
                    Messages(
                      errors = Some(
                        List(
