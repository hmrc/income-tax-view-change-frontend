...
class PaymentControllerSpec extends TestSupport with MockAuthenticationPredicate with MockAuditingService {
  ...
  class SetupTestPaymentController(response: Future[PaymentJourneyResponse]) {
    ...
    when(mockPayApiConnector.startPaymentJourney(ArgumentMatchers.eq(testSaUtrId), ArgumentMatchers.eq(BigDecimal(10000)), ArgumentMatchers.eq(false))
  ...
  "The PaymentController.paymentHandoff action" should {
    ...
    "return an internal server error" when {
      ...
      "an error response is returned by the connector" in new SetupTestPaymentController(Future.successful(
