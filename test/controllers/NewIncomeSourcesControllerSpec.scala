...
class NewIncomeSourcesControllerSpec extends MockAuthenticationPredicate
  ...
  val controller = new NewIncomeSourcesController(
  ...
  "The NewIncomeSourcesController for Individuals" should {
    ...
    "redirect a user back to the home page" when {
      ...
      "the IncomeSources FS is disabled" in {
        ...
        redirectLocation(result) shouldBe Some(controllers.routes.NewIncomeSourcesController.show().url)
      }
    }
  }
}
