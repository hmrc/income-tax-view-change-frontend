          ...
          s"return $OK showing ceased business obligation page" when {
            if (incomeSourceType == SelfEmployment) {
              "all required data is available" in {
                ...
                setupMockGetSessionKeyMongoTyped[String](key = CeaseIncomeSourceData.incomeSourceIdField, journeyType = IncomeSourceJourneyType(Cease, incomeSourceType), result = Right(Some(testSelfEmploymentId)))
          ...
          s"return $INTERNAL_SERVER_ERROR showing error page" when {
            ...
            if (incomeSourceType == SelfEmployment) {
              s"income source ID is missing in session for $SelfEmployment business" in {
                setupMockSuccess(mtdRole)
                enable(IncomeSourcesFs)
                setupMockGetSessionKeyMongoTyped[String](key = CeaseIncomeSourceData.incomeSourceIdField, journeyType = IncomeSourceJourneyType(Cease, incomeSourceType), result = Right(None))
