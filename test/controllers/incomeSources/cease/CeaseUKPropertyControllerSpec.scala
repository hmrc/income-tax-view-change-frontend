...
import testConstants.BaseTestConstants.testIndividualAuthSuccessWithSaUtrResponse
import testUtils.TestSupport
import uk.gov.hmrc.http.{HttpClient, HttpResponse}
import views.html.CeaseUKProperty
...
class CeaseUKPropertyControllerSpec extends TestSupport with MockAuthenticationPredicate with MockIncomeSourceDetailsPredicate with FeatureSwitching {
  ...
  val mockDateUKPropertyCeased = mock(classOf[Da])
  ...
  object TestCeaseUKPropertyController extends CeaseUKPropertyController(
    ...
    val fields: Map[String, String] = Map(
      "declaration" -> "cease-uk-property-declaration",
      "csrfToken" -> "token"
    )
  ...
  "CeaseUKPropertyController.show" should {
    "return 200 OK" when {
      "navigating to the page with FS Enabled" in {
        ...
        document.select("h1:nth-child(1)").text shouldBe TestCeaseUKPropertyController.heading
      }
    }
    "return 303 SEE_OTHER and redirect to custom not found error page" when {
      ...
      "called with an unauthenticated user" in {
        ...
        val result = TestCeaseUKPropertyController.show()(fakeRequestWithActiveSession)
        status(result) shouldBe Status.SEE_OTHER
      }
    }
  }

  "CeaseUKPropertyController.submit" should {
    s"return 303 SEE_OTHER and redirect to ${controllers.incomeSources.cease.routes.DateUKPropertyCeasedController.show().url}" when {
      s"form is completed successfully" in {
        setupMockAuthRetrievalSuccess(testIndividualAuthSuccessWithSaUtrResponse())
        enable(IncomeSources)
        mockPropertyIncomeSource()
        when(mockHttpClient.POSTForm[HttpResponse](any(), any(), any())(any(), any(), any()))
          .thenReturn(Future.successful(HttpResponse(OK, "cease-uk-property-declaration")))

        NotImplemented

        val result = {
          TestCeaseUKPropertyController.submit()(fakeRequestWithActiveSession.withFormUrlEncodedBody(TestCeaseUKPropertyController.fields.toSeq: _*))
        }
        status(result) shouldBe Status.SEE_OTHER
