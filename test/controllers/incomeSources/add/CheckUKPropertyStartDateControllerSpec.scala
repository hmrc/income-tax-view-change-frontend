...
class CheckUKPropertyStartDateControllerSpec extends TestSupport with MockAuthenticationPredicate
  ...
  "Individual - AddUKPropertyStartDateController.submit" should {
    s"return 303 SEE_OTHER and redirect to ${controllers.incomeSources.add.routes.UKPropertyBusinessAccountingMethodController.show().url}" when {
      "user confirms the date is correct" in {
        ...
        status(result) shouldBe Status.SEE_OTHER
        redirectLocation(result) shouldBe Some(controllers.incomeSources.add.routes.UKPropertyBusinessAccountingMethodController.show().url)
      }
  ...
  "Agent - AddUKPropertyStartDateController.submitAgent" should {
    s"return 303 SEE_OTHER and redirect to ${controllers.incomeSources.add.routes.UKPropertyBusinessAccountingMethodController.showAgent().url}" when {
      "user confirms the date is correct" in {
        setupMockAgentAuthRetrievalSuccess(testAgentAuthRetrievalSuccess)
        enable(IncomeSources)
        mockSingleBusinessIncomeSource()
        ...
        redirectLocation(result) shouldBe Some(controllers.incomeSources.add.routes.UKPropertyBusinessAccountingMethodController.showAgent().url)
      }
      "user confirms the date is incorrect" in {
        setupMockAgentAuthRetrievalSuccess(testAgentAuthRetrievalSuccess)
        enable(IncomeSources)
