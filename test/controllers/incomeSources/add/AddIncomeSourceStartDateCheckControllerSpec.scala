...
import models.incomeSourceDetails.AddIncomeSourceData.{dateStartedField, journeyIsCompleteField}
  ...
  "Individual - AddIncomeSourceStartDateCheckController.show" should {
    ...
    s"return ${Status.SEE_OTHER}: redirect to You Cannot Go Back page" when {
      "user has already completed the journey" in {
        ...
        mockNoIncomeSources()
        setupMockAuthRetrievalSuccess(BaseTestConstants.testIndividualAuthSuccessWithSaUtrResponse())
        setupMockGetSessionKeyMongoTyped[Boolean](journeyIsCompleteField, journeyTypeSE, Right(Some(true)))
    ...
    s"return ${Status.OK}" when {
      s"calling Business Start Date Check Page and session contains key: ${AddIncomeSourceData.accountingPeriodStartDateField}" in {
        ...
        setupMockGetSessionKeyMongoTyped[Boolean](journeyIsCompleteField, journeyTypeSE, Right(None))
    ...
    s"return ${Status.OK}" when {
      s"calling Foreign Property Start Date Check Page and session contains key: ${AddIncomeSourceData.accountingPeriodStartDateField}" in {
        ...
        setupMockGetSessionKeyMongoTyped[Boolean](journeyIsCompleteField, journeyTypeFP, Right(None))
    ...
    s"return ${Status.OK}" when {
      s"calling UK Property Start Date Check Page and session contains key: ${AddIncomeSourceData.accountingPeriodStartDateField}" in {
        ...
        setupMockGetSessionKeyMongoTyped[Boolean](journeyIsCompleteField, journeyTypeUK, Right(None))
