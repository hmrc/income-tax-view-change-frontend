...
import enums.JourneyType.{Add, JourneyType, IncomeSources}
...
class AddBusinessNameControllerSpec extends TestSupport
  ...
  val journeyType: JourneyType = journeyType
      ...
      s"ADD - AddBusinessNameController.show (${if (isAgent) "Agent" else "Individual"}, isChange = $isChange)" should {
        "return 200 OK" when {
          "the user is authenticated" in {
            ...
            if (isChange) {
              setupMockGetMongo(Right(Some(notCompletedUIJourneySessionData(journeyType))))
              ...
              setupMockGetMongo(Right(Some(emptyUIJourneySessionData(journeyType))))
            }
        ...
        s"return ${Status.SEE_OTHER}: redirect to the relevant You Cannot Go Back page" when {
          "user has already completed the journey" in {
            ...
            mockNoIncomeSources()
            setupMockAuthorisationSuccess(isAgent)
            setupMockGetMongo(Right(Some(completedUIJourneySessionData(journeyType))))
