...
class RefundToTaxPayerControllerSpec extends MockAuthActions with MockRepaymentHistoryConnector {
  ...
  mtdAllRoles.foreach {
    case mtdUserRole =>
      ...
      val action =
        if (isAgent) testController.showAgent(repaymentRequestNumber) else testController.show(repaymentRequestNumber)
          ...
          if (mtdUserRole == MTDSupportingAgent) {
            ...
            "render the error page" when {
              "Failing to retrieve a user's re-payments history" in {
                ...
                setupMockSuccess(mtdUserRole)
                mockSingleBusinessIncomeSource()
                setupGetRepaymentHistoryByRepaymentIdError(testNino, repaymentRequestNumber)(
                  RepaymentHistoryErrorModel(404, "Not found")
                )
