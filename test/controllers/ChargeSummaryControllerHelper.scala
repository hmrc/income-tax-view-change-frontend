...
trait ChargeSummaryControllerHelper
  ...
  def financialDetailsWithLocks(taxYear: Int): List[FinancialDetail] =
    List(
  ...
  def emptyAdjustmentHistoryModel: AdjustmentHistoryModel =
    AdjustmentHistoryModel(AdjustmentModel(1000, None, AmendedReturnReversalReason), List())
  ...
  def successHeadingForRAR1(startYear: String, endYear: String) =
    s"$startYear to $endYear tax year ${messages("chargeSummary.reviewAndReconcilePoa1.text")}"

  def successHeadingForRAR2(startYear: String, endYear: String) =
    s"$startYear to $endYear tax year ${messages("chargeSummary.reviewAndReconcilePoa2.text")}"

  def successHeadingRAR1Interest(startYear: String, endYear: String) =
    s"$startYear to $endYear tax year ${messages("chargeSummary.lpi.reviewAndReconcilePoa1.text")}"
  ...
  val paymentprocessingbullet1: String =
    s"${messages("chargeSummary.payments-bullet1-1")} ${messages("chargeSummary.payments-bullet1-2")} ${messages("pagehelp.opensInNewTabText")} ${messages("chargeSummary.payments-bullet2")}"
  val paymentprocessingbullet1Agent: String =
    s"${messages("chargeSummary.payments-bullet1-1")} ${messages("chargeSummary.payments-bullet1-2-agent")} ${messages("pagehelp.opensInNewTabText")} ${messages("chargeSummary.payments-bullet2-agent")}"
  val warningText: String = "Warning " + messages("chargeSummary.reviewAndReconcilePoa.warning")
  val explanationTextForRAR1: String = messages("chargeSummary.reviewAndReconcilePoa.p1") + " " + messages(
    "chargeSummary.reviewAndReconcilePoa1.linkText"
  ) + " " + messages("chargeSummary.reviewAndReconcilePoa.p2")
  val explanationTextForRAR2: String = messages("chargeSummary.reviewAndReconcilePoa.p1") + " " + messages(
    "chargeSummary.reviewAndReconcilePoa2.linkText"
  ) + " " + messages("chargeSummary.reviewAndReconcilePoa.p2")
  ...
  class Setup(financialDetails: FinancialDetailsResponseModel, chargeHistoryHasError: Boolean = false) {
