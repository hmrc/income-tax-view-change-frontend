...
class ItsaStatusRetrievalActionSpec extends TestSupport with ScalaFutures {
  ...
  ".refine()" should {
    ...
    "redirect to Home page when only next-year ITSA status exists - Agent" in {
      ...
      val result = action.refine(authorisedAndEnrolledRequest).futureValue
    ...
    "return an MtdItUser when valid ITSA status exists for current and future years" in {
      ...
      result.map(_.mtditid) shouldBe Right(request.mtditId)
      result.map(_.nino) shouldBe Right(defaultIncomeSourcesData.nino)
      result.map(_.usersRole) shouldBe Right(request.mtdUserRole)
      result.map(_.authUserDetails) shouldBe Right(request.authUserDetails)
      result.map(_.clientDetails) shouldBe Right(request.clientDetails)
