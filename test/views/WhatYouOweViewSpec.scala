...
class WhatYouOweViewSpec
  ...
  val saNoteAgent: String =
    s"${messages("whatYouOwe.sa-note-agent-1")}. ${messages("whatYouOwe.sa-note-agent-2")} ${messages(
      "whatYouOwe.sa-link-agent"
    )} ${messages("pagehelp.opensInNewTabText")}. ${messages("whatYouOwe.sa-note-agent-3")}"
  ...
  val paymentUnderReviewPara: String =
    s"${messages("whatYouOwe.dunningLock.text", s"${messages("whatYouOwe.dunningLock.link")} ${messages("pagehelp.opensInNewTabText")}")}."
  ...
  val paymentProcessingBullet1: String =
    s"${messages("whatYouOwe.payments-made-bullet-1.1")} ${messages("whatYouOwe.payments-made-bullet-1.2")} ${messages("pagehelp.opensInNewTabText")}"
  val paymentProcessingBullet2: String = messages("whatYouOwe.payments-made-bullet-2")
  val paymentProcessingBulletAgent1: String =
    s"${messages("whatYouOwe.payments-made-bullet-1.1")} ${messages("whatYouOwe.payments-made-bullet-agent-1.2")} ${messages("pagehelp.opensInNewTabText")}"
  ...
  val preMTDRemainingBalance: String =
    s"${messages("whatYouOwe.balancingCharge.text")} ${messages("whatYouOwe.pre-mtd-digital")}"
  ...
  def ctaViewModel(isFSEnabled: Boolean): WYOClaimToAdjustViewModel = {
    if (isFSEnabled) {
      WYOClaimToAdjustViewModel(
        ...
        poaTaxYear = Some(TaxYear(startYear = 2024, endYear = 2025))
      )
    } else {
      WYOClaimToAdjustViewModel(adjustPaymentsOnAccountFSEnabled = false, poaTaxYear = None)
    }
  }
  ...
  def taxYearSummaryText(from: String, to: String): String =
    s"${messages("whatYouOwe.tax-year-summary.taxYear", from, to)} ${messages("whatYouOwe.taxYear")}"
  ...
  class AgentTestSetup(
    ...
    val agentUser: MtdItUser[_] =
      defaultMTDITUser(
        Some(testUserTypeAgent),
        IncomeSourceDetailsModel("AA111111A", "testMtditid", Some(migrationYear.toString), List(), List())
      )
  ...
  def financialDetailsOverdueWithLpiDunningLock(
      latePaymentInterest: Option[BigDecimal],
      lpiWithDunningLock:  Option[BigDecimal]
    ): FinancialDetailsModel =
    testFinancialDetailsModelWithLPIDunningLock(
  ...
  def financialDetailsOverdueWithLpiDunningLockZero(
      latePaymentInterest: Option[BigDecimal],
      lpiWithDunningLock:  Option[BigDecimal]
    ): FinancialDetailsModel =
    testFinancialDetailsModelWithLpiDunningLockZero(
      documentDescription = List(Some(itsaPOA1), Some(itsaPOA2)),
      mainType = List(Some(saPaymentOnAccount1), Some(saPaymentOnAccount2)),
      mainTransaction = List(Some("4920"), Some("4930")),
      dueDate = List(Some(fixedDate.minusDays(10).toString), Some(fixedDate.minusDays(1).toString)),
  ...
  def whatYouOweDataWithOverdueInterestData(latePaymentInterest: List[Option[BigDecimal]]): WhatYouOweChargesList =
  ...
  def whatYouOweDataWithOverdueLPI(
  ...
  def whatYouOweDataWithOverdueLPIDunningLock(
      latePaymentInterest: Option[BigDecimal],
      lpiWithDunningLock:  Option[BigDecimal]
    ): WhatYouOweChargesList =
    WhatYouOweChargesList(
  ...
  def whatYouOweDataWithOverdueLPIDunningLockZero(
      latePaymentInterest: Option[BigDecimal],
      lpiWithDunningLock:  Option[BigDecimal]
    ): WhatYouOweChargesList =
    WhatYouOweChargesList(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
      chargesList = financialDetailsOverdueWithLpiDunningLockZeroCi(
        TaxYear.forYearEnd(fixedDate.getYear),
        latePaymentInterest,
        false,
        lpiWithDunningLock
      ),
  ...
  def whatYouOweDataWithOverdueMixedData2(latePaymentInterest: List[Option[BigDecimal]]): WhatYouOweChargesList =
    WhatYouOweChargesList(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
  ...
  def whatYouOweDataTestActiveWithMixedData2(latePaymentInterest: List[Option[BigDecimal]]): WhatYouOweChargesList =
    WhatYouOweChargesList(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
      chargesList = List(financialDetailsOverdueWithLpi(latePaymentInterest, noDunningLocks)(1))
        ++ List(financialDetailsWithMixedData3Ci.head),
  ...
  val codingOutNotice = s"${messages("whatYouOwe.codingOut-1a")} £43.21 ${messages(
    "whatYouOwe.codingOut-1b"
  )} ${messages("whatYouOwe.codingOut-2", "2020", "2021")} ${messages("whatYouOwe.codingOut-3")}"
  val codingOutNoticeFullyCollected = s"${messages("whatYouOwe.credit-overpaid-prefix")} £0.00 ${messages(
    "whatYouOwe.codingOut-1b"
  )} ${messages("whatYouOwe.codingOut-2", "2020", "2021")} ${messages("whatYouOwe.codingOut-individual")}"

  val codedOutDocumentDetailNICs: DocumentDetail = DocumentDetail(
    taxYear = 2021,
  ...
  val codedOutDocumentDetail: DocumentDetail = DocumentDetail(
    taxYear = 2021,
    transactionId = "CODINGOUT02",
    documentDescription = Some("TRM New Charge"),
    documentText = Some(CODING_OUT_CLASS2_NICS),
  ...
  val codedOutDocumentDetailFullyCollected: DocumentDetail = DocumentDetail(
    taxYear = 2021,
    transactionId = "CODINGOUT02",
    documentDescription = Some("TRM New Charge"),
    documentText = Some(CODING_OUT_CLASS2_NICS),
  ...
  val codedOutDocumentDetailPayeSA: DocumentDetail = DocumentDetail(
    taxYear = 2021,
    transactionId = "CODINGOUT02",
    documentDescription = Some("TRM New Charge"),
  ...
  val outstandingChargesWithAciValueZeroAndOverdue: OutstandingChargesModel =
    outstandingChargesModel(fixedDate.minusDays(15).toString, 0.00)
  ...
  val whatYouOweDataCodingOutWithoutAmountCodingOut: WhatYouOweChargesList =
    whatYouOweDataWithCodingOutNics2.copy(codedOutDocumentDetail = None)
  ...
  val noChargesModel: WhatYouOweChargesList =
    WhatYouOweChargesList(balanceDetails = BalanceDetails(0.00, 0.00, 0.00, None, None, None, None, None))
  ...
  val noUtrModel: WhatYouOweChargesList =
    WhatYouOweChargesList(balanceDetails = BalanceDetails(0.00, 0.00, 0.00, None, None, None, None, None))
  ...
  "individual" when {
    "The What you owe view with financial details model" when {
      "the user has charges and access viewer before 30 days of due date" should {
        ...
        "Balancing Payment row data exists and should not contain hyperlink and overdue tag " in new TestSetup(
        ...
        "have POA data in same table" in new TestSetup(
          charges = whatYouOweDataWithDataDueInMoreThan30Days(
          ...
          poa1Table.select("td").get(2).text() shouldBe taxYearSummaryText(
            (fixedDate.getYear - 1).toString,
          ...
          poa2Table.select("td").get(2).text() shouldBe taxYearSummaryText(
            (fixedDate.getYear - 1).toString,
            fixedDate.getYear.toString
          )
        ...
        "have review and reconcile extra payments with Accrues Interest tags in the same table" in new TestSetup(
          ...
          poaExtra1Table.select("td").get(2).text() shouldBe taxYearSummaryText(
            (fixedDate.getYear - 1).toString,
            fixedDate.getYear.toString
          )
          ...
          poa2ExtraTable.select("td").get(2).text() shouldBe taxYearSummaryText(
            (fixedDate.getYear - 1).toString,
            fixedDate.getYear.toString
          )
        ...
        "have interest charges for paid reconciliation charges in the same table" in new TestSetup(
          charges = whatYouOweReconciliationInterestData,
          reviewAndReconcileEnabled = true
        ) {
          val poaExtra1Table: Element = pageDocument.getElementById("due-0")
        ...
        "should have payment processing bullets when payment due in more than 30 days" in new TestSetup(
          charges = whatYouOweDataWithDataDueInMoreThan30Days()
        ) {
          ...
          pageDocument
            .getElementById("sa-tax-bill")
            .attr("href") shouldBe "https://www.gov.uk/pay-self-assessment-tax-bill"
        ...
        "money in your account section with available credits" in new TestSetup(
          charges = whatYouOweDataWithAvailableCredits()
        ) {
          pageDocument
            .getElementById("money-in-your-account")
            .text shouldBe messages("whatYouOwe.moneyOnAccount") + " " +
            messages("whatYouOwe.moneyOnAccount-1") + " £300.00" + " " +
            messages("whatYouOwe.moneyOnAccount-2") + " " +
            messages("whatYouOwe.moneyOnAccount-3") + "."
        }

        "money in your account section with no available credits" in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30Days()
        ) {
          findElementById("money-in-your-account") shouldBe None
        }

        "money in your account section with available credits equal to £0 " in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30DaysAvailableCreditZero()
        ) {
          findElementById("money-in-your-account") shouldBe None
        }
      ...
      "the user has charges and access viewer within 30 days of due date" should {
        s"have the Balancing Payment header and table data" in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30Days()
        ) {
          val remainingBalanceHeader: Element = pageDocument.select("tr").first()
          remainingBalanceHeader.select("th").first().text() shouldBe dueDate
        ...
        "have POA data in same table as balancing payment " in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30Days()(dateService)
        ) {
          val poa1Table: Element = pageDocument.select("tr").get(2)
        ...
        "have Data for due within 30 days" in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30Days()(dateService)
        ) {

          pageDocument.getElementById("due-0-link").attr("href") shouldBe controllers.routes.ChargeSummaryController
            .show(fixedDate.getYear, "1040000124")
            .url
          findElementById("due-0-overdue") shouldBe None
          pageDocument
            .getElementById("taxYearSummary-link-0")
            .attr("href") shouldBe controllers.routes.TaxYearSummaryController
            .renderTaxYearSummaryPage(fixedDate.getYear)
            .url
        }

        "have data with POA2 with hyperlink and no overdue" in new TestSetup(
          charges = whatYouOweDataWithDataDueIn30Days()(dateService)
        ) {
          pageDocument.getElementById("due-1-link").attr("href") shouldBe controllers.routes.ChargeSummaryController
            .show(fixedDate.getYear, "1040000125")
            .url
        ...
        "have payment details and should not contain future payments " +
          ...
          pageDocument.getElementById("payment-button-link").attr("href") shouldBe controllers.routes.PaymentController
            .paymentHandoff(5000)
            .url
        }
      ...
      "the user has charges and access viewer after due date" should {
        ...
        "have the mtd payments header, table header and data with Balancing Payment data with no hyperlink but have overdue tag" in new TestSetup(
          ...
          interestTable.select("td").get(1).text() shouldBe messages(
            "whatYouOwe.balancingCharge.interest.line1.text"
          ) + " " +
            messages(
              "whatYouOwe.balancingCharge.interest.line2.text",
              fixedDate.minusDays(30).toLongDateShort,
              fixedDate.toLongDateShort
            )
        ...
        "have payment type dropdown details and bullet point list" in new TestSetup(
          charges = whatYouOweDataWithOverdueDataAndInterest()
        ) {
          pageDocument.getElementById("payments-made").text shouldBe paymentsMade
          val paymentProcessingBullet: Element = pageDocument.getElementById("payments-made-bullets")
        ...
        "have overdue payments header and data with POA1 charge type and show Late payment interest on payment on account 1 of 2" in
          new TestSetup(charges = whatYouOweDataWithOverdueLPI(List(Some(34.56), None))) {
            ...
            overduePaymentsTableRow1.select("td").get(2).text() shouldBe taxYearSummaryText(
              (fixedDate.getYear - 1).toString,
              fixedDate.getYear.toString
            )
            ...
            pageDocument
              .getElementById("due-0-late-link")
              .attr("href") shouldBe controllers.routes.ChargeSummaryController
              .show(fixedDate.getYear, "1040000124", isInterestCharge = true)
              .url
        ...
        "should have payment processing bullets when there is POA1 charge and lpi on poa 1 of 2" in new TestSetup(
          charges = whatYouOweDataWithOverdueLPI(List(Some(34.56), None))
        ) {
        ...
        "have overdue payments header, bullet points and data with POA1 charge type and No Late payment interest" in new TestSetup(
          charges = whatYouOweDataWithOverdueLPI(List(None, None))
        ) {
          ...
          pageDocument
            .getElementById("due-0-late-link2")
            .attr("href") shouldBe controllers.routes.ChargeSummaryController.show(fixedDate.getYear, "1040000124").url
          pageDocument.getElementById("due-0-overdue").text shouldBe overdueTag
          pageDocument
            .getElementById("taxYearSummary-link-0")
            .attr("href") shouldBe controllers.routes.TaxYearSummaryController
        ...
        "have overdue payments header, bullet points and data with POA1 charge type" in new TestSetup(
          charges = whatYouOweDataWithOverdueLPI(List(None, None))
        ) {
        ...
        "have overdue payments with POA2 charge type with hyperlink and overdue tag" in new TestSetup(
          charges = whatYouOweDataWithOverdueLPI(List(None, None))
        ) {
          ...
          pageDocument
            .getElementById("due-1-late-link2")
            .attr("href") shouldBe controllers.routes.ChargeSummaryController.show(fixedDate.getYear, "1040000125").url
          pageDocument.getElementById("due-1-overdue").text shouldBe overdueTag
        }

        "have accruing interest displayed below each overdue POA" in new TestSetup(
          charges = whatYouOweDataWithOverdueInterestData(List(None, None))
        ) {
          def overduePaymentsInterestTableRow(index: String): Element =
            pageDocument.getElementById(s"charge-interest-$index")

          overduePaymentsInterestTableRow("0").select("td").get(1).text() shouldBe interestFromToDate(
            "25 May 2019",
            "25 Jun 2019",
            "2.6"
          )
          ...
          overduePaymentsInterestTableRow("1").select("td").get(1).text() shouldBe interestFromToDate(
            "25 May 2019",
            "25 Jun 2019",
            "6.2"
          )
        ...
        "should have payment processing bullets when there is accruing interest" in new TestSetup(
          charges = whatYouOweDataWithOverdueInterestData(List(None, None))
        ) {
        ...
        "only show interest for POA when there is no late Payment Interest" in new TestSetup(
          charges = whatYouOweDataWithOverdueInterestData(List(Some(34.56), None))
        ) {
          def overduePaymentsInterestTableRow(index: String): Element =
            pageDocument.getElementById(s"charge-interest-$index")
        ...
        "not have a paragraph explaining interest rates when there is no accruing interest" in new TestSetup(
          charges = whatYouOweDataWithOverdueDataIt()
        ) {
      ...
      "the user has charges and access viewer with mixed dates" should {
        ...
        "should have payment processing bullets when there is mixed dates" in new TestSetup(
          charges = whatYouOweDataWithMixedData1
        ) {
        ...
        s"have overdue table header, bullet points and data with hyperlink and overdue tag" in new TestSetup(
          charges = whatYouOweDataWithOverdueMixedData2(List(None, None, None))
        ) {
          ...
          dueWithInThirtyDaysTableRow1.select("td").get(2).text() shouldBe taxYearSummaryText(
            (fixedDate.getYear - 1).toString,
            fixedDate.getYear.toString
          )
          ...
          pageDocument.getElementById("due-1-link").attr("href") shouldBe controllers.routes.ChargeSummaryController
            .show(fixedDate.getYear, "1040000123")
            .url
          findElementById("due-1-overdue") shouldBe None
          pageDocument
            .getElementById("taxYearSummary-link-1")
            .attr("href") shouldBe controllers.routes.TaxYearSummaryController
            .renderTaxYearSummaryPage(fixedDate.getYear)
            .url
    ...
    "the user has charges and access viewer with mixed dates and ACI value of zero" should {
      ...
      s"have overdue table header and data with hyperlink and overdue tag" in new TestSetup(
        charges = whatYouOweDataTestActiveWithMixedData2(List(None, None, None, None))
      ) {
        val overdueTableHeader: Element = pageDocument.select("tr").get(0)
        overdueTableHeader.select("th").first().text() shouldBe dueDate
        overdueTableHeader.select("th").get(1).text() shouldBe chargeType
        ...
        pageDocument.getElementById("due-0-late-link2").attr("href") shouldBe controllers.routes.ChargeSummaryController
          .show(fixedDate.getYear, "1040000125")
          .url
        pageDocument.getElementById("due-0-overdue").text shouldBe overdueTag
        pageDocument
    ...
    "the user has no charges" should {
      ...
      "have the link to their previous Self Assessment online account in the sa-note" in new TestSetup(
        charges = noChargesModel
      ) {
    ...
    "AdjustPaymentsOnAccount is enabled" when {
      ...
      "user has a POA that can be adjusted" when {
        ...
        "POA is paid off fully should display link with additional content" in new TestSetup(
          ...
          pageDocument.getElementById("adjust-poa-link").text() shouldBe messages(
            "whatYouOwe.adjust-poa.paid-2",
            "2024",
            "2025"
          )
    ...
    "codingOut is enabled" should {
      "have coding out message displayed at the bottom of the page" in new TestSetup(
        charges = whatYouOweDataWithCodingOutNics2
      ) {
      ...
      "should have payment processing bullets when there is coding out" in new TestSetup(
        charges = whatYouOweDataWithCodingOutNics2
      ) {
    ...
    "At crystallization, the user has the coding out requested amount fully collected" should {
      "only show coding out content under header" in new TestSetup(
        charges = whatYouOweDataWithCodingOutFullyCollected
      ) {
  ...
  "agent" when {
    "The What you owe view with financial details model" when {
      ...
      s"have the title '${messages("htmlTitle.agent", messages("whatYouOwe.heading-agent"))}'" in new AgentTestSetup(
        charges = whatYouOweDataWithDataDueIn30Days()(dateService)
      ) {
        pageDocument.title() shouldBe messages("htmlTitle.agent", messages("whatYouOwe.heading-agent"))
        pageDocument.getElementById("due-0-link").attr("href") shouldBe controllers.routes.ChargeSummaryController
          .showAgent(fixedDate.getYear, "1040000124")
          .url
        pageDocument
          .getElementById("taxYearSummary-link-0")
          .attr("href") shouldBe controllers.routes.TaxYearSummaryController
          .renderAgentTaxYearSummaryPage(fixedDate.getYear)
          .url
      }
      ...
      "money in your account section with available credits" in new AgentTestSetup(
        charges = whatYouOweDataWithAvailableCredits()
      ) {
        pageDocument
          .getElementById("money-in-your-account")
          .text shouldBe messages("whatYouOwe.moneyOnAccount-agent") + " " +
          messages("whatYouOwe.moneyOnAccount-1") + " £300.00" + " " +
          messages("whatYouOwe.moneyOnAccount-agent-2") + " " +
          messages("whatYouOwe.moneyOnAccount-3") + "."
      }

      "money in your account section with no available credits" in new AgentTestSetup(
        charges = whatYouOweDataWithDataDueIn30Days()
      ) {
        findAgentElementById("money-in-your-account") shouldBe None
      }

      "money in your account section with an available credit of £0.00" in new AgentTestSetup(
        charges = whatYouOweDataWithDataDueIn30DaysAvailableCreditZero()
      ) {
        findAgentElementById("money-in-your-account") shouldBe None
      }
    ...
    "the user has no charges" should {
      s"have the title ${messages("agent.header.serviceName", messages("whatYouOwe.heading-agent"))} and page header and notes" in new AgentTestSetup(
        charges = noChargesModel
      ) {
        pageDocument.title() shouldBe messages("htmlTitle.agent", messages("whatYouOwe.heading-agent"))
  ...
  "what you owe view" should {
    ...
    val unallocatedCreditMsg =
      "You have £100.00 in your account. We’ll use this to pay the amount due on the next due date."
    "show unallocated credits" when {
      "user is an individual with the feature switch on" in new TestSetup(
        charges = whatYouOweDataWithDataDueInMoreThan30Days()
      ) {
        pageDocument.getElementById("unallocated-credit-note").text() shouldBe unallocatedCreditMsg
      }

      "user is an agent with the feature switch on" in new AgentTestSetup(
        charges = whatYouOweDataWithDataDueInMoreThan30Days()
      ) {
        pageDocument.getElementById("unallocated-credit-note").text() shouldBe unallocatedCreditMsg
      }
