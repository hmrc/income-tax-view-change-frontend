...
class CheckCeaseUKPropertyDetailsViewSpec extends TestSupport with ViewSpec{
 ...
 def agentUserConfirmedClientWithSession(clientNino: String = "AA111111A"): MtdItUser[_] = MtdItUser(
  ...
  val changeUrl = controllers.incomeSources.cease.routes.IncomeSourceEndDateController.show(None, UkProperty.key).url
  val changeUrlAgent = controllers.incomeSources.cease.routes.IncomeSourceEndDateController.showAgent(None, UkProperty.key).url
