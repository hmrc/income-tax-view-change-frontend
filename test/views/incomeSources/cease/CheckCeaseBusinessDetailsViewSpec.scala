...
class CheckCeaseBusinessDetailsViewSpec extends TestSupport with ViewSpec{
 ...
 def agentUserConfirmedClientWithSession(clientNino: String = "AA111111A"): MtdItUser[_] = MtdItUser(
  ...
  val changeUrl = controllers.incomeSources.cease.routes.IncomeSourceEndDateController.show(Some(testSelfEmploymentId), SelfEmployment.key).url
  val changeUrlAgent = controllers.incomeSources.cease.routes.IncomeSourceEndDateController.showAgent(Some(testSelfEmploymentId), SelfEmployment.key).url
  ...
  def validateHtmlValues(isAgent: Boolean, document: Document): Unit = {
    ...
    document.getElementById("businessAddress").text() shouldBe address.addressLine1+" "+
      ...
      address.abbreviatedCountryCode
