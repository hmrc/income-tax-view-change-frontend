...
class CeaseForeignPropertyViewSpec extends TestSupport {
  val ceaseForeignPropertyView: CeaseForeignProperty = app.injector.instanceOf[CeaseForeignProperty]
  ...
  "CeaseForeignPropertyView - Individual" should {
    "render the legend" in new Setup(false) {
      document.getElementsByClass("govuk-fieldset__legend govuk-fieldset__legend--l").first().text() shouldBe messages("incomeSources.ceaseForeignProperty.heading")
    }
    "render the checkbox" in new Setup(false) {
      document.getElementById("cease-foreign-property-declaration").attr("type") shouldBe "checkbox"
    }
    "render the checkbox label" in new Setup(false) {
      document.getElementsByClass("govuk-label govuk-checkboxes__label").first().text() shouldBe messages("incomeSources.ceaseForeignProperty.checkboxLabel")
    }
    ...
    "render the error summary" in new Setup(false, true) {
      ...
      document.getElementsByClass("govuk-error-summary__body").first().text() shouldBe messages("incomeSources.ceaseForeignProperty.checkboxError")
    }
  }
  "CeaseForeignPropertyView - Agent" should {
    "render the legend" in new Setup(false) {
      document.getElementsByClass("govuk-fieldset__legend govuk-fieldset__legend--l").first().text() shouldBe messages("incomeSources.ceaseForeignProperty.heading")
    }
    "render the checkbox label" in new Setup(false) {
