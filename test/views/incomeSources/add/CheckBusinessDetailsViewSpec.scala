...
class CheckBusinessDetailsViewSpec extends TestSupport {
  ...
  class Setup(isAgent: Boolean, error: Boolean = false) {
    ...
    val postAction: Call = if (isAgent) controllers.incomeSources.add.routes.AddIncomeSourceStartDateCheckController.submit(SelfEmployment, isAgent = true, isChange = false) else
      controllers.incomeSources.add.routes.AddIncomeSourceStartDateCheckController.submit(SelfEmployment, isAgent = false, isChange = false)
