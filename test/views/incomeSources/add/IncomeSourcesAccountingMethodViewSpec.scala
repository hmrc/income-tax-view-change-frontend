  ...
  class Setup(isAgent: Boolean, incomeSourcePrefix: String, incomeSourceType: String, error: Boolean = false) extends TestSupport {
    ...
    val (backUrl, postAction) = incomeSourceType match {
      case SelfEmployment.key =>
        (controllers.incomeSources.add.routes.AddIncomeSourceStartDateCheckController.show(SelfEmployment, isAgent, isChange = false).url, controllers.incomeSources.add.routes.IncomeSourcesAccountingMethodController.submitAgent (SelfEmployment.key))
      case UkProperty.key =>
        (controllers.incomeSources.add.routes.AddIncomeSourceStartDateCheckController.show(UkProperty, isAgent, isChange = false).url, controllers.incomeSources.add.routes.IncomeSourcesAccountingMethodController.submitAgent(UkProperty.key))
      case _ =>
        (controllers.incomeSources.add.routes.AddIncomeSourceStartDateCheckController.show(ForeignProperty, isAgent, isChange = false).url, controllers.incomeSources.add.routes.IncomeSourcesAccountingMethodController.submitAgent(ForeignProperty.key))
    }
