...
class SignUpTaxYearQuestionViewSpec extends TestSupport {
  ...
  class Setup(viewModel: SignUpTaxYearQuestionViewModel, withError: Boolean = false) {
    ...
    val pageDocument = org.jsoup.Jsoup.parse(contentAsString(signUpTaxYearQuestion(isAgent = true, viewModel, form, controllers.optIn.newJourney.routes.SignUpTaxYearQuestionController.submit(isAgent = true))))
  }
