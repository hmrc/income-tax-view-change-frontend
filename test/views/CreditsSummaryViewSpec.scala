...
class CreditsSummaryViewSpec
  ...
  val creditsDropDownListCreditFomHmrcAdjustment: String = messages(
    "credits.drop-down-list.credit-from-hmrc-adjustment"
  )
  val creditsDropDownListCreditFomHmrcAdjustmentValue: String = messages(
    "credits.drop-down-list.credit-from-hmrc-adjustment.value"
  )
  val creditsDropDownListCreditFromAnEarlierTaxYear: String = messages(
    "credits.drop-down-list.credit-from-an-earlier-tax-year"
  )
  val saNoteMigratedIndividual: String =
    s"${messages("credits.drop-down-list.credit-from-an-earlier-tax-year.sa-note")} ${messages("credits.drop-down-list.sa-link")} ${messages("pagehelp.opensInNewTabText")}."
  val saNoteMigratedAgent: String =
    s"${messages("credits.drop-down-list.credit-from-an-earlier-tax-year.agent.sa-note")} ${messages("credits.drop-down-list.sa-link-agent")} ${messages("pagehelp.opensInNewTabText")}."
  val saNoteMigratedOnlineAccountLink: String = s"/self-assessment/ind/$testMtditid/account"
  val saNoteMigratedOnlineAccountLinkText: String =
    s"${messages("credits.drop-down-list.sa-link")} ${messages("pagehelp.opensInNewTabText")}"
  val saNoteMigratedOnlineAccountAgentLink: String =
    s"https://www.gov.uk/guidance/self-assessment-for-agents-online-service"
  val saNoteMigratedOnlineAccountAgentLinkText: String =
    s"${messages("credits.drop-down-list.sa-link-agent")} ${messages("pagehelp.opensInNewTabText")}"
  ...
  val moneyInYourAccountMoneyClaimARefundLinkText: String = messages(
    "credits.money-in-your-account-section.claim-a-refund-link"
  )
  val moneyInYourAccountContent: String = s"""${messages(
    "credits.money-in-your-account-section.content",
    s"${financialDetailCreditCharge.balanceDetails.availableCredit.get.toCurrencyString}"
  )} ${moneyInYourAccountMoneyClaimARefundLinkText}."""
  val moneyInYourAccountAgentContent: String = s"""${messages(
    "credits.money-in-your-account-section.agent.content",
    s"${financialDetailCreditCharge.balanceDetails.availableCredit.get.toCurrencyString}"
  )} ${moneyInYourAccountMoneyClaimARefundLinkText}."""
  val moneyInYourAccountMoneyClaimARefundLink: String = "/report-quarterly/income-and-expenses/view/claim-refund"
  val moneyInYourAccountMoneyClaimARefundAgentLink: String =
    "/report-quarterly/income-and-expenses/view/agents/claim-refund"

  class TestSetup(
      creditCharges:        List[CreditDetailModel] = List.empty,
  ...
  "display the Credits Summary page" when {
    "a user have multiple credits" in new TestSetup(
      creditCharges = List(
      ...
      document
        .selectById("h2-credit-from-hmrc-adjustment")
        .nextElementSibling()
        .text() shouldBe creditsDropDownListCreditFomHmrcAdjustmentValue
      document
        .selectById("h2-credit-from-an-earlier-tax-year")
        .text() shouldBe creditsDropDownListCreditFromAnEarlierTaxYear
      ...
      new URL(
        document.selectById("sa-note-migrated-online-account-link").attr("href")
      ).getPath shouldBe saNoteMigratedOnlineAccountLink
      ...
      document
        .selectById("money-in-your-account-claim-a-refund-link")
        .text() shouldBe moneyInYourAccountMoneyClaimARefundLinkText
      document
        .selectById("money-in-your-account-claim-a-refund-link")
        .attr("href") shouldBe moneyInYourAccountMoneyClaimARefundLink

      document.getElementsByClass("govuk-link").last().text() shouldBe getPageHelpLinkTextBtn
    }

    "a user has a credit and the Status is Fully allocated" in new TestSetup(
      creditCharges = creditAndRefundCreditDetailListFullyAllocatedMFA
    ) {
    ...
    "a user has a credit and the Status is Not yet allocated" in new TestSetup(
      creditCharges = creditAndRefundCreditDetailListNotYetAllocatedMFA
    ) {
    ...
    "a user has a credit and the Status is Partially allocated" in new TestSetup(
      creditCharges = creditAndRefundCreditDetailListPartiallyAllocatedMFA
    ) {
  ...
  "displaying agent credit and refund page" should {
    "display the page" when {
      "correct data is provided" in new TestSetup(
        creditCharges = creditAndRefundCreditDetailListMultipleChargesMFA,
        isAgent = true,
        maybeAvailableCredit = testMaybeBalanceDetails
      ) {
        ...
        document
          .selectById("balancing-charge-type-0")
          .select("td:nth-child(2)")
          .first()
          .text() shouldBe creditsTableHeadTypeValue
        document
          .selectById("balancing-charge-type-0")
          .select("td:nth-child(3)")
          .first()
          .text() shouldBe creditsTableStatusPartiallyAllocatedValue
        ...
        document
          .selectById("balancing-charge-type-1")
          .select("td:nth-child(2)")
          .first()
          .text() shouldBe creditsTableHeadTypeValue
        document
          ...
          .select("td:nth-child(3)")
          .first()
          .text() shouldBe creditsTableStatusNotYetAllocatedValue
        ...
        document
          .selectById("sa-note-migrated-agent-online-account-link")
          .text() shouldBe saNoteMigratedOnlineAccountAgentLinkText
        document
          .selectById("sa-note-migrated-agent-online-account-link")
          .attr("href") shouldBe saNoteMigratedOnlineAccountAgentLink
        ...
        document
          .selectById("money-in-your-account-claim-a-refund-link")
          .attr("href") shouldBe moneyInYourAccountMoneyClaimARefundAgentLink
