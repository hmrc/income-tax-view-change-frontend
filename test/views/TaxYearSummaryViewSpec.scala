...
class TaxYearSummaryViewSpec extends ViewSpec with FeatureSwitching with ChargeConstants {
  ...
  val modelWithMultipleErrorMessages = modelComplete(Some(false)).copy(messages =
    Some(
      Messages(errors =
        Some(
          List(
  ...
  val testDunningLockChargesList: List[TaxYearSummaryChargeItem] = List(
    TaxYearSummaryChargeItem.fromChargeItem(
      chargeItemModel(transactionType = PoaOneDebit, dueDate = Some(LocalDate.of(2019, 6, 15)), dunningLock = true),
      dueDate = Some(LocalDate.of(2019, 6, 15))
    ),
    TaxYearSummaryChargeItem.fromChargeItem(
      chargeItemModel(transactionType = PoaTwoDebit, dueDate = Some(LocalDate.of(2019, 7, 15)), dunningLock = false),
  ...
  val testChargesList: List[TaxYearSummaryChargeItem] = List(
    ...
    TaxYearSummaryChargeItem.fromChargeItem(
      chargeItemModel(
        ...
        dueDate = Some(LocalDate.of(2019, 7, 15)),
        latePaymentInterestAmount = Some(80.0)
      ),
      dueDate = Some(LocalDate.of(2019, 7, 15)),
      isLatePaymentInterest = true
    ),
  ...
  val testChargesWithoutLpiList: List[TaxYearSummaryChargeItem] =
    testChargesList.map(_.copy(isLatePaymentInterest = false))
  ...
  val class2NicsChargesList: List[TaxYearSummaryChargeItem] = List(
    ...
    chargeItemModel(
      ...
      dueDate = Some(LocalDate.of(2021, 7, 30)),
      latePaymentInterestAmount = None
    )
  ...
  val testCTAModel: TYSClaimToAdjustViewModel =
    TYSClaimToAdjustViewModel(adjustPaymentsOnAccountFSEnabled = true, poaTaxYear = Some(TaxYear(2023, 2024)))
  ...
  def class2NicsView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
  ...
  def estimateViewWithNoCalcData(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        None,
  ...
  def unattendedCalcView(
    ...
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false), unattendedCalc)),
        testChargesList,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
  ...
  def multipleDunningLockView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
        testDunningLockChargesList,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
      ),
  ...
  def crystallisedView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(true))),
        testChargesList,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
  ...
  def payeView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
  ...
  def testBalancingPaymentChargeWithZeroValueView(
      reviewAndReconcileEnabled: Boolean = false,
      isAgent:                   Boolean = false
    ): Html =
    taxYearSummaryView(
      ...
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
        testBalancingPaymentChargeWithZeroValue,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
      ),
  ...
  def immediatelyRejectedByNpsView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
        immediatelyRejectedByNps,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
      ),
  ...
  def rejectedByNpsPartWayView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
        rejectedByNpsPartWay,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
      ),
  ...
  def codingOutPartiallyCollectedView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
        codingOutPartiallyCollected,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
      ),
  ...
  def forecastCalcView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
  ...
  def forecastCalcViewCrystalised(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(true))),
  ...
  def noForecastDataView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(false))),
  ...
  def forecastWithNoCalcData(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        None,
  ...
  def mfaDebitsView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelComplete(Some(true))),
  ...
  def calculationMultipleErrorView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelWithMultipleErrorMessages),
        testChargesList,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
  ...
  def calculationSingleErrorView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html =
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelWithErrorMessages),
        testChargesList,
        testObligationsModel,
        reviewAndReconcileEnabled = reviewAndReconcileEnabled,
        ctaViewModel = emptyCTAModel
  ...
  def poaView(reviewAndReconcileEnabled: Boolean = false, isAgent: Boolean = false): Html = {
    val ctaLink =
      if (isAgent) "/report-quarterly/income-and-expenses/view/agents/adjust-poa/start"
      else "/report-quarterly/income-and-expenses/view/adjust-poa/start"
    taxYearSummaryView(
      testYear,
      TaxYearSummaryViewModel(
        Some(modelWithErrorMessages),
  ...
  "taxYearSummary" when {
    "the user is an individual" should {
      "display forecast data when forecast data present" in new Setup(forecastCalcView()) {
        ...
        document
          .select(".forecast_table tbody tr:nth-child(1) th:nth-child(1) a")
          .attr("href") shouldBe incomeForecastUrl
        ...
        document
          .select(".forecast_table tbody tr:nth-child(4) th:nth-child(1) a")
          .attr("href") shouldBe taxDueForecastUrl
        document.select(".forecast_table tbody tr:nth-child(4) td:nth-child(2)").text() shouldBe "£5,000.99"
        document.select("#inset_forecast").text() shouldBe messagesLookUp(
          "tax-year-summary.forecast_tab.insetText",
          testYear.toString
        )
      }
      ...
      "display the estimate due for an ongoing tax year" in new Setup(estimateView()) {
        ...
        layoutContent.selectHead("dl > div:nth-child(2) > dd:nth-child(2)").text shouldBe modelComplete(
          Some(false)
        ).taxDue.toCurrencyString
      }
      ...
      "when in an ongoing year should display the correct heading in the Tax Calculation tab" in new Setup(
        estimateView()
      ) {
      ...
      "show the unattended calculation info when an unattended calc is returned" in new Setup(
        unattendedCalcView(unattendedCalc = true)
      ) {
        layoutContent.selectHead(".govuk-warning-text").text shouldBe unattendedCalcPara
      }

      "not show the unattended calculation info when the calc returned isn't unattended" in new Setup(
        unattendedCalcView(unattendedCalc = false)
      ) {
      ...
      "display the section header in the Tax Calculation tab" in new Setup(estimateView()) {
        val sectionHeader: Element =
          layoutContent.selectHead(" #income-deductions-contributions-table tr:nth-child(1) th:nth-child(1)")
        sectionHeader.text shouldBe section
      }

      "display the amount header in the Tax Calculation tab" in new Setup(estimateView()) {
        val amountHeader: Element =
          layoutContent.selectHead(" #income-deductions-contributions-table tr:nth-child(1) th:nth-child(2)")
        amountHeader.text shouldBe amount
      }

      "display the income row in the Tax Calculation tab" in new Setup(estimateView()) {
        val incomeLink: Element =
          layoutContent.selectHead(" #income-deductions-contributions-table tr:nth-child(1) th:nth-child(1) a")
        ...
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(1) td:nth-child(2)")
          .text shouldBe modelComplete(Some(false)).income.toCurrencyString
      }

      "when there is no calc data should display the correct heading in the Tax Calculation tab" in new Setup(
        estimateViewWithNoCalcData()
      ) {
        layoutContent.selectHead("#taxCalculation > h2").text shouldBe taxCalculationNoData
      }

      "when there is no calc data should display the correct notes in the Tax Calculation tab" in new Setup(
        estimateViewWithNoCalcData()
      ) {
        layoutContent.selectHead("#taxCalculation > p").text shouldBe taxCalculationNoDataNote
      }

      "when there is no calc data should display the correct heading in the Forecast tab" in new Setup(
        forecastWithNoCalcData()
      ) {
        layoutContent.getElementById("no-forecast-data-header").text shouldBe forecastNoData
      }

      "when there is no calc data should display the correct notes in the Forecast tab" in new Setup(
        forecastWithNoCalcData()
      ) {
      ...
      "when there are multiple error messages from calculation" in new Setup(calculationMultipleErrorView()) {
        ...
        errorMessageList.child(0).text shouldBe messages(
          "tax-year-summary.message.C15014",
          dateService.getCurrentDate.toLongDate
        )
        errorMessageList.child(1).text shouldBe messages(
          "tax-year-summary.message.C55014",
          dateService.getCurrentDate.toLongDate
        )
        ...
        errorMessageList
          .child(6)
          .text shouldBe s"${messages("tax-year-summary.message.C15104.1")} ${messages("tax-year-summary.message.C15104.2")} ${messages("tax-year-summary.message.C15104.3")}"
        errorMessageList.child(7).text shouldBe messages("tax-year-summary.message.C15105")
        errorMessageList.child(8).text shouldBe s"${messages("tax-year-summary.message.C15322.1")} ${messages(
          "tax-year-summary.message.C15322.2"
        )} ${messages("tax-year-summary.message.C15322.3")} ${messages("tax-year-summary.message.C15322.4")}"
        ...
        errorMessageList.child(30).text shouldBe messages(
          "tax-year-summary.message.C55008",
          dateService.getCurrentDate.toLongDate
        )
        errorMessageList.child(31).text shouldBe messages(
          "tax-year-summary.message.C55011",
          dateService.getCurrentDate.toLongDate
        )
        ...
        errorMessageList.child(34).text shouldBe messages(
          "tax-year-summary.message.C55012",
          dateService.getCurrentDate.toLongDate
        )
        errorMessageList.child(35).text shouldBe messages(
          "tax-year-summary.message.C55013",
          dateService.getCurrentDate.toLongDate
        )
        ...
        errorMessageList.child(51).text shouldBe s"${messages("tax-year-summary.message.C159028.1")} ${messages(
          "tax-year-summary.message.C159028.2"
        )} ${messages("tax-year-summary.message.C159028.3")} ${messages("tax-year-summary.message.C159028.4")}"
      ...
      "display the Allowances and deductions row in the Tax Calculation tab" in new Setup(estimateView()) {
        val allowancesLink: Element =
          layoutContent.selectHead(" #income-deductions-contributions-table tr:nth-child(2) th:nth-child(1) a")
        allowancesLink.text shouldBe allowancesAndDeductions
        allowancesLink
          .attr("href") shouldBe controllers.routes.DeductionsSummaryController.showDeductionsSummary(testYear).url
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(2) td:nth-child(2)")
          .text shouldBe "£2.02"
      }

      "display the Total income on which tax is due row in the Tax Calculation tab" in new Setup(estimateView()) {
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(3) th:nth-child(1)")
          .text shouldBe totalIncomeDue
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(3) td:nth-child(2)")
          .text shouldBe modelComplete(Some(false)).totalTaxableIncome.toCurrencyString
      }

      "display the Income Tax and National Insurance Contributions Due row in the Tax Calculation tab" in new Setup(
        estimateView()
      ) {
        val totalTaxDueLink: Element =
          layoutContent.selectHead(" #income-deductions-contributions-table tr:nth-child(4) th:nth-child(1) a")
        ...
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(4) td:nth-child(2)")
          .text shouldBe modelComplete(Some(false)).taxDue.toCurrencyString
      }
      ...
      "display the payment type as a link to Charge Summary in the Payments tab" in new Setup(
        estimateView(chargeItems = testChargesWithoutLpiList)
      ) {
        ...
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .show(testYear, fullDocumentDetailModel.transactionId)
          .url
      }
      ...
      "display no payments due when there are no charges in the payments tab" in new Setup(
        estimateView(emptyChargeList)
      ) {
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest POA1" in new Setup(
        estimateView()
      ) {
        val paymentTypeLink: Element = layoutContent.selectHead("#payments-table tr:nth-child(1) a")
        paymentTypeLink.text shouldBe lpiPaymentOnAccount1
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .show(testYear, fullDocumentDetailModel.transactionId, true)
          .url
      }
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest POA2" in new Setup(
        estimateView()
      ) {
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest Balancing payment" in new Setup(
        estimateView()
      ) {
      ...
      "display the Due date in the Payments tab for late payment interest Balancing payment" in new Setup(
        estimateView()
      ) {
        layoutContent.selectHead("#payments-table tr:nth-child(3) td:nth-child(2)").text shouldBe "15 Aug 2019"
      }
      ...
      "display the Dunning lock subheading in the payments tab for multiple lines POA1 and Balancing payment" in new Setup(
        multipleDunningLockView()
      ) {
        layoutContent
          .selectHead("#payments-table tbody tr:nth-child(1) div:nth-child(3)")
          .text shouldBe paymentUnderReview
        layoutContent
          .selectHead("#payments-table tbody tr:nth-child(3) div:nth-child(3)")
          .text shouldBe paymentUnderReview
      ...
      "display the PAYE Self Assessment link on the payments table when coding out is enabled" in new Setup(
        payeView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
      ...
      "display Class 2 National Insurance - User has Coding out that is requested and immediately rejected by NPS" in new Setup(
        immediatelyRejectedByNpsView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Balancing payment - User has Coding out that is requested and immediately rejected by NPS" in new Setup(
        immediatelyRejectedByNpsView()
      ) {
      ...
      "display Class 2 Nics - User has Coding out that has been accepted and rejected by NPS part way through the year" in new Setup(
        rejectedByNpsPartWayView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Cancelled Self Assessment payment - User has Coding out that has been accepted and rejected by NPS part way through the year" in new Setup(
        rejectedByNpsPartWayView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-1")
      ...
      "display Class 2 National Insurance - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected)" in new Setup(
        codingOutPartiallyCollectedView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Balancing payment - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected)" in new Setup(
        codingOutPartiallyCollectedView()
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-1")
        paymentTypeLink.text shouldBe remainingBalance
      ...
      "display Cancelled Self Assessment payment - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected)" in new Setup(
        codingOutPartiallyCollectedView()
      ) {
      ...
      "display the Balancing payment on the payments table when coding out is enabled and a zero amount" in new Setup(
        testBalancingPaymentChargeWithZeroValueView()
      ) {
        ...
        val paymentTabRow: Element =
          layoutContent.getElementById("payments-table").getElementsByClass("govuk-table__row").get(1)
      ...
      "display updates by due-date" in new Setup(estimateView()) {
        ...
        testObligationsModel
          .allDeadlinesWithSource(previous = true)
          .groupBy[LocalDate] { nextUpdateWithIncomeType =>
            nextUpdateWithIncomeType.obligation.due
          }
          .toList
          .sortBy(_._1)(localDateOrdering)
          .reverse
          .foreach {
            case (due: LocalDate, obligations: Seq[ObligationWithIncomeType]) =>
              layoutContent.selectHead(s"#table-default-content-$due").text shouldBe messages(
                "updateTab.due",
                due.toLongDate
              )
              ...
              obligations.zip(1 to obligations.length).foreach {
                case (testObligation, index) =>
                  ...
                  divAccordion.selectHead("caption").text shouldBe
                    "Quarterly period from " + messages(
                      "updateTab.dateToDate",
                      testObligation.obligation.start.toLongDateShort,
                      testObligation.obligation.end.toLongDateShort
                    )
                  ...
                  row
                    .selectNth("td", 2)
                    .text shouldBe testObligation.obligation.dateReceived.map(_.toLongDateShort).getOrElse("")
              }
          }
      }
    }
    "the user is an agent" should {
      ...
      "display No forecast data yet when calcData returns NOT_FOUND" in new Setup(
        forecastWithNoCalcData(isAgent = true)
      ) {
        document.title shouldBe agentTitle
        document.getOptionalSelector("#forecast").isDefined shouldBe true
        assert(document.select(hrefForecastSelector).text.contains(messagesLookUp("tax-year-summary.forecast")))
      ...
      "display the Allowances and deductions row in the Tax Calculation tab" in new Setup(
        ...
        allowancesLink.text shouldBe allowancesAndDeductions
        allowancesLink
          .attr("href") shouldBe controllers.routes.DeductionsSummaryController.showDeductionsSummaryAgent(testYear).url
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(2) td:nth-child(2)")
          .text shouldBe "£2.02"
      }
      ...
      "display the Income Tax and National Insurance Contributions Due row in the Tax Calculation tab" in new Setup(
        ...
        totalTaxDueLink
          .attr("href") shouldBe controllers.routes.TaxDueSummaryController.showTaxDueSummaryAgent(testYear).url
        layoutContent
          .selectHead("#income-deductions-contributions-table tr:nth-child(4) td:nth-child(2)")
          .text shouldBe modelComplete(Some(false)).taxDue.toCurrencyString
      ...
      "display the payment type as a link to Charge Summary in the Payments tab" in new Setup(
        estimateView(chargeItems = testChargesWithoutLpiList, isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.selectHead("#payments-table tr:nth-child(1) a")
        paymentTypeLink.text shouldBe paymentOnAccount1
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .showAgent(testYear, fullDocumentDetailModel.transactionId)
          .url
      }
      ...
      "display the payment type as a link to Charge Summary in the Payments tab for late payment interest POA1" in new Setup(
        ...
        paymentTypeLink.text shouldBe lpiPaymentOnAccount1
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .showAgent(testYear, fullDocumentDetailModel.transactionId, true)
          .url
      }
      ...
      "display Class 2 National Insurance - User has Coding out that is requested and immediately rejected by NPS - Agent" in new Setup(
        immediatelyRejectedByNpsView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Balancing payment - User has Coding out that is requested and immediately rejected by NPS - Agent" in new Setup(
        immediatelyRejectedByNpsView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-1")
        paymentTypeLink.text shouldBe remainingBalance
      ...
      "display Class 2 Nics - User has Coding out that has been accepted and rejected by NPS part way through the year - Agent" in new Setup(
        rejectedByNpsPartWayView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Cancelled Self Assessment payment - User has Coding out that has been accepted and rejected by NPS part way through the year - Agent" in new Setup(
        rejectedByNpsPartWayView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-1")
        paymentTypeLink.text shouldBe cancelledPaye
      ...
      "display Class 2 National Insurance - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected) - Agent" in new Setup(
        codingOutPartiallyCollectedView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-0")
        paymentTypeLink.text shouldBe taxYearSummaryClass2Nic
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
      ...
      "display Balancing payment - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected) - Agent" in new Setup(
        codingOutPartiallyCollectedView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-1")
        paymentTypeLink.text shouldBe remainingBalance
      ...
      "display Cancelled Self Assessment payment - At crystallization, the user has the coding out requested amount has not been fully collected (partially collected) - Agent" in new Setup(
        codingOutPartiallyCollectedView(isAgent = true)
      ) {
        val paymentTypeLink: Element = layoutContent.getElementById("paymentTypeLink-2")
        paymentTypeLink.text shouldBe cancelledPaye
      ...
      "display MFA Debits - Individual" in new Setup(mfaDebitsView(isAgent = false)) {
        ...
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .show(testYear, MFADebitsDocumentDetailsWithDueDates.head.documentDetail.transactionId)
          .url
      }
      ...
      "display MFA Debits - Agent" in new Setup(mfaDebitsView(isAgent = true)) {
        ...
        paymentTypeLink.text shouldBe hmrcAdjustment
        paymentTypeLink.attr("href") shouldBe controllers.routes.ChargeSummaryController
          .showAgent(testYear, MFADebitsDocumentDetailsWithDueDates.head.documentDetail.transactionId)
          .url
      }
