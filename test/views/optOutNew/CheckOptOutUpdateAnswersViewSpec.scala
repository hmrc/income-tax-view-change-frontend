...
import views.messages.{CheckOptOutUpdateAnswersMesasges => viewMessages}
...
class CheckOptOutUpdateAnswersViewSpec extends TestSupport {
  ...
  val confirmOptOutUpdateSubmitURL: (Boolean, String) => String = (isAgent: Boolean, taxYear: String) => controllers.optOutNew.routes.ConfirmOptOutUpdateController.submit(isAgent, taxYear).url
      ...
      Jsoup.parse(
        contentAsString(
          checkOptOutUpdateAnswersView(
            viewModel = CheckOptOutUpdateAnswersViewModel(TaxYear(2025, 2026)),
            isAgent = isAgent,
