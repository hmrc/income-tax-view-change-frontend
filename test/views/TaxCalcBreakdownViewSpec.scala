...
class TaxCalcBreakdownViewSpec extends TaxCalcBreakdownViewBehaviour {
  ...
  override def taxCalcBreakdown(
      taxDueSummaryViewModel: TaxDueSummaryViewModel,
      taxYear:                Int,
      backUrl:                String,
      isAgent:                Boolean
    ): Html =
...
abstract class TaxCalcBreakdownViewBehaviour extends ViewSpec {
  ...
  val sectionHeadingNationalInsuranceContributionsChar: String = messages(
    "taxCal_breakdown.national_insurance_contributions"
  )
  ...
  def shouldHaveACorrectTableContent(
      view: Html
    )(
      tableNumber:       Int,
      expectedCaption:   String,
      expectedTableRows: BREAKDOWN_TABLE
    ): Unit = {
  ...
  "The taxCalc breakdown view with UK tax regime" when {
    ...
    "provided with a calculation that is without Pay, pensions and profit, without Savings, without Dividends, " +
      ...
      "have the correct heading" in new Setup(view) {
        ...
        pageContent(pageContentSelector).h1.select(headingSelector).text() shouldBe messages(
          "taxCal_breakdown.dates",
          s"${taxYear2017 - 1}",
          s"$taxYear2017"
        )
      }
    ...
    "provided with a calculation with all the sections displayed which includes Pay, pensions and profit, Savings, Dividends," +
      ...
      lazy val viewMarriageAllowanceTransfer =
        taxCalcBreakdown(taxDueSummaryViewModelMarriageAllowance, taxYear, backUrl)
      ...
      "have no Savings and Gains Income" which {
        "has no Savings and Gains heading" in new Setup(zeroIncome) {
          pageContent(pageContentSelector).select("caption").text should not include messages(
            "taxCal_breakdown.savings"
          )
        }
      }
      ...
      "have a Tax reductions table" which {
        ...
        shouldHaveACorrectTableContent(view)(
          ...
          expectedTableRows = Table(
            ...
            (
              13,
              messages("taxCal_breakdown.table.qualifyingDistributionRedemptionOfSharesAndSecurities"),
              "−£8,000.00"
            ),
      ...
      "have no National Insurance contributions table and heading when there is no Nic4 or Nic2 data" in new Setup(
        viewNone
      ) {
        pageContent(pageContentSelector)
          .select("caption")
          .text should not include sectionHeadingNationalInsuranceContributionsChar
      }

      "have no Capital Gains Tax table and heading when there is no any CGT value" in new Setup(zeroIncome) {
        pageContent(pageContentSelector).select("caption").text should not include messages(
          "taxCal_breakdown.table.capital_gains_tax"
        )
      }
      ...
      "have a Capital Gains Tax table" which {
        ...
        shouldHaveACorrectTableContent(view)(
          ...
          expectedTableRows = Table(
            ...
            (
              2,
              messages("taxCal_breakdown.table.cgt.assets_or_investors_relief.band.single", "£10,000.00", "10.0"),
              "£1,000.00"
            ),
            (
              3,
              messages("taxCal_breakdown.table.cgt.property_and_interest.band.lowerRate", "£20,000.00", "18.0"),
              "£3,600.00"
            ),
            (
              4,
              messages("taxCal_breakdown.table.cgt.property_and_interest.band.higherRate", "£30,000.00", "28.0"),
              "£8,400.00"
            ),
  ...
  "The tax calc view should display messages" when {
    ...
    "provided with all matching generic messages" in {
      lazy val view = taxCalcBreakdown(
    ...
    "provided with message C22201" in {
      ...
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.C22201",
        "£5,000.98",
        "£15,000.00"
      )
    }
    ...
    "provided with message C22205" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.C22205",
        "£1,000.00"
      )
    }
    ...
    "provided with message C22208" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.C22208",
        "£5,000.99",
        "£15,000.00"
      )
    }
    ...
    "provided with message C22209" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.C22209",
        "£5,000.99",
        "£15,000.00"
      )
    }
  ...
  "The tax calc view should display messages for agent" when {
    ...
    "provided with message C22201" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.agent.C22201",
        "£5,000.98",
        "£15,000.00"
      )
    }
    ...
    "provided with message C22205" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.agent.C22205",
        "£1,000.00"
      )
    }
    ...
    "provided with message C22208" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.agent.C22208",
        "£5,000.99",
        "£15,000.00"
      )
    }
    ...
    "provided with message C22209" in {
      ...
      document.select(messageContentSelector).size shouldBe 2
      document.select(messageContentSelector).get(1).text shouldBe messages(
        "taxCal_breakdown.message.agent.C22209",
        "£5,000.99",
        "£15,000.00"
      )
    }
  ...
  "The taxCalc breakdown view with Self Assessment tax amount" when {
    ...
    "have the correct Self Assessment tax amount" in new Setup(view) {
      pageContent(pageContentSelector)
        ...
        .get(11)
        .text() shouldBe "Self Assessment tax amount £0.00"
    }
  }
