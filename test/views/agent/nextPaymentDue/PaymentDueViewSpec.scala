...
class PaymentDueViewSpec extends TestSupport with FeatureSwitching with ImplicitDateFormatter {
  ...
  val financialDetailsDueInMoreThan30Days: FinancialDetailsModel = {
    testFinancialDetailsModel(List(Some("SA Payment on Account 1"), Some("SA Payment on Account 2")),
  ...
  val whatYouOweDataWithMixedData: WhatYouOweChargesList = WhatYouOweChargesList(overduePaymentList = List(financialDetailsWithMixedData.financialDetails(2)),
    dueInThirtyDaysList = List(financialDetailsWithMixedData.financialDetails(1)), futurePayments = List(financialDetailsWithMixedData.financialDetails.head))
