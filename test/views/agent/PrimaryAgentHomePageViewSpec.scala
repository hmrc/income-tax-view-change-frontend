...
class PrimaryAgentHomePageViewSpec extends TestSupport with FeatureSwitching with ViewSpec {
  ...
  val testMtdItUserNotMigrated: MtdItUser[_] =
    defaultMTDITUser(Some(Agent), IncomeSourceDetailsModel(testNino, testMtditid, None, Nil, Nil))

  val testMtdItUserMigrated: MtdItUser[_] =
    defaultMTDITUser(Some(Agent), IncomeSourceDetailsModel(testNino, testMtditid, Some("2018"), Nil, Nil))

  val testMtdItUserNoClientName =
    getMinimalMTDITUser(Some(Agent), IncomeSourceDetailsModel(testNino, testMtditid, Some("2018"), Nil, Nil))
  ...
  "home" when {
    ...
    "all features are enabled" should {
        ...
        "has content of the next payment due" which {
          "is overdue" in new TestSetup(nextPaymentDueDate = Some(nextPaymentDue), overDuePaymentsCount = 1) {
            getElementById("payments-tile").map(_.select("p:nth-child(2)").text) shouldBe Some(
              s"OVERDUE 31 January $year2019"
            )
          }
          "has payments accruing interest" in new TestSetup(
            nextPaymentDueDate = Some(nextPaymentDue),
          ...
          "is a count of overdue payments" in new TestSetup(
            nextPaymentDueDate = Some(nextPaymentDue),
      ...
      "dont display any warning message when no payment is overdue, and no payments are accruing interest" in new TestSetup(
      ...
      "display an overdue warning message when a payment is overdue and dunning lock does not exist" in new TestSetup(
        overDuePaymentsCount = 1
      ) {
        val overdueMessageWithoutDunningLock =
      ...
      "display an overdue warning message when a payment is overdue and dunning lock exists" in new TestSetup(
        overDuePaymentsCount = 1,
        dunningLockExists = true
      ) {
        val overdueMessageWithDunningLock =
          "! Warning Your client has overdue payments and one or more of their tax decisions are being reviewed. They may be charged interest on these until they are paid in full."
        getTextOfElementById("overdue-warning") shouldBe Some(overdueMessageWithDunningLock)
      }

      "display daily interest warning when payments are accruing interest" in new TestSetup(
        paymentsAccruingInterestCount = 2,
        reviewAndReconcileEnabled = true
      ) {
      ...
      "have a returns tile" which {
        ...
        "has a link to the view payments page" in new TestSetup {
          ...
          link.map(_.attr("href")) shouldBe Some(
            controllers.routes.TaxYearSummaryController.renderAgentTaxYearSummaryPage(currentTaxYear).url
          )
        ...
        "has a link to the update and submit page" in new TestSetup {
          ...
          link.map(_.text) shouldBe Some(
            s"Update and submit your client’s ${currentTaxYear - 1} to $currentTaxYear return"
          )
        }
        "dont have a link to the update and submit page when ITSASubmissionIntegrationEnabled is disabled" in new TestSetup(
          ...
          link.map(_.text) should not be Some(
            s"Update and submit your ${currentTaxYear - 1} to ${currentTaxYear} return"
          )
        }
      ...
      "have a payment history tile" which {
        ...
        "has payment history heading when payment history feature switch is disabled" in new TestSetup(
          paymentHistoryEnabled = false
        ) {
          getElementById("payment-history-tile").map(_.select("h2").text) shouldBe Some("Payment history")
        }

        "has payment and refund history link when CreditsRefundsRepay OFF / PaymentHistoryRefunds ON" in new TestSetup(
        ...
        "has payment and credit history link when CreditsRefundsRepay ON / PaymentHistoryRefunds OFF" in new TestSetup(
        ...
        "has payment, credit and refund history link when CreditsRefundsRepay ON / PaymentHistoryRefunds ON" in new TestSetup(
          creditAndRefundEnabled = true,
          paymentHistoryEnabled = true
        ) {
          val link: Option[Element] = getElementById("payment-history-tile").map(_.select("a").first)
        ...
        "has payment history link when CreditsRefundsRepay OFF / PaymentHistoryRefunds OFF" in new TestSetup(
          paymentHistoryEnabled = false,
          creditAndRefundEnabled = false
        ) {
          val link: Option[Element] = getElementById("payment-history-tile").map(_.select("a").first)
    ...
    "the feature switches are disabled" should {
      ...
      "not have a link to previous payments in the tax years tile" in new TestSetup(
        ITSASubmissionIntegrationEnabled = false
      ) {
    ...
    "the feature switches are enabled" should {
      ...
      "dont display the saViewLandPTile when isAgent is true" in new TestSetup(
        ITSASubmissionIntegrationEnabled = true
      ) {
        getElementById("saViewLandPTile") shouldBe None
      }

      "not have a link to the saViewLandPTile when isAgent and UTR is present" in new TestSetup(
        ITSASubmissionIntegrationEnabled = true
      ) {
        val link: Option[Elements] = getElementById("saViewLandPTile").map(_.select("a"))
        link.map(_.attr("href")) shouldBe None
      ...
      "dont display the saViewLandPTile when isAgent is true and UTR is present" in new TestSetup(
        ITSASubmissionIntegrationEnabled = true
      ) {
        getElementById("saViewLandPTile") shouldBe None
      }
      ...
      "have a Penalties and Appeals tile" when {
        "PenaltiesAndAppeals FS is enabled" which {
          ...
          "has a link to Self Assessment Penalties and Appeals page" in new TestSetup(
            penaltiesAndAppealsIsEnabled = true
          ) {
          ...
          "has no penalty tag if 2 points reached but User is reporting Quarterly" in new TestSetup(
