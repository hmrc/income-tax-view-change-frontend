        ...
        testPayments.groupBy { payment => LocalDate.parse(payment.date.get).getYear }.map { case (year, payments) =>
          ...
          payments.zipWithIndex.foreach {
            case (payment, index) =>
              ...
              row.selectNth("td", 2).childNode(0).attr("href") shouldBe s"/report-quarterly/income-and-expenses/view/agents/charges/payments-made?documentNumber=${payment.transactionId.get}"
