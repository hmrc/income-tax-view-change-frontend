  ...
  "TaxYearOverview" should {
    ...
    "not display the information about an estimate calculation" when {
      "the calculation is crystallised" in new Setup(view()) {
        layoutContent.select("div.panel").headOption shouldBe None
//        content.select("div.panel").toList.headOption shouldBe None
    ...
    "have tabs to display information about the user's tax year" which {
      "has a contents label" in new Setup(view()) {
        layoutContent.selectHead("div.govuk-tabs").selectHead("h2.govuk-tabs__title").text shouldBe TaxYearOverviewMessages.contents
      }
      "has a calculation tab" which {
        "has a tab label" in new Setup(view()) {
          val tab: Element = layoutContent.selectHead("div.govuk-tabs").selectHead("ul").selectNth("li", 1).selectHead("a")
        ...
        "has the tab contents" which {
          "has different headings" when {
            ...
            "there is no calculation data" in new Setup(view(overview = None)) {
              layoutContent.selectHead("#taxCalculation").selectHead("h2").text shouldBe TaxYearOverviewMessages.taxCalculationNoData
            }
          }
          "has a paragraph" when {
            "there is no calculation data" in new Setup(view(overview = None)) {
              layoutContent.selectHead("#taxCalculation").selectHead("p").text shouldBe TaxYearOverviewMessages.taxCalculationNoDataNote
            }
          }
          "has a table detailing a users income calculation" which {
            ...
            "has a row for the user's income" in new Setup(view()) {
              ...
              layoutContent.selectHead("#income-deductions-table tr:nth-child(1) td:nth-child(2)").text shouldBe "£150.00"
            }
            "has a row for the user's allowances and deductions" in new Setup(view()) {
              val row: Element = layoutContent.selectHead("#taxCalculation").selectNth("table", 1).selectNth("tr", 2)
            ...
            "has a row for the total income on which tax is due" in new Setup(view()) {
              val row: Element = layoutContent.selectHead("#taxCalculation").selectNth("table", 1).selectNth("tr", 3)
          ...
          "has a table details a users tax calculation" in new Setup(view()) {
            val row: Element = layoutContent.selectHead("#taxCalculation").selectNth("table", 2).selectNth("tr", 1)
      ...
      "has a payments tab" which {
        "has a tab label" in new Setup(view()) {
          val tab: Element = layoutContent.selectHead("div.govuk-tabs").selectHead("ul").selectNth("li", 2).selectHead("a")
        ...
        "has a tab contents" which {
          "has a heading" in new Setup(view()) {
            layoutContent.selectHead("#payments").selectHead("h2").text shouldBe TaxYearOverviewMessages.paymentsTabHeading
          }
          "has a caption" in new Setup(view()) {
            layoutContent.selectHead("#payments").selectHead(" caption").text.contains(TaxYearOverviewMessages.paymentsTabLabel)
          }
          "has no payments due content" when {
            "no payments are due" in new Setup(view(documentDetailsWithDueDates = Nil)) {
              layoutContent.selectHead("#payments").selectHead("p").text shouldBe TaxYearOverviewMessages.paymentsTabNoPayments
              layoutContent.selectHead("#payments").selectHead("h2").text shouldBe TaxYearOverviewMessages.paymentsTabHeading
          ...
          "has a table of the payments for the year" which {
            "has table headings" in new Setup(view()) {
              val row: Element = layoutContent.selectHead("#payments").selectHead("table").selectNth("tr", 1)
      ...
      "has an updates tab" which {
        "has a tab label" in new Setup(view()) {
          val tab: Element = layoutContent.selectHead("div.govuk-tabs").selectHead("ul").selectNth("li", 3).selectHead("a")
        ...
        "has a tab contents" which {
          "has a heading" in new Setup(view()) {
            layoutContent.selectHead("#updates").selectHead("h2").text shouldBe TaxYearOverviewMessages.updatesTabHeading
          }
          "has all obligations grouped by their due date" which {
            "has a first grouped obligations" which {
              "has a heading of the due date for those obligations" in new Setup(view()) {
                layoutContent.selectHead("#updates").selectNth("div", 1).selectHead("h3").text shouldBe TaxYearOverviewMessages.updatesTabUpdatesDue("31 March 2020")
              }
              "has a table of the obligations in that group" which {
                "has a caption of the start and end dates" in new Setup(view()) {
                  layoutContent.selectHead("#updates").selectNth("div", 1).selectHead("table")
                ...
                "has table headings for each table column" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 1).selectHead("table").selectHead("thead").selectHead("tr")
                ...
                "has the first obligation (quarterly, non submitted) for the users property" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 1).selectHead("table").selectHead("tbody").selectNth("tr", 1)
                ...
                "has the second obligation (quarterly, submitted) for the users business" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 1).selectHead("table").selectHead("tbody").selectNth("tr", 2)
                  row.selectNth("td", 1).text shouldBe TaxYearOverviewMessages.updatesTabQuarterlyUpdate
            ...
            "has the second grouped obligations" which {
              "has a heading of the due date for those obligations" in new Setup(view()) {
                layoutContent.selectHead("#updates").selectNth("div", 2).selectHead("h3").text shouldBe TaxYearOverviewMessages.updatesTabUpdatesDue("30 June 2020")
              }
              "has a table of the obligations in that group" which {
                "has a caption of the start and end dates" in new Setup(view()) {
                  layoutContent.selectHead("#updates").selectNth("div", 2).selectHead("table")
                    .selectHead("caption").text shouldBe TaxYearOverviewMessages.updatesTabCaption("1 April 2020", "30 June 2020")
                }
                "has table headings for each table column" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 2).selectHead("table").selectHead("thead").selectHead("tr")
                  row.selectNth("th", 1).text shouldBe TaxYearOverviewMessages.updatesTabUpdateTypeHeading
                  row.selectNth("th", 2).text shouldBe TaxYearOverviewMessages.updatesTabIncomeSourceHeading
                  row.selectNth("th", 3).text shouldBe TaxYearOverviewMessages.updatesTabDateSubmittedHeading
                }
                "has the first obligation (annual, submitted) for the users business" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 2).selectHead("table").selectHead("tbody").selectNth("tr", 1)
                ...
                "has the second obligation (final declaration, submitted) for the user" in new Setup(view()) {
                  val row: Element = layoutContent.selectHead("#updates").selectNth("div", 2).selectHead("table").selectHead("tbody").selectNth("tr", 2)
