  ...
  "The BtaPartialHelper's whichStatus method" should {
    "return Html corresponding to the ObligationStatus" when {
      ...
      "passed 'Open' the Open Html is returned" in {
        BtaPartialHelper.whichStatus(openObligation)(applicationMessages, mockAppConfig) shouldBe openHtml
      }
      "passed 'Overdue' the Overdue Html is returned" in {
        BtaPartialHelper.whichStatus(overdueObligation)(applicationMessages, mockAppConfig) shouldBe overdueHtml
      }
      "passed 'Received' the Received Html is returned" in {
        BtaPartialHelper.whichStatus(receivedObligation)(applicationMessages, mockAppConfig) shouldBe receivedHtml
      }
    }
  }
  ...
  "The BtaPartialHelper's showLastEstimate method" should {
    "return Html corresponding to the estimated tax for a single tax year" when {
      ...
      "passed a number the Html is returned" in {
        BtaPartialHelper.showLastEstimate(List(lastTaxCalcSuccessWithYear))(applicationMessages, mockAppConfig) shouldBe List(successHtml)
      }
    }
    ...
    "return Html corresponding to the estimated tax for both tax years" when {
      ...
      "return Html corresponding to the estimated tax for misaligned tax years" in {
        BtaPartialHelper.showLastEstimate(List(lastTaxCalcSuccessWithYear, LastTaxCalculationWithYear(LastTaxCalculation("CALCID","2018-07-06T12:34:56.789Z", 6543.21), 2019)))(applicationMessages, mockAppConfig) shouldBe List(successHtml1,successHtml2)
      }
    }
    ...
    "return Html with an estimate for 2018 tax year and no estimate for 2019 tax year" when {
      ...
      "" in {
        BtaPartialHelper.showLastEstimate(List(lastTaxCalcSuccessWithYear, LastTaxCalculationWithYear(NoLastTaxCalculation, 2019)))(applicationMessages, mockAppConfig) shouldBe List(successHtml,noCalcHtml)
      }
    }
  }
