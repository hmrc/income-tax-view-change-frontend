...
class ConfirmedOptOutViewSpec extends TestSupport {
  ...
  val submitYourTaxReturnContent = confirmedOptOutViewUtils.submitYourTaxReturnContent(
    Voluntary,
    Voluntary,
    Voluntary,
    NextTaxYear,
  ...
  class Setup(
    ...
    val pageDocument: Document = Jsoup.parse(
      contentAsString(
        confirmedOptOutView(viewModel, isAgent, showReportingFrequencyContent, submitYourTaxReturnContent)
      )
    )
  }
  ...
  "Opt-out confirmed page" should {
    ...
    "have the correct confirmation panel content" when {
      ...
      "one year opt out is followed by mandated ITSA Status" in new Setup(
        isAgent = false,
        state = OneYearOptOutFollowedByMandated,
        showReportingFrequencyContent = false
      ) {
      ...
      "multi year opt out" in new Setup(
        isAgent = false,
    ...
    "have the correct submit your tax return content" in new Setup(
      isAgent = false,
      showReportingFrequencyContent = false
    ) {
    ...
    "Individual - revised deadlines content " in new Setup(isAgent = false, showReportingFrequencyContent = false) {
      ...
      revisedDeadlinesBlock
        .getElementById("revised-deadlines-heading")
        .text() shouldBe ConfirmedOptOutMessages.yourRevisedDeadlinesHeading
      revisedDeadlinesBlock
        .getElementById("revised-deadlines-p1")
        .text() shouldBe ConfirmedOptOutMessages.yourRevisedDeadlinesContentP1
      revisedDeadlinesBlock.getElementById("your-reporting-frequency-block").text shouldBe ""
      revisedDeadlinesBlock
        .getElementById("view-upcoming-updates-link")
        .attr("href") shouldBe controllers.routes.NextUpdatesController.show().url
    }
    ...
    "Agent - revised deadlines content" in new Setup(isAgent = true, showReportingFrequencyContent = false) {
      ...
      revisedDeadlinesBlock
        .getElementById("view-upcoming-updates-link")
        .attr("href") shouldBe controllers.routes.NextUpdatesController.showAgent.url
    }

    "Individual - revised deadlines with reporting frequency content" in new Setup(
      isAgent = false,
      ...
      revisedDeadlinesBlock
        ...
        .text() shouldBe ConfirmedOptOutMessages.yourRevisedDeadlinesContentP1
      revisedDeadlinesBlock
        .getElementById("your-reporting-frequency-block")
        .text() shouldBe ConfirmedOptOutMessages.yourRevisedDeadlinesContentP2
      revisedDeadlinesBlock
        .getElementById("view-upcoming-updates-link")
        .attr("href") shouldBe controllers.routes.NextUpdatesController.show().url
    }

    "Agent - revised deadlines with reporting frequency content" in new Setup(
      isAgent = true,
      showReportingFrequencyContent = true
    ) {
      val revisedDeadlinesBlock: Element = pageDocument.getElementById("revised-deadlines")
    ...
    "have the correct reporting updates content" when {
      ...
      "one year opt out is followed by mandated ITSA Status" in new Setup(
        ...
        reportingUpdateBlock
          .getElementById("software-compatible-ext")
          .attr("href") shouldBe ConfirmedOptOutMessages.singleYearSoftwareCompatibleLink
        reportingUpdateBlock
          .getElementById("sign-up-criteria-ext")
          .attr("href") shouldBe ConfirmedOptOutMessages.multiYearReportingUpdatesP3Link
      }
