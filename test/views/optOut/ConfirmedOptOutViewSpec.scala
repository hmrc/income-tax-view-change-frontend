...
import models.optout.{ConfirmedOptOutViewModel, Scenario1Content}
...
class ConfirmedOptOutViewSpec extends TestSupport {
  ...
  "ConfirmedOptOutView" when {
    ...
    "testing for Scenario1Content" when {

      "return multi year opt out" in {

        val viewModel = ConfirmedOptOutViewModel(optOutTaxYear = optOutTaxYear.taxYear, state = Some(OneYearOptOutFollowedByMandated))
        val pageDocument = Jsoup.parse(contentAsString(confirmedOptOutView(viewModel, isAgent = false, showReportingFrequencyContent = true, Scenario1Content)))

        val reportingUpdateBlock: Element = pageDocument.getElementById(Selectors.reportingUpdateBlock)
