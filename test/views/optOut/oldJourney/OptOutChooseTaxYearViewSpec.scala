...
class OptOutChooseTaxYearViewSpec extends TestSupport {
  ...
  object optOutChooseTaxYear {
    ...
    val confirmOptOutURL: String =
      if(areAllEnabled(OptOutFs, ReportingFrequencyPage)) controllers.optOut.newJourney.routes.ConfirmOptOutUpdateController.show(isAgent = false, taxYear.toString).url
      else controllers.optOut.oldJourney.routes.ConfirmOptOutController.show(isAgent = false).url
    val confirmOptOutURLAgent: String =
      if(areAllEnabled(OptOutFs, ReportingFrequencyPage)) controllers.optOut.newJourney.routes.ConfirmOptOutUpdateController.show(isAgent = true, taxYear = taxYear.toString).url
      else controllers.optOut.oldJourney.routes.ConfirmOptOutController.show(isAgent = true).url
