...
class ConfirmOptOutViewSpec extends TestSupport {
  ...
  val confirmedOptOutUrl: Boolean => String = (isAgent: Boolean) =>
    controllers.optOut.routes.ConfirmOptOutController.submit(isAgent).url
  ...
  class Setup(isAgent: Boolean = true, infoMessage: Boolean = false, quarterlyUpdates: Int = 0) {
    ...
    val pageDocument: Document = {
      Jsoup.parse(
        contentAsString(
          confirmOptOutView(
            viewModel = OneYearOptOutCheckpointViewModel(
  ...
  "Opt-out confirm view" when {
    ...
    "infoMessage == false, not show the info message when " should {
      ...
      "with quarterly updates as 4 count" in new Setup(isAgent = false, quarterlyUpdates = expectedQuarterlyUpdates) {
        pageDocument
          .getElementById("warning-inset")
          .text()
          .startsWith("You have 4 quarterly updates submitted") shouldBe true
      }
    }

    "infoMessage == true, show the info message" should {

      "have the correct summary heading and page contents with info message" in new Setup(
        isAgent = false,
        infoMessage = true
      ) {
        pageDocument.getElementById("summary").text() shouldBe ConfirmOptOutMessages.summary
      ...
      "have the correct summary heading and page contents for Agents with info message" in new Setup(
        isAgent = true,
        infoMessage = true
      ) {
        pageDocument.getElementById("summary").text() shouldBe ConfirmOptOutMessages.summary
