...
class PaymentHistoryViewSpec extends ViewSpec with ImplicitDateFormatter {
  ...
  object PaymentHistoryMessages {
    ...
    val titlePaymentRefundEnabled: String =
      messages("htmlTitle", messages("paymentHistory.paymentAndRefundHistory.heading"))

    val info: String =
      s"${messages("PaymentHistory.classicSA")} ${messages("taxYears.oldSa.content.link")} ${messages("pagehelp.opensInNewTabText")}."
    ...
    (
      ...
      List(
        PaymentHistoryEntry(
          ...
          transactionId = Some("TRANS123"),
          linkUrl = "link1",
          visuallyHiddenText = "hidden-text1"
    ...
    (
      ...
      List(
        PaymentHistoryEntry(
          ...
          None,
          None,
          s"refund-to-taxpayer/$repaymentRequestNumber",
          repaymentRequestNumber
        )(dateServiceInterface),
        PaymentHistoryEntry(
          "2021-08-21",
          Repayment,
  ...
  val expectedDatesOrder =
    List("25 December 2020", "13 April 2020", "25 December 2019", "25 September 2019", "25 April 2019", "25 April 2018")
  ...
  val viewModel =
    PaymentCreditAndRefundHistoryViewModel(paymentHistoryAndRefundsEnabled = false, creditsRefundsRepayEnabled = false)

  class PaymentHistorySetup(
      testPayments:                           List[(Int, List[PaymentHistoryEntry])] = groupedRepayments,
      paymentCreditAndRefundHistoryViewModel: PaymentCreditAndRefundHistoryViewModel = viewModel,
      saUtr:                                  Option[String] = Some("1234567890"),
      isAgent:                                Boolean = false)
      extends Setup(
        paymentHistoryView(
          testPayments,
          paymentCreditAndRefundHistoryViewModel,
          paymentHistoryAndRefundsEnabled = true,
          "testBackURL",
          saUtr,
          isAgent = isAgent
  ...
  class PaymentHistorySetup1(
      paymentsnotFull: List[(Int, List[PaymentHistoryEntry])],
      saUtr:           Option[String] = Some("1234567890"))
      extends Setup(
        paymentHistoryView(
          paymentsnotFull,
          PaymentCreditAndRefundHistoryViewModel(
            paymentHistoryAndRefundsEnabled = false,
  ...
  class PaymentHistorySetupMFA(
      testPayments: List[(Int, List[PaymentHistoryEntry])],
      saUtr:        Option[String] = Some("1234567890"))
      extends Setup(
        paymentHistoryView(
          testPayments,
  ...
  val paymentHistoryMessageInfo =
    s"${messages("paymentHistory.info")} ${messages("taxYears.oldSa.agent.content.2")} ${messages("pagehelp.opensInNewTabText")}. ${messages("paymentHistory.info.2")}"
  ...
  "The payments history view" when {
    ...
    "logged in as a user" when {
      ...
      "the user has payment history for a single Year" should {
        ...
        s"display correct content" in new PaymentHistorySetup(
          List(
            (
              2020,
              List(
        ...
        s"display Review and Reconcile Credits" in new PaymentHistorySetup(
          List(
            (
              2020,
              List(
        ...
        "has payment and refund history title when CreditsRefundsRepay OFF / PaymentHistoryRefunds ON" in
          new PaymentHistorySetup(
            paymentCreditAndRefundHistoryViewModel = PaymentCreditAndRefundHistoryViewModel(false, true)
          ) {
        ...
        "has payment and credit history title when CreditsRefundsRepay ON / PaymentHistoryRefunds OFF" in
          new PaymentHistorySetup(
            paymentCreditAndRefundHistoryViewModel = PaymentCreditAndRefundHistoryViewModel(true, false)
          ) {
        ...
        "has payment, credit and refund history title when CreditsRefundsRepay ON / PaymentHistoryRefunds ON" in
          new PaymentHistorySetup(
            paymentCreditAndRefundHistoryViewModel = PaymentCreditAndRefundHistoryViewModel(true, true)
          ) {
            document.title() shouldBe messages(
              "htmlTitle",
              messages("paymentHistory.paymentCreditAndRefundHistory.heading")
            )
            layoutContent.selectHead("h1").text shouldBe messages(
              "paymentHistory.paymentCreditAndRefundHistory.heading"
            )
          }
        "has payment history title when CreditsRefundsRepay OFF / PaymentHistoryRefunds OFF" in
          new PaymentHistorySetup(
            paymentCreditAndRefundHistoryViewModel = PaymentCreditAndRefundHistoryViewModel(false, false)
          ) {
      ...
      s"the user has has a payment history for multiple years" should {
        s"have the table caption" in new PaymentHistorySetup(paymentEntriesMFA) {
          layoutContent
            ...
            .selectHead("caption")
            .text
            .contains(PaymentHistoryMessages.partialH2Heading)
        }
        s"have table headings for each table column" in new PaymentHistorySetup(paymentEntriesMFA) {
          val row: Element =
            layoutContent.selectHead("div").selectNth("div", 2).selectHead("table").selectHead("thead").selectHead("tr")
        ...
        s"have the information  ${PaymentHistoryMessages.info}" in new PaymentHistorySetup(paymentEntriesMFA) {
          ...
          layoutContent
            ...
            .hasCorrectLink(
              PaymentHistoryMessages.saLink,
              "http://localhost:8930/self-assessment/ind/1234567890/account"
            )
        }

        s"not have the information  ${PaymentHistoryMessages.info} when no utr is provided" in new PaymentHistorySetup(
          paymentEntriesMFA,
          saUtr = None
        ) {
    ...
    "logged as an Agent" should {
      ...
      s"have the information ${messages("paymentHistory.info")}" in new PaymentHistorySetup(
        paymentEntriesMFA,
        isAgent = true
      ) {
      ...
      s"should have a refund block with correct relative link" in new PaymentHistorySetup(
        groupedRepayments,
        saUtr = None,
        isAgent = true
      ) {
  ...
  class PaymentHistorySetupWhenAgentView(
      testPayments: List[(Int, List[PaymentHistoryEntry])],
      saUtr:        Option[String] = Some("1234567890"))
      extends Setup(
        paymentHistoryView(
