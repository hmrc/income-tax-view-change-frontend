  ...
  def executeTest(isAgent: Boolean): Unit = {
    s"${if (isAgent) "Agent" else "Individual"}: CheckYourAnswersView" should {
      "render the heading" in new Setup(isAgent) {
        document.getElementsByClass("govuk-caption-l").first().ownText() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.caption",
          "2023",
          "2024"
        )
        document.getElementsByClass("govuk-heading-l").first().text() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.heading"
        )
      }
      ...
      "render the first change link" in new Setup(isAgent) {
        document.getElementById("change-1").text() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.summary-list.change"
        )
        document.getElementById("change-1").getElementsByTag("a").attr("href") shouldBe SelectYourReasonController
          .show(isAgent, CheckMode)
          .url
      }
      "render the second change link" in new Setup(isAgent) {
        document.getElementById("change-2").text() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.summary-list.change"
        )
        document.getElementById("change-2").getElementsByTag("a").attr("href") shouldBe EnterPoaAmountController
          .show(isAgent, CheckMode)
          .url
      }
      "render the continue button" in new Setup(isAgent) {
        ...
        document
          ...
          .getElementsByTag("a")
          .attr("href") shouldBe ConfirmationForAdjustingPoaController.show(isAgent).url
      }
      "render the Confirm and Save button" in new Setup(isAgent = isAgent, poaReason = Increase) {
        document.getElementById("confirm-and-save-button").text() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.confirm-and-save"
        )
      }
      ...
      "hide the Payment On Account reason row when the user has increased the Poa after a prior adjustment" in new Setup(
        isAgent,
        poaReason = Increase
      ) {
        document.getElementsByClass("govuk-summary-list__key").size() shouldBe 1
        document.getElementsByClass("govuk-summary-list__key").get(0).text() shouldBe messages(
          "claimToAdjustPoa.checkYourAnswers.summary-list-2.key"
        )
