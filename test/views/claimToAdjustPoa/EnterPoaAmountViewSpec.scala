...
class EnterPoaAmountViewSpec extends TestSupport {
  ...
  def poAAmountViewModel(poaPreviouslyAdjusted: Option[Boolean] = Some(false), poaPartiallyPaid: Boolean = false) =
  ...
  val prePopulatedForm =
    Some(1200).fold(EnterPoaAmountForm.form)(value => EnterPoaAmountForm.form.fill(EnterPoaAmountForm(value)))

  val noInputErrorForm = EnterPoaAmountForm.form
    .withError(EnterPoaAmountForm.amount, "claimToAdjustPoa.enterPoaAmount.emptyError")
    .fill(EnterPoaAmountForm(0))
  val invalidErrorForm = EnterPoaAmountForm.form
    .withError(EnterPoaAmountForm.amount, "claimToAdjustPoa.enterPoaAmount.invalidError")
    .fill(EnterPoaAmountForm(-1))
  val sameErrorForm = EnterPoaAmountForm.form
    .withError(EnterPoaAmountForm.amount, "claimToAdjustPoa.enterPoaAmount.sameError")
    .fill(EnterPoaAmountForm(5000))
  val higherErrorForm = EnterPoaAmountForm.form
    .withError(EnterPoaAmountForm.amount, "claimToAdjustPoa.enterPoaAmount.higherError")
    .fill(EnterPoaAmountForm(7000))

  class Setup(
      isAgent:   Boolean = false,
      form:      Form[EnterPoaAmountForm] = EnterPoaAmountForm.form,
      viewModel: PaymentOnAccountViewModel = poAAmountViewModel()) {
    val view: Html = enterAmountView(
      form,
      viewModel,
      isAgent,
      controllers.claimToAdjustPoa.routes.EnterPoaAmountController.submit(isAgent, NormalMode)
    )
  ...
  "The Enter PoA Amount page" should {
    "render the correct constant page content" in new Setup {
      ...
      document.getElementById("bulletPoints").text() shouldBe {
        msg("p1") + " " + msg("bullet1") + " " + msg("bullet2")
      }

      document.getElementById("poa-amount").text shouldBe {
        msg("howMuch") + " " + msg("howMuchHint") + " £ " +
    ...
    "render the table with only Initial Amount for user on first visit" in new Setup(viewModel = poAAmountViewModel()) {
      ...
      tableBody
        ...
        .text shouldBe msg("firstPayment")
      tableBody
        .select(".govuk-table__row:nth-of-type(1)")
        .select(".govuk-table__cell:nth-of-type(1)")
        .text shouldBe poAAmountViewModel().relevantAmountOne.toCurrencyString
      tableBody
        .select(".govuk-table__row:nth-of-type(2)")
        .select(".govuk-table__header:nth-of-type(1)")
        .text shouldBe msg("secondPayment")
      tableBody
        .select(".govuk-table__row:nth-of-type(2)")
        .select(".govuk-table__cell:nth-of-type(1)")
        .text shouldBe poAAmountViewModel().relevantAmountTwo.toCurrencyString
    }
    "render the table with Initial and Adjusted amount for user who has decrease previously" in new Setup(
      viewModel = poAAmountViewModel(poaPreviouslyAdjusted = Some(true))
    ) {
      ...
      tableBody
        .select(".govuk-table__row:nth-of-type(1)")
        .select(".govuk-table__cell:nth-of-type(1)")
        .text shouldBe poAAmountViewModel(poaPreviouslyAdjusted = Some(true)).relevantAmountOne.toCurrencyString
      tableBody
        .select(".govuk-table__row:nth-of-type(1)")
        .select(".govuk-table__cell:nth-of-type(2)")
        .text shouldBe poAAmountViewModel(poaPreviouslyAdjusted = Some(true)).totalAmountOne.toCurrencyString
      tableBody
        .select(".govuk-table__row:nth-of-type(2)")
        .select(".govuk-table__header:nth-of-type(1)")
        .text shouldBe msg("secondPayment")
      tableBody
        .select(".govuk-table__row:nth-of-type(2)")
        .select(".govuk-table__cell:nth-of-type(1)")
        .text shouldBe poAAmountViewModel(poaPreviouslyAdjusted = Some(true)).relevantAmountTwo.toCurrencyString
      tableBody
        .select(".govuk-table__row:nth-of-type(2)")
        .select(".govuk-table__cell:nth-of-type(2)")
        .text shouldBe poAAmountViewModel(poaPreviouslyAdjusted = Some(true)).totalAmountOne.toCurrencyString
    }
    "render the inset text specific to the first adjustment attempt" in new Setup(
      viewModel = poAAmountViewModel(poaPartiallyPaid = true)
    ) {
      val expectedText: String = (msg("insetText.h2") + " " + msg("insetText.firstAttempt.para1") + " " + msg(
        "insetText.firstAttempt.para2"
    ...
    "render the inset text specific to the second adjustment attempt" in
      new Setup(viewModel = poAAmountViewModel(poaPartiallyPaid = true, poaPreviouslyAdjusted = Some(true))) {
        val expectedText: String = (msg("insetText.h2") + " " + msg("insetText.secondAttempt.para1") + " " + msg(
          "insetText.secondAttempt.para2"
        )).replaceAll("<b>", "").replaceAll("</b>", "")
