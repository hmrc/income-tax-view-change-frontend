...
class AmendablePOAControllerViewSpec extends TestSupport {
  ...
  def executeTest(isAgent: Boolean): Unit = {
    s"${if (isAgent) "Agent" else "Individual"}: AmendablePaymentOnAccountView" should {
      "render the heading" in new Setup(isAgent) {
        document.getElementsByClass("govuk-caption-xl").text() should include(messages("paymentOnAccount.caption", "2023", "2024"))
      ...
      "render the first paragraph text" in new Setup(isAgent) {
        document.getElementById("paragraph-1-text").text() should include(messages("paymentOnAccount.p1"))
        document.getElementById("paragraph-1-text").text() should include(messages("paymentOnAccount.class4NationalInsurance.link.text"))
        document.getElementById("paragraph-1-text").text() should include(messages("paymentOnAccount.p2"))
  ...
  def getWhatYouNeedToKnowUrl(isAgent: Boolean): String =
    controllers.routes.WhatYouNeedToKnowController.show(isAgent).url
