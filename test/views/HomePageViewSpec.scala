...
class HomePageViewSpec extends TestSupport with FeatureSwitching {
  ...
  def testMtdItUser(
      hasSAUtr:                    Boolean = true,
      optIncomeSourceDetailsModel: Option[IncomeSourceDetailsModel] = None
    ): MtdItUser[_] = {
  ...
  def testMtdItUserMigrated(hasSAUtr: Boolean = true): MtdItUser[_] =
    testMtdItUser(hasSAUtr, Some(getIncomeSource(true)))

  def testMtdItUserNotMigrated(hasSAUtr: Boolean = true): MtdItUser[_] =
    testMtdItUser(hasSAUtr, Some(getIncomeSource(false)))
  ...
  val overdueMessage =
    "! Warning You have overdue charges. You may be charged interest on these until they are paid in full."
  val overdueMessageForDunningLocks =
    "! Warning You have overdue payments and one or more of your tax decisions are being reviewed. You may be charged interest on these until they are paid in full."
  ...
  private val viewModelThreeOverdue: NextUpdatesTileViewModel = NextUpdatesTileViewModel(
    Seq(LocalDate.of(2018, 1, 1), LocalDate.of(2018, 2, 1), LocalDate.of(2018, 3, 1)),
    currentDate,
    false
  )
  ...
  val paymentTileOverdueDateLongFormat: String =
    s"OVERDUE ${paymentTileOverdueDate.format(DateTimeFormatter.ofPattern("d MMMM YYYY"))}"

  class Setup(
      paymentDueDate:                   LocalDate = nextPaymentDueDate,
    ...
    val penaltiesAndAppealsTileViewModel =
      PenaltiesAndAppealsTileViewModel(penaltiesAndAppealsIsEnabled, submissionFrequency, penaltyPoints)
  ...
  "home" should {
    ...
    "have the right keep-alive url in hmrc timeout dialog" in new Setup {
      ...
      document
        .head()
        .select("meta[name='hmrc-timeout-dialog']")
    ...
    "have an updates tile" which {
      ...
      "display an overdue tag when a single update is overdue" in new Setup(
        nextUpdatesTileViewModel = viewModelOneOverdue
      ) {
        getElementById("updates-tile").map(_.select("p:nth-child(2)").text) shouldBe Some("OVERDUE " + "1 January 2018")
      }
      "has the correct number of overdue updates when three updates are overdue" in new Setup(
        nextUpdatesTileViewModel = viewModelThreeOverdue
      ) {
    ...
    "have a payments due tile" which {
      ...
      "don't display any warning messages when no payment is overdue and no payments are accruing interest" in new Setup(
      ...
      "display an dunning lock overdue warning message when a payment is overdue" in new Setup(
        overDuePaymentsCount = 1,
        dunningLockExists = true
      ) {
        getTextOfElementById("overdue-warning") shouldBe Some(overdueMessageForDunningLocks)
      }

      "display daily interest warning when payments are accruing interest" in new Setup(
        paymentsAccruingInterestCount = 2,
        reviewAndReconcileEnabled = true
      ) {
        val dailyInterestMessage =
          "! Warning You have charges with added daily interest. These charges will be accruing interest until they are paid in full."
      ...
      "display an overdue tag when a single update is overdue" in new Setup(overDuePaymentsCount = 1) {
        getElementById("payments-tile").map(_.select("p:nth-child(2)").text) shouldBe Some(
          "OVERDUE " + paymentDateLongDate
        )
      }

      "display only the date when there are payments due but none being overdue" in new Setup(
        paymentDueDate = paymentTileFutureDate,
        overDuePaymentsCount = 0
      ) {
        document.select("#payments-tile > div > p.govuk-body").text() shouldBe paymentTileFutureDateLongFormat
      }

      "display the date and an overdue tag when there is one payment that is overdue" in new Setup(
        paymentDueDate = paymentTileOverdueDate,
        overDuePaymentsCount = 1
      ) {
        document.select("#payments-tile > div > p.govuk-body").text() shouldBe paymentTileOverdueDateLongFormat
      }

      "display daily interest tag when there are payments accruing interest" in new Setup(
        paymentsAccruingInterestCount = 2,
        reviewAndReconcileEnabled = true
      ) {
    ...
    "have a returns tile" which {
      ...
      "has a link to the view payments page" in new Setup {
        ...
        link.map(_.attr("href")) shouldBe Some(
          s"/report-quarterly/income-and-expenses/view/tax-year-summary/$currentTaxYear"
        )
      ...
      "dont have a link to the update and submit page when ITSASubmissionIntegrationEnabled is disabled" in new Setup(
        ITSASubmissionIntegrationEnabled = false
      ) {
        val link: Option[Element] = getElementById("returns-tile").map(_.select("a").get(1))
    ...
    "have a payment history tile" which {
      ...
      "has a payment history heading when payment history feature switch is disabled" in new Setup(
      ...
      "has a link to the payment and refund history page" which {
        "has payment and refund history link when CreditsRefundsRepay OFF / PaymentHistoryRefunds ON" in new Setup(
          creditAndRefundEnabled = false,
          paymentHistoryEnabled = true
        ) {
        ...
        "has payment and credit history link when CreditsRefundsRepay ON / PaymentHistoryRefunds OFF" in new Setup(
        ...
        "has payment, credit and refund history link when CreditsRefundsRepay ON / PaymentHistoryRefunds ON" in new Setup(
          creditAndRefundEnabled = true,
          paymentHistoryEnabled = true
        ) {
          val link: Option[Element] = getElementById("payment-history-tile").map(_.select("a").first)
        ...
        "has payment history link when CreditsRefundsRepay OFF / PaymentHistoryRefunds OFF" in new Setup(
          creditAndRefundEnabled = false,
          paymentHistoryEnabled = false
        ) {
          val link: Option[Element] = getElementById("payment-history-tile").map(_.select("a").first)
      ...
      "has an link to the 'How to claim a refund' for not migrated user" in new Setup(
        user = testMtdItUserNotMigrated()
      ) {
        val link: Option[Element] = getElementById("payment-history-tile").map(_.select("a").first)
    ...
    "have an Income Sources tile with feature switch enabled" which {
      ...
      "has a link to AddIncomeSourceController.show()" in new Setup(
        ...
        getElementById("income-sources-tile").map(_.select("div > p:nth-child(2) > a").text()) shouldBe Some(
          "Add a new sole trader or property income source"
        )
        getElementById("income-sources-tile").map(_.select("div > p:nth-child(2) > a").attr("href")) shouldBe Some(
          "/report-quarterly/income-and-expenses/view/income-sources/add/new-income-sources"
        )
      }
      "has a link to ManageIncomeSourceController.show()" in new Setup(
        user = testMtdItUserMigrated(),
        incomeSourcesEnabled = true
      ) {
        getElementById("income-sources-tile").map(_.select("div > p:nth-child(3) > a").text()) shouldBe Some(
          "View and manage income sources"
        )
        getElementById("income-sources-tile").map(_.select("div > p:nth-child(3) > a").attr("href")) shouldBe Some(
          "/report-quarterly/income-and-expenses/view/income-sources/manage/view-and-manage-income-sources"
        )
      }
    }
    ...
    "have a Your Businesses tile" when {
      "the new income sources journey FS is enabled" which {
        ...
        "has a link to AddIncomeSourceController.show()" in new Setup(
          ...
          getElementById("income-sources-tile").map(_.select("div > p:nth-child(2) > a").text()) shouldBe Some(
            "Add, manage or cease a business or income source"
          )
          getElementById("income-sources-tile").map(_.select("div > p:nth-child(2) > a").attr("href")) shouldBe Some(
            "/report-quarterly/income-and-expenses/view/manage-your-businesses"
          )
        }
      }
    }
    ...
    "have an Account Settings tile" when {
      "the reporting frequency page FS is enabled" which {
        ...
        "has text for reporting quarterly(voluntary)" in new Setup(
          user = testMtdItUserMigrated(),
        ...
        "has text for reporting quarterly(mandated)" in new Setup(
          user = testMtdItUserMigrated(),
          reportingFrequencyEnabled = true,
        ...
        "has text for reporting annually" in new Setup(
          user = testMtdItUserMigrated(),
          reportingFrequencyEnabled = true,
        ...
        "has a link to the reporting frequency page" in new Setup(
          user = testMtdItUserMigrated(),
          reportingFrequencyEnabled = true
        ) {
      ...
      "the reporting frequency page FS is disabled" which {
        "does not have the Account Settings tile" in new Setup(
          user = testMtdItUserMigrated(),
    ...
    "have a Penalties and Appeals tile" when {
      "PenaltiesAndAppeals FS is enabled" which {
        "has a heading" in new Setup(submissionFrequency = "Annual", penaltyPoints = 2) {
          getElementById("penalties-and-appeals-tile").map(_.select("h2").first().text()) shouldBe Some(
            "Penalties and appeals"
          )
        }
        "has a link to Self Assessment Penalties and Appeals page" in new Setup(penaltiesAndAppealsIsEnabled = true) {
          getElementById("sa-penalties-and-appeals-link").map(_.text()) shouldBe Some(
            "Check Self Assessment penalties and appeals"
          )
        ...
        "has no penalty tag if 2 points reached but User is reporting Quarterly" in new Setup(
          submissionFrequency = "Quarterly",
          penaltyPoints = 2
        ) {
    ...
    "show the 'Claim refund' link for migrated user" when {
      "the claim a refund feature switch is on" in new Setup(
        user = testMtdItUserMigrated(),
        creditAndRefundEnabled = true
      ) {
