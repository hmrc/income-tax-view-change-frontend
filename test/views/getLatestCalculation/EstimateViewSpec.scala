    ...
    "given a EstimatesViewModel containing an annual estimate" should {
      ...
      s"have an Estimated Tax Liability section" which {
        ...
        "has a section for In Year (Current) Estimate" which {
          ...
          s"has the correct estimate p1 paragraph '${messages.InYearEstimate.p1(lastTaxCalcSuccess.timestamp.get.toLocalDateTime.toLongDateTime)}'" in {
            inYearSection.getElementById("inYearP1").text shouldBe
              messages.InYearEstimate.p1(lastTaxCalcSuccess.timestamp.get.toLocalDateTime.toLongDateTime)
          }
