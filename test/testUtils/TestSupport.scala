...
trait TestSupport
  ...
  implicit lazy val mockItvcHeaderCarrierForPartialsConverter: ItvcHeaderCarrierForPartialsConverter = mock(
    classOf[ItvcHeaderCarrierForPartialsConverter]
  )
  ...
  def getIndividualUserIncomeSourcesConfigurable(
  ...
  def fakeRequestUnconfirmedClient(
      clientNino:        String = testNino,
  ...
  def fakeRequestConfirmedClient(
      clientNino:        String = "AA111111A",
      isSupportingAgent: Boolean = false
    ): FakeRequest[AnyContentAsEmpty.type] =
    fakeRequestWithActiveSession.withSession(
      sessionUtils.SessionKeys.clientFirstName   -> "Test",
  ...
  def fakePostRequestConfirmedClient(
      clientNino:        String = "AA111111A",
      isSupportingAgent: Boolean = false
    ): FakeRequest[AnyContentAsEmpty.type] =
  ...
  def agentUserConfirmedClient(): MtdItUser[_] = defaultMTDITUser(Some(testUserTypeAgent), businessesAndPropertyIncome)

  lazy val fakeRequestWithNino: FakeRequest[AnyContentAsEmpty.type] =
    fakeRequestWithActiveSession.withSession("nino" -> testNino)
