...
object BaseTestConstants extends UnitSpec with GuiceOneAppPerSuite {
  ...
  lazy val testAuthorisedAndEnrolled: AuthorisedAndEnrolledRequest[_] =
    defaultAuthorisedAndEnrolledRequest(MTDIndividual, FakeRequest())
  ...
  lazy val testMtdItUserNoIncomeSource: MtdItUser[_] = defaultMTDITUser(
    Some(testUserTypeIndividual),
    IncomeSourceDetailsModel(
      testNino,
      "",
      Some("2018"),
      List(business1.copy("", None, None, None)),
      List(propertyDetails.copy("", None, None))
    )
  )
  ...
  def testAuthSuccessResponse(
      confidenceLevel: ConfidenceLevel = testConfidenceLevel,
  ...
  def testAuthSuccessResponseOrgNoNino(confidenceLevel: ConfidenceLevel = testConfidenceLevel) =
    new ~(
      new ~(
        new ~(
          new ~(
  ...
  def testIndividualAuthSuccessWithSaUtrResponse(
      confidenceLevel: ConfidenceLevel = testConfidenceLevel,
      affinityGroup:   AffinityGroup = AffinityGroup.Individual
    ) =
    new ~(
  ...
  def testAuthAgentSuccessWithSaUtrResponse(
      confidenceLevel: ConfidenceLevel = testConfidenceLevel,
  ...
  val testAgentAuthRetrievalSuccess =
    new ~(new ~(new ~(Enrolments(Set(arnEnrolment)), Some(AffinityGroup.Agent)), testConfidenceLevel), testCredentials)
  val testAgentAuthRetrievalSuccessNoEnrolment =
    new ~(new ~(new ~(Enrolments(Set()), Some(AffinityGroup.Agent)), testConfidenceLevel), testCredentials)

  val agentAuthRetrievalSuccess = new ~(
    new ~(new ~(new ~(Enrolments(Set(arnEnrolment)), None), testCredentials), Some(AffinityGroup.Agent)),
    testConfidenceLevel
  )
