...
object ClaimToAdjustPoaTestConstants {
  ...
  def genericFinancialDetailPOA1(taxYearEnd: Int, outstandingAmount: BigDecimal = 0.0) =
    FinancialDetail(
  ...
  def genericFinancialDetailPOA2(taxYearEnd: Int, outstandingAmount: BigDecimal = 0.0) =
    FinancialDetail(
      taxYear = taxYearEnd.toString,
  ...
  def genericFinancialDetailPOA2NoTransactionID(taxYearEnd: Int, outstandingAmount: BigDecimal = 0.0) =
    FinancialDetail(
      taxYear = taxYearEnd.toString,
      mainType = Some("SA Payment on Account 2"),
      mainTransaction = Some("4930"),
  ...
  def genericDocumentDetailPOA1(taxYearEnd: Int, outstandingAmount: BigDecimal = 150.00) =
    DocumentDetail(
      taxYear = taxYearEnd,
  ...
  def genericDocumentDetailPOA2(taxYearEnd: Int, outstandingAmount: BigDecimal = 250.00) =
    DocumentDetail(
      taxYear = taxYearEnd,
  ...
  val userPOADetails2023: FinancialDetailsModel = FinancialDetailsModel(
    ...
    financialDetails = List(genericFinancialDetailPOA1(2023, 150.00), genericFinancialDetailPOA2(2023, 250.00))
  )

  def financialDetailsWithUnpaidPoas(taxYearEnd: Int): FinancialDetailsModel =
    FinancialDetailsModel(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
      documentDetails = List(genericDocumentDetailPOA1(taxYearEnd), genericDocumentDetailPOA2(taxYearEnd)),
      financialDetails =
        List(genericFinancialDetailPOA1(taxYearEnd, 150.00), genericFinancialDetailPOA2(taxYearEnd, 250.00))
    )

  def genericUserPoaDetails(taxYearEnd: Int, outstandingAmount: BigDecimal): FinancialDetailsModel =
    FinancialDetailsModel(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
      documentDetails = List(
        genericDocumentDetailPOA1(taxYearEnd, outstandingAmount = outstandingAmount),
        genericDocumentDetailPOA2(taxYearEnd, outstandingAmount = outstandingAmount)
      ),
      financialDetails = List.empty
    )

  def genericUserPoaDetailsPOA1Only(taxYearEnd: Int): FinancialDetailsModel =
    FinancialDetailsModel(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
  ...
  def genericUserPoaDetailsPOA2Only(taxYearEnd: Int): FinancialDetailsModel =
    FinancialDetailsModel(
      balanceDetails = BalanceDetails(1.00, 2.00, 3.00, None, None, None, None, None),
      documentDetails = List(genericDocumentDetailPOA1(taxYearEnd)),
      financialDetails = List.empty
  ...
  def financialDetailsErrorModel(errorCode: Int = 404): FinancialDetailsErrorModel =
    FinancialDetailsErrorModel(errorCode, "There was an error...")
  ...
  def whatYouNeedToKnowViewModel(
    ...
    ): WhatYouNeedToKnowViewModel =
    WhatYouNeedToKnowViewModel(
      poaTaxYear = TaxYear(fixedDate.getYear, fixedDate.getYear + 1),
      showIncreaseAfterPaymentContent,
      controllers.claimToAdjustPoa.routes.SelectYourReasonController.show(isAgent, NormalMode).url
    )

  def chargeHistoryModelNoPOA(taxYear: Int): ChargeHistoryModel =
    ChargeHistoryModel(
      s"${taxYear.toString}",
      "1040000124",
      LocalDate.of(taxYear, 7, 6),
      "documentDescription",
      1500,
